...
let t_open_bound_unsafe  bt = bt
let f_open_bound_unsafe  bf = bf
let f_open_quant_unsafe  qf = qf
let t_open_branch_unsafe br = br
let f_open_branch_unsafe br = br

let rec f_open_forall_unsafe f = match f.f_node with
  | Fquant (Fforall, f) ->
      let vl, n, _, f = f_open_quant_unsafe f in
      let vl', n', f = f_open_forall_unsafe f in
      vl@vl', n + n', f
  | _ -> [], 0, f

let rec f_open_exists_unsafe f = match f.f_node with
  | Fquant (Fexists, f) ->
      let vl, n, _, f = f_open_quant_unsafe f in
      let vl', n', f = f_open_exists_unsafe f in
      vl@vl', n + n', f
  | _ -> [], 0, f
