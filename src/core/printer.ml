...
(** Comments *)
let comment_regex = Re.Str.regexp "comment:\\(.*\\)"
let line_delim = Re.Str.regexp "[\n\r]+"

let check_comment attr acc =
  if Re.Str.string_match comment_regex attr.attr_string 0 then
...
let split_lines = Re.Str.split line_delim
