...
module Make(TDom : TERM_DOMAIN) = struct
  ...
  let top man e = { i = 0; t = [TDom.top (fst man) e]; c = true; }

  let canonicalize m a = List.iter (TDom.canonicalize (fst m)) a.t

  let print fmt e = List.iter (fun b ->
      TDom.print fmt b) e.t

  let is_bottom man t =
    let man = fst man in
    List.fold_left ( && ) true (List.map (TDom.is_bottom man) t.t)
  ...
  let join_one (man, _) { t; i; _ } =
    match t with
    | [] -> bottom () ()
    | t::q -> { t = [List.fold_left (TDom.join man) t q]; c = true; i; }
  ...
  let widening man a b =
    ...
    let b_leq = List.map (fun b ->
        b,
        try List.find (fun a -> TDom.is_leq (fst man) a b) a.t
        with | Not_found -> b
      ) b.t
    in
    let t = List.map (fun (k, v) -> TDom.widening (fst man) v k) b_leq in
  ...
  let forget_region (man, _) a b d =
    { d with t = List.map (TDom.forget_region man a b) d.t }

  let forget_var (man, _) v d =
    { d with t = List.map (TDom.forget_var man v) d.t }

  let forget_term (man, _) v d =
    { d with t = List.map (TDom.forget_term man v) d.t }
  ...
  let rec meet_term man term elt =
    ...
    | _ -> {elt with t = List.map (TDom.meet_term (fst man) term) elt.t }
