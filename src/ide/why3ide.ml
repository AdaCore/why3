...
let spec =
  ...
  [
    ( Key ('F', "format"),
      Hnd1 (AString, fun s -> opt_parser := Some s),
...
let env, gconfig =
  try
    ...
    in
    if Queue.is_empty files then Whyconf.Args.exit_with_usage usage_str;
    Gconfig.load_config config;
    (env, Gconfig.config ())
  with
  | e when not (Debug.test_flag Debug.stack_trace) ->
    eprintf "Anomaly while loading configuration: %a@." Exn_printer.exn_printer
      e;
    exit 1
...
let get_extra_lang =
  ...
  let languages_manager = GSourceView.source_language_manager ~default:true in
  languages_manager#set_search_path (load_path :: languages_manager#search_path);
  fun shortcut name ->
    let ll = ref None in
    fun () ->
      match !ll with
...
let try_convert s =
  try
    if Glib.Utf8.validate s then
      s
    else
      Glib.Convert.locale_to_utf8 s
  with
  | Glib.Convert.Error _ -> (
    try
      Glib.Convert.convert_with_fallback
        ~fallback:"#neither UTF-8 nor locale nor ISO-8859-15#"
        ~to_codeset:"UTF-8" ~from_codeset:"ISO_8859-15" s
    with
    | Glib.Convert.Error _ as e -> Printexc.to_string e)
...
let color_tags_list config =
  ...
  [
    ("error_line_tag", [ `BACKGROUND config.error_line_color ]);
    (* less priority *)
    ...
    ( "error_tag_msg_zone",
      [
        `BACKGROUND gconfig.error_color_msg_zone_bg;
...
(* For each view, we have to recreate the tags. TODO The type annotatin is
   necessary for the type inference to know that <name> is optional. *)
let create_colors config
  ...
  List.iter
    (fun (name, props) ->
      let (_ : GText.tag) = v#buffer#create_tag ~name props in
      ())
    list_tags
...
(* When colors are changed in the preferences, the tag need to change so we
   remove and regenerate the tags. This works only on sourceview (not on
   message_zone where tags are not added by name *)
...
let erase_color_loc (v : GSourceView.source_view) =
  ...
  buf#remove_tag_by_name "error_line_tag" ~start:buf#start_iter
    ~stop:buf#end_iter;
  buf#remove_tag_by_name "error_font_tag" ~start:buf#start_iter
    ~stop:buf#end_iter
...
(* Contains triples (tab page, source_view, label_of_tab): - tab_page is a
   unique number for each pages of the notebook - source_view is the graphical
   element inside a tab - label_of_tab is the mutable title of the tab *)
let source_view_table :
    (int * GSourceView.source_view * GMisc.label * bool) Hstr.t =
  Hstr.create 14
...
let files_need_saving () =
  Hstr.fold
    (fun _k (_, s, _, read_only) acc ->
      acc || ((not read_only) && s#source_buffer#modified))
    source_view_table false

(* Ask if the user wants to save session before exit. Exit is then delayed until
   the [Saved] notification is received *)
let exit_function_safe () = send_request Check_need_saving_req
...
let exit_function_handler b =
  if (not b) && not (files_need_saving ()) then
    ...
    let answer =
      GToolbox.question_box ~title:"Why3 saving session and files"
...
let () =
  ...
  let dir =
    try Server_utils.get_session_dir ~allow_mkdir:true files with
    | Invalid_argument s ->
...
let main_window : GWindow.window =
  ...
  (* callback to record the new size of the main window when changed, so that on
     restart the window size is the same as the last session *)
  let (_ : GtkSignal.id) =
...
(* the main window contains a vertical box, containing: 1. the menu [menubar] 2.
   an horizontal box [hb] *)
...
let (_ : GtkSignal.id) =
  main_window#connect#destroy ~callback:exit_function_safe

let (_ : GtkSignal.id) =
  main_window#event#connect#delete ~callback:(fun _ ->
      exit_function_safe ();
      true)

(* 2. horizontal box contains: 2.1 TODO: a tool box ? 2.2 a horizontal paned
   [hp] containing: 2.2.1 a scrolled window to hold the tree view of the session
   [scrolledview] 2.2.2 a vertical paned containing [vpan222] *)
...
let scrolled_session_view =
  let sv =
    GBin.scrolled_window ~hpolicy:`AUTOMATIC ~vpolicy:`AUTOMATIC
      ~shadow_type:`ETCHED_OUT ~packing:hp#add ()
  in
  hp#set_position gconfig.tree_width;
  let (_ : GtkSignal.id) =
    sv#misc#connect#size_allocate
      ~callback:(fun { Gtk.width = w; Gtk.height = _h } ->
...
(* second column: an icon identifying the object: file,theory, goal,
   transformation *)
...
(* fourth column: icon status for the object: proved/unproved, unknown, timeout,
   etc. *)
...
let view_status_column =
  ...
  let v =
    GTree.view_column ~title:column_status_title
...
let view_time_column =
  ...
  let v =
    GTree.view_column ~title:column_time_title
...
let get_node_proved id =
  try Hint.find node_id_proved id with
  | Not_found -> false

let get_node_id_pa id = Hint.find node_id_pa id

let get_obs (pa_st : pa_status) =
  match pa_st with
  | _, b, _ -> b

let get_proof_attempt (pa_st : pa_status) =
  match pa_st with
  | pa, _, _ -> pa
...
let get_node_detached id = Hint.find node_id_detached id
...
let remove_tree goals_model =
  Hint.iter
    (fun _x i ->
      try ignore (goals_model#remove i#iter) with
      | _ -> ())
    node_id_to_gtree
...
(* vpan222 contains: 2.2.2.1 a notebook containing view of the current task,
   source code etc 2.2.2.2 a vertical pan which contains [vbox2222] 2.2.2.2.1
   the input field to type commands 2.2.2.2.2 a scrolled window to hold the
   output of the commands *)

(* 2.2.2.2 a vertical pan which contains [vbox2222] 2.2.2.2.1 the input field to
   type commands [hbox22221] 2.2.2.2.2 a scrolled window to hold the output of
   the commands [message_zone] *)
let vbox2222 = GPack.vbox ()

(* 2.2.2.2.1 Horizontal box [hbox22221] [monitor] number of scheduled/running
   provers [command_entry] Commands to run on the session *)
let hbox22221 =
  GPack.hbox
...
let monitor =
  GMisc.label ~text:"  0/0/0" ~width:100 ~xalign:0.0
    ~packing:(hbox22221#pack ?from:None ?expand:None ?fill:None ?padding:None)
    ()

let command_entry =
  GEdit.entry ~placeholder_text:"type commands here" ~packing:hbox22221#add ()
...
let error_page, error_view =
  ...
  ( 0,
    GPack.vbox ~homogeneous:false
      ~packing:(fun w ->
        ignore (messages_notebook#append_page ~tab_label:label#coerce w))
      () )
...
let edited_view =
  GSourceView.source_view ~editable:false ~show_line_numbers:true
...
let message_zone =
  ...
  in
  GText.view ~editable:false ~cursor_visible:false ~packing:sv#add ()
...
let create_msg_zone_error_tag () =
  message_zone#buffer#create_tag ~name:"error_tag_msg_zone"
    [
      `BACKGROUND gconfig.error_color_msg_zone_bg;
      `FOREGROUND gconfig.error_color_msg_zone_fg;
    ]
...
let add_to_log =
  ...
  fun notif_kind s ->
    ...
    (* scroll to end of text. Since Gtk3 we must do it in idle() because of the
       "smooth scrolling". *)
    when_idle (fun () -> log_zone#scroll_to_mark `INSERT)

let clear_message_zone () =
  let buf = message_zone#buffer in
  buf#remove_tag_by_name "error_tag_msg_zone" ~start:buf#start_iter
    ~stop:buf#end_iter;
...
let notebook =
  GPack.notebook ~packing:(vpan222#pack1 ~resize:true ~shrink:true) ()
...
let (_ : GtkSignal.id) =
  ...
  notebook#misc#connect#size_allocate
    ~callback:(fun { Gtk.width = _w; Gtk.height = h } ->
...
let task_view =
  GSourceView.source_view ~editable:false ~cursor_visible:true
    ~show_line_numbers:true ~packing:scrolled_task_view#add ()
...
let create_eventbox ~read_only file =
  ...
  let () =
    ...
    if read_only then
      let (_ : GtkSignal.id) =
        close_item#connect#activate ~callback:(fun () ->
            match Hstr.find source_view_table file with
            | exception Not_found ->
            ...
            | page_number, _, _, _ ->
              ...
              Hstr.remove source_view_table file)
      in
      right_click_label_menu#append close_item
  in
...
let create_raw_source_view ~read_only f contents =
  ...
  let markup =
    if read_only then
      "<span font-style=\"italic\">" ^ fl ^ "</span>"
    else
      fl
  in
  ...
  let scrolled_source_view =
    GBin.scrolled_window ~hpolicy:`AUTOMATIC ~vpolicy:`AUTOMATIC
      ~shadow_type:`ETCHED_OUT (* ~packing:scrolled_source_view#add *) ()
  in
  let source_page =
    notebook#append_page ~tab_label:eventbox#coerce scrolled_source_view#coerce
  in
  let editable = gconfig.allow_source_editing && not read_only in
  let source_view =
    GSourceView.source_view ~auto_indent:editable
      ~insert_spaces_instead_of_tabs:true ~tab_width:2
      ~show_line_numbers:true
        (* ~right_margin_position:80 ~show_right_margin:true *)
        (* ~smart_home_end:true *)
      ~editable ~packing:scrolled_source_view#add ()
  in
...
(* Create a page with tabname [f] and buffer equal to [content] in the notebook.
   Also add a corresponding page in source_view_table. *)
let create_source_view ~read_only f content f_format =
  if not (Hstr.mem source_view_table f) then (
    let source_page, source_view =
      create_raw_source_view ~read_only f content
    in
    (* At initialization, file has not changed. When it changes, changes the
       name of the tab and update has_changed boolean. *)
    let (_ : GtkSignal.id) =
      source_view#source_buffer#connect#modified_changed ~callback:(fun () ->
          try
            let _source_page, source_view, label, read_only =
              Hstr.find source_view_table f
            in
            if not read_only then
    ...
    (* We have to create the tags for background colors for each view. They are
       not reusable from the other views. *)
  ...
  | Some loc ->
    ...
    let n, v =
      if f = "Task" then
        ...
        | Nosourceview f ->
          let s =
            try Sysutil.file_contents f with
            | exn ->
              Format.asprintf "Error with scroll_to_loc to loc='%a':@ %a"
                Loc.pp_position loc Exn_printer.exn_printer exn
          in
...
let find_current_file_view () =
  ...
  if n = 0 then
    ...
    let acc = ref None in
    Hstr.iter
      (fun k (x, v, _, _) -> if x = n then acc := Some (k, v))
      source_view_table;
    !acc
...
let reload_unsafe () =
  save_cursor_loc current_cursor_loc;
  clear_message_zone ();
...
let _ =
  command_entry#connect#paste_clipboard ~callback:(fun () ->
      let clip = GtkBase.Clipboard.get Gdk.Atom.clipboard in
      let text = GtkBase.Clipboard.wait_for_text clip in
      match text with
      | None -> ()
      | Some text ->
        saved_clip := text;
        ...
        in
        String.iter sanitize text;
        s := String.trim !s;
        GtkBase.Clipboard.set_text clip !s)

(* Restore the clipboard after pasting *)
let _ =
  command_entry#connect#after#paste_clipboard ~callback:(fun () ->
      let clip = GtkBase.Clipboard.get Gdk.Atom.clipboard in
      GtkBase.Clipboard.set_text clip !saved_clip;
      saved_clip := "")
...
(* Add a color tag on the right locations on the correct file. If the file was
   not open yet, nothing is done *)
let color_loc ?(ce = false) ~color loc =
  (* This apply a background [color] on a location given by its file view [v]
     line [l] beginning char [b] and end char [e]. *)
  ...
  if is_proved then
    ...
    for i = 0 to n - 1 do
      collapse_proven_goals_from_iter
        (goals_model#iter_children ?nth:(Some i) (Some iter))
    done
...
let move_current_row_selection_to_parent () : unit =
  (* We hijack CTRL+left shortcut when the command entry is focused *)
  if command_entry#has_focus then (
    if command_entry#position > 0 then
      command_entry#set_position (command_entry#position - 1);
    (* Move the cursor to the beginning of the current word *)
    while
      command_entry#position > 0
      && command_entry#get_chars
           ~start:(command_entry#position - 1)
           ~stop:command_entry#position
         <> " "
    do
      command_entry#set_position (command_entry#position - 1)
    done
...
let move_to_next_unproven_node_id strat =
  (* We hijack CTRL+right shortcut when the command entry is focused *)
  match strat with
  | Clever when command_entry#has_focus ->
    if
      command_entry#get_chars ~start:command_entry#position
        ~stop:(command_entry#position + 1)
      <> ""
    then
      command_entry#set_position (command_entry#position + 1);
...
(* unused let rec update_status_column_from_iter cont iter = set_status_column
   iter; match goals_model#iter_parent iter with | Some p ->
   update_status_column_from_iter cont p | None -> () *)
...
let ide_command cmd = List.exists (fun x -> fst x = cmd) ide_command_list

let interp_ide cmd =
  match cmd with
  | "up" -> move_current_row_selection_to_parent ()
  | "down" -> move_current_row_selection_to_first_child ()
  | "next" -> move_to_next_unproven_node_id Clever
  | "expand" -> expand_row ()
  | "collapse" -> collapse_row ()
  | "list_ide_command" ->
    let s =
      List.fold_left
        (fun acc x -> fst x ^ ": " ^ snd x ^ "\n" ^ acc)
        "" ide_command_list
    in
    clear_command_entry ();
    ...
    | NProofAttempt ->
      ...
      let output_text =
        ...
        | Controller_itp.Detached ->
          "detached proof attempt: parent goal has no task"
...
let (_ : GtkSignal.id) =
  goals_view#selection#connect#after#changed ~callback:(fun () ->
      Debug.dprintf debug
        "running callback of goals_view#selection#connect#after#changed@.";
...
let (_ : GtkSignal.id) =
  let callback ev =
    Debug.dprintf debug
      "running callback of goals_view#event#connect#button_press@.";
    ...
    | 3 ->
      ...
      (match goals_view#get_path_at_pos ~x ~y with
    ...
    | 1 ->
      (* Left click *)
      (* Call get-ce only when clicked on the Status of a proofattempt (which is
         unproved) *)
      let x = int_of_float (GdkEvent.Button.x ev) in
      let y = int_of_float (GdkEvent.Button.y ev) in
      (match goals_view#get_path_at_pos ~x ~y with
      | Some (path, col, _, _) ->
        if col#title = column_status_title then
          let node_id = get_node_id (goals_model#get_row_reference path)#iter in
...
let (_ : GtkSignal.id) =
  let callback ev =
    ...
    | k when k = GdkKeysyms._Return ->
      command_entry#misc#grab_focus ();
      true
    | k when k = GdkKeysyms._Left ->
      collapse_row ();
      true
    | k when k = GdkKeysyms._Right ->
      expand_row ();
      true
    | _ -> false
  in
...
let filter_why_files () =
  GFile.filter ~name:"Why3 source files" ~patterns:[ "*.why"; "*.mlw" ] ()

let select_file ~request =
  let d =
    GWindow.file_chooser_dialog ~action:`OPEN ~title:"Why3: Add file in project"
      ()
  in
...
let treat_message_notification msg =
  match msg with
  (* TODO: do something ! *)
  | Proof_error (_id, s) ->
  ...
  | Transf_error (false, _id, tr_name, arg, loc, msg, doc) ->
    if arg = "" then
      ...
      buf#remove_tag_by_name "error_tag" ~start:buf#start_iter
        ~stop:buf#end_iter;
  ...
  | Replay_Info s -> print_message ~kind:0 ~notif_kind:"Replay_info" "%s" s
  | Query_Info (_id, s) -> print_message ~kind:1 ~notif_kind:"Query_info" "%s" s
  | Query_Error (_id, s) ->
    print_message ~kind:1 ~notif_kind:"Query_error" "%s" s
  | Information s -> print_message ~kind:1 ~notif_kind:"Information" "%s" s
        ...
        | C.Done r ->
          ...
          let s =
            if gconfig.show_time_limit then
              match limit with
              | Some l ->
                Format.sprintf "%.2f [%.2f]" time l.Call_provers.limit_time
              | None -> Format.sprintf "%.2f" time
            else
        ...
        | C.Running -> (
          ...
          | Some l ->
            ...
            ^ Format.sprintf " [limit=%.2f sec., %d M]"
                l.Call_provers.limit_time l.Call_provers.limit_mem
...
let new_node ?parent id name typ detached =
  if not (Hint.mem node_id_to_gtree id) then (
...
let on_selected_rows ~multiple ~notif_kind ~action f () =
  ...
  | l -> List.iter (fun r -> send_request (f (get_node_id r#iter))) l
...
class menu_factory ~accel_path:menu_path ~accel_group:menu_group menu =
  object (self)
    method add_item ?accel_path ?(accel_group = menu_group) ?modi ?key
      ...
      item#misc#show ();
      menu#append item;
      if add_accel || accel_path <> None then (
        let accel_path =
          match accel_path with
          | None ->
            menu_path
            ^
            if use_mnemonic then
              remove_mnemonic label
            else
              label
          | Some ap -> ap
        in
...
let string_of_desc desc =
  let print_trans_desc fmt (x, r) =
    fprintf fmt "@[<hov 2>%s@\n%a@]" x Pp.formatted r
  in
  Glib.Markup.escape_text (Pp.string_of print_trans_desc desc)
...
let add_submenu_strategy (str_fac : menu_factory) (con_fac : menu_factory)
  ...
  let (_ : GMenu.menu_item) =
    str_fac#add_item name ~use_mnemonic:false ~accel_path ~key ~modi ~tooltip
      ~callback
  in
  let (_ : GMenu.menu_item) =
    con_fac#add_item name ~use_mnemonic:false ~accel_path ~add_accel:false
      ~tooltip ~callback
  in
  ()
...
let hide_context_provers, add_submenu_prover =
  ...
  let add_submenu_prover (pro_fac : menu_factory) (con_fac : menu_factory)
    ...
    let key, modi = parse_shortcut_as_key shortcut in
    let (_ : GMenu.menu_item) =
      pro_fac#add_item prover_name ~use_mnemonic:false ~accel_path ~key ~modi
        ~tooltip ~callback
    in
    let (i : GMenu.menu_item) =
      con_fac#add_item prover_name ~use_mnemonic:false ~accel_path
        ~add_accel:false ~tooltip ~callback
    in
    Hstr.add context_provers prover_parseable_name i;
    if List.mem prover_parseable_name gconfig.hidden_provers then i#misc#hide ()
  in
...
let context_factory =
  new menu_factory
    context_tools_menu ~accel_path:"" ~accel_group:tools_accel_group
...
let file_factory =
  new menu_factory file_menu ~accel_path:"<Why3-Main>/File/" ~accel_group

let (_ : GMenu.menu_item) =
  file_factory#add_item "Add file to session"
    ~tooltip:"Insert another file in the current session" ~callback:(fun () ->
...
let (_ : GMenu.menu_item) =
  ...
  in
  file_factory#add_item "Preferences" ~tooltip:"Open Preferences Window"
    ~callback

let (_ : GMenu.menu_item) =
  file_factory#add_item "Save session"
    ~tooltip:"Save the current proof session on disk" ~callback:(fun () ->
      send_request Save_req)

let (_ : GMenu.menu_item) =
  file_factory#add_item "Save files"
    ~tooltip:"Save the edited source files on disk" ~callback:save_sources

let (_ : GMenu.menu_item) =
  file_factory#add_item "_Save session and files" ~modi:primary_modifiers
    ~key:GdkKeysyms._S
...
let (_ : GMenu.menu_item) =
  file_factory#add_item "Save all and _Refresh session" ~modi:primary_modifiers
    ~key:GdkKeysyms._R
    ~tooltip:
      "Save the current proof session and the source files, then refresh the \
       proof session with updated source files"
...
let (_ : GMenu.menu_item) =
  file_factory#add_item "_Quit" ~modi:primary_modifiers ~key:GdkKeysyms._Q
    ~tooltip:
      "See the Preferences for setting the policy on automatic file saving at \
       exit."
...
let search_forward =
  ...
  let search_forward ~forward () =
    ...
    match cur_fv with
    | None ->
      print_message ~kind:0 ~notif_kind:"Search"
      ...
      let search_string =
        let find =
          if forward then
            ...
            "Backward search"
        in
        GToolbox.input_string ~title:find ~ok:"Search" ~cancel:"Cancel"
          ~text:!last_search "Type a string to find in the current document"
      in
...
let edit_factory =
  new menu_factory edit_menu ~accel_path:"<Why3-Main>/Edit/" ~accel_group

let (_ : GMenu.menu_item) =
  edit_factory#add_item "Search forward" ~modi:primary_modifiers
    ~key:GdkKeysyms._F ~tooltip:"Search in the source file."
    ~callback:(search_forward ~forward:true)

let (_ : GMenu.menu_item) =
  edit_factory#add_item "Search backward" ~modi:primary_modifiers
    ~key:GdkKeysyms._B ~tooltip:"Search backward in the source file."
...
let get_word_around_iter iter =
  ...
  let get_right_words (iter : GText.iter) =
    let rec get_words n start_iter =
      if n = 0 then raise SearchLimit;
      (* Forbid too long search *)
...
let (_ : GMenu.menu_item) =
  edit_factory#add_item "Find cursor ident" ~modi:primary_modifiers
    ~key:GdkKeysyms._L
    ~tooltip:"This finds the definition of the ident under user cursor"
    ~callback:find_cursor_ident

let (_ : GMenu.menu_item) =
  edit_factory#add_item "Back" ~modi:primary_modifiers
...
let tools_factory =
  new menu_factory
    tools_menu ~accel_path:"<Why3-Main>/Tools/" ~accel_group:tools_accel_group

let strategies_factory =
  let tools_submenu_strategies = tools_factory#add_submenu "Strategies" in
  tools_factory#add_separator ();
  new menu_factory
    tools_submenu_strategies ~accel_path:"<Why3-Main>/Tools/Strategies/"
    ~accel_group:tools_accel_group

let provers_factory =
  let tools_submenu_provers = tools_factory#add_submenu "Provers" in
  tools_factory#add_separator ();
  new menu_factory
    tools_submenu_provers ~accel_path:"<Why3-Main>/Tools/Provers/"
    ~accel_group:tools_accel_group
...
let view_factory =
  new menu_factory ~accel_path:"<Why3-Main>/View/" ~accel_group view_menu

let (_ : GMenu.menu_item) =
  view_factory#add_item "Enlarge font" ~modi:primary_modifiers
    ~key:GdkKeysyms._plus ~callback:enlarge_fonts

let (_ : GMenu.menu_item) =
  view_factory#add_item "Reduce font" ~modi:primary_modifiers
    ~key:GdkKeysyms._minus ~callback:reduce_fonts

let (_ : GMenu.menu_item) =
  view_factory#add_item "Collapse proved goals" ~accel_group:tools_accel_group
...
let (_ : GMenu.menu_item) =
  view_factory#add_item "Expand all"
    ~tooltip:"Expand all nodes of the tree view" ~callback:goals_view#expand_all

let (_ : GMenu.menu_item) =
  view_factory#add_item "Collapse current node" ~accel_group:tools_accel_group
    ~key:GdkKeysyms._minus ~callback:collapse_row

let (_ : GMenu.menu_item) =
  view_factory#add_item "Expand current node" ~accel_group:tools_accel_group
    ~key:GdkKeysyms._plus
    ~tooltip:"Expand current node, or its children when already expanded"
    ~callback:expand_row

let (_ : GMenu.menu_item) =
  view_factory#add_item "Go to parent node" ~modi:primary_modifiers
    ~key:GdkKeysyms._Left ~callback:move_current_row_selection_to_parent
...
let (_ : GMenu.menu_item) =
  view_factory#add_item "Select next unproven goal" ~modi:primary_modifiers
    ~key:GdkKeysyms._Right ~callback:(fun () ->
      move_to_next_unproven_node_id Clever)

let (_ : GMenu.menu_item) =
  view_factory#add_item "Go down (skipping proved goals)"
    ~modi:primary_modifiers ~key:GdkKeysyms._Down ~callback:(fun () ->
      move_to_next_unproven_node_id Next)

let (_ : GMenu.menu_item) =
  view_factory#add_item "Go up (skipping proved goals)" ~modi:primary_modifiers
    ~key:GdkKeysyms._Up ~callback:(fun () -> move_to_next_unproven_node_id Prev)
...
let help_factory =
  new menu_factory help_menu ~accel_path:"<Why3-Main>/Help/" ~accel_group

let (_ : GMenu.menu_item) =
  help_factory#add_item "Legend"
    ~callback:(show_legend_window ~parent:main_window)

let (_ : GMenu.menu_item) =
  help_factory#add_item "About"
...
let init_completion provers transformations strategies commands =
  ...
  let all_strings =
    List.fold_left
      (fun acc (shortcut, strategy) ->
        Debug.dprintf debug "string for completion: '%s' '%s'@." shortcut
          strategy;
...
let () =
  ...
  let add_submenu_transform name filter =
    ...
    let iter ((name, _) as desc) =
      let (_ : GMenu.menu_item) =
        submenu#add_item (Glib.Markup.escape_text name)
          ~use_mnemonic:false ~tooltip:(string_of_desc desc)
          ~callback:(fun () -> interp name)
      in
      ()
    in
  ...
  in
  add_submenu_transform "Transformations (a-e)" (fun (x, _) -> x < "eliminate");
  add_submenu_transform "Transformations (eliminate)" (fun (x, _) ->
      x >= "eliminate" && x < "eliminatf");
  add_submenu_transform "Transformations (e-r)" (fun (x, _) ->
      x >= "eliminatf" && x < "s");
  add_submenu_transform "Transformations (s-z)" (fun (x, _) -> x >= "s");
  tools_factory#add_separator ()
...
let (_ : GMenu.menu_item) =
  ...
  in
  tools_factory#add_item "_Edit" ~key:GdkKeysyms._E
    ~tooltip:"View or edit proof script" ~callback

let (_ : GMenu.menu_item) =
  let callback =
    on_selected_rows ~multiple:false ~notif_kind:"get-ce error"
      ~action:"Get Counterexamples" (fun id -> Command_req (id, "get-ce"))
  in
  tools_factory#add_item "_Get Counterexamples" ~key:GdkKeysyms._G
    ~tooltip:"Launch the prover with counterexamples" ~callback
...
let (_ : GMenu.menu_item) =
  ...
  in
  tools_factory#add_item "_Replay valid obsolete proofs" ~key:GdkKeysyms._R
    ~tooltip:"Replay valid obsolete proofs under the current node" ~callback

let (_ : GMenu.menu_item) =
  let callback =
    on_selected_rows ~multiple:false ~notif_kind:"Replay error"
      ~action:"replay all" (fun id -> Command_req (id, "replay all"))
  in
  tools_factory#add_item "Replay all obsolete proofs"
    ~tooltip:"Replay all obsolete proofs under the current node" ~callback
...
let (_ : GMenu.menu_item) =
  ...
  in
  tools_factory#add_item "_Clean node" ~key:GdkKeysyms._C
    ~tooltip:
      "Remove unsuccessful proofs or transformations that are under a proved \
       goal"
    ~callback
...
let (_ : GMenu.menu_item) =
  ...
  let callback () =
    let answer =
      GToolbox.question_box ~default:2 ~title:"Launching unsafe command"
        ~buttons:[ "Yes"; "No" ]
  ...
  tools_factory#add_item "Reset proofs"
    ~tooltip:"Remove all proofs attempts or transformations" ~callback

let (_ : GMenu.menu_item) =
  let callback =
    on_selected_rows ~multiple:true ~notif_kind:"Remove_subtree error"
      ~action:"remove" (fun id -> Remove_subtree id)
  in
  tools_factory#add_item "Remove node" ~key:GdkKeysyms._Delete
    ~tooltip:"Remove the selected proof attempts or transformations" ~callback

let (_ : GMenu.menu_item) =
  let callback =
    on_selected_rows ~multiple:true ~notif_kind:"Mark_obsolete error"
      ~action:"mark obsolete" (fun id -> Command_req (id, "mark"))
  in
  tools_factory#add_item "Mark _obsolete" ~key:GdkKeysyms._O
    ~tooltip:"Mark all proof attempts under the current node as obsolete"
    ~callback

let (_ : GMenu.menu_item) =
  let callback =
    on_selected_rows ~multiple:true ~notif_kind:"Interrupt error"
      ~action:"interrupt" (fun id -> Command_req (id, "interrupt"))
  in
  tools_factory#add_item "_Interrupt" ~tooltip:"Stop all running proof attempts"
    ~callback
...
let (_ : GMenu.menu_item) =
  let callback =
    on_selected_rows ~multiple:false ~notif_kind:"Focus_req error"
      ~action:"focus" (fun id -> Command_req (id, "Focus"))
  in
  tools_factory#add_item "_Focus" ~tooltip:"Focus view on the current node"
    ~callback

let (_ : GMenu.menu_item) =
  let callback () = send_request Unfocus_req in
  tools_factory#add_item "_Unfocus" ~callback

let () = tools_factory#add_separator ()

let copy_item =
  tools_factory#add_item "Copy node" ~modi:primary_modifiers ~key:GdkKeysyms._C
    ~tooltip:"Copy the current node"

let paste_item =
  tools_factory#add_item "Paste node" ~modi:primary_modifiers ~key:GdkKeysyms._V
    ~tooltip:"Paste the copied node below the current node"

(* complete the contextual menu (but only after provers and strategies, hence
   the function) *)
...
let complete_context_menu () =
  ...
  let (_ : GMenu.menu_item) =
    let callback =
      on_selected_rows ~multiple:false ~notif_kind:"Edit error" ~action:"edit"
        (fun id -> Command_req (id, "edit"))
    in
    context_factory#add_item "_Edit" ~accel_path:"<Why3-Main>/Tools/Edit"
      ~add_accel:false ~tooltip:"View or edit proof script" ~callback
  in
  ();
  ...
  let (_ : GMenu.menu_item) =
    let callback =
      on_selected_rows ~multiple:false ~notif_kind:"Replay error"
        ~action:"replay" (fun id -> Command_req (id, "replay"))
    in
    context_factory#add_item "_Replay valid obsolete proofs"
      ~accel_path:"<Why3-Main>/Tools/Replay valid obsolete proofs"
      ~add_accel:false
      ~tooltip:"Replay valid obsolete proofs under the current node" ~callback
  in
  ();
  ...
  let (_ : GMenu.menu_item) =
    ...
    context_factory#add_item "Replay all obsolete proofs"
      ~accel_path:"<Why3-Main>/Tools/Replay all obsolete proofs"
      ~add_accel:false
      ~tooltip:"Replay all obsolete proofs under the current node" ~callback
  in
  ();
  ...
  let (_ : GMenu.menu_item) =
    ...
    context_factory#add_item "_Interrupt"
      ...
      ~tooltip:"Stop all running proof attempts" ~callback
  in
  ()
...
let check_uninstalled_prover =
  ...
  fun p ->
    if not (Whyconf.Hprover.mem uninstalled_prover_seen p) then (
      Whyconf.Hprover.add uninstalled_prover_seen p ();
      let callback p u = send_request (Set_prover_policy (p, u)) in
...
let treat_notification n =
  ...
  | Node_change (id, uinfo) -> (
    ...
    | Proved b ->
      ...
      if old <> b then (
        ...
        if b then (
          if
            (* if the node newly proved is selected, then force moving the
               selection the next unproved goal *)
            is_selected_alone id
          then
          ...
          with
          | Not_found ->
            Debug.dprintf debug "Warning: no gtk row registered for node %d@."
              id
      )
    | Name_change n ->
  ...
  | Next_Unproven_Node_Id (asked_id, next_unproved_id) ->
    ...
    then (
      ...
      (* Unselect the potentially selected goal to avoid having two tasks
         selected at once when a prover successfully end. To continue the proof,
         it is better to only have the new goal selected *)
      goals_view#selection#unselect_all ();
  ...
  | New_node (id, parent_id, typ, name, detached) -> (
    try
      ...
      match typ with
      | NTransformation ->
        (* if this new node is a transformation, and its parent goal is
           selected, then ask for the next goal to prove. *)
        if is_selected_alone parent_id then
          send_request (Get_first_unproven_node (Clever, parent_id))
      | _ -> ()
    with
    | Not_found -> ignore (new_node id name typ detached))
  | Remove id ->
    let n = get_node_row id in
    (if get_node_detached id then (
       ...
       let is_ancestor =
         List.exists
           (fun row ->
             let row_id = get_node_id row#iter in
             row_id = id
             || goals_model#is_ancestor ~iter:n#iter ~descendant:row#iter)
       ...
       if is_ancestor then
         ...
         | Some parent ->
           goals_view#selection#unselect_all ();
           select_iter parent
     (* TODO Go to the next unproved goal ? let parent_id = get_node_id parent
        in send_request (Get_first_unproven_node parent_id)*));
  ...
  | Task (id, s, list_loc, goal_loc, lang) ->
    if is_selected_alone id then (
      ...
      (* Avoid erasing colors at startup when selecting the first node. In all
      ...
      (* Scroll to the end of task text without any animation. We cannot use the
         ...
         gtk_adjustment_set_value (through get_adjustment_clamp_page, because of
         GTK2). We cannot do that immediately though, as the vertical adjustment
         does not yet reflect the new content. *)
      when_idle (fun () ->
  ...
  | File_contents (file_name, content, f_format, read_only) -> (
    ...
    try
      let _, sc_view, _, _source_view_is_read_only =
        Hstr.find source_view_table file_name
      in
    ...
    with
    | Not_found -> create_source_view ~read_only file_name content f_format)
  ...
  | Dead _ ->
    print_message ~kind:1 ~notif_kind:"Server Dead ?"
      "Server sent the notification '%a'. Please report." print_notify n
  | Ident_notif_loc loc -> scroll_to_loc ~force_tab_switch:true (Some loc));
  ()
...
let () =
  let (_ : GtkSignal.id) =
    goals_view#event#connect#focus_in ~callback:(fun _ ->
        main_window#add_accel_group tools_accel_group;
        true)
  in
  let (_ : GtkSignal.id) =
    goals_view#event#connect#focus_out ~callback:(fun _ ->
        GtkWindow.Window.remove_accel_group main_window#as_window
          tools_accel_group;
        true)
  in
  ()
...
let pos_cursor_and_color (v : GSourceView.source_view) =
  ...
  Format.printf "Cursor is placed at position: (%d, %d)@." line column;
      ...
      List.iter
        (fun (tag : GText.tag) ->
          ...
          if tag_name <> "" then
            ...
            Format.printf "Color \"%s\" from (%d, %d) to (%d, %d)@." tag_name
              !iter#line !iter#line_index iter2#line iter2#line_index)
        l;
...
let batch s =
  ...
  fun () ->
    let t = Sys.time () in
    if (not !initialization_complete) || t -. !last < 0.2 then
      true
    else
      match !cmd with
      | c :: tl ->
        cmd := tl;
        last := t;
        (match Strings.split ' ' c with
        | [ "" ] -> ()
        | [ "down" ] -> move_current_row_selection_to_down ()
        ...
        | "snap" :: cmd ->
          ...
          let cmd =
            Printf.sprintf
              "import -window \"%s\" -define png:include-chunk=none %s"
              window_title cmd
          in
          if Sys.command cmd <> 0 then
            Printf.eprintf "Batch command failed: %s\n%!" cmd
        | "color" :: cmd ->
          let cmd = Strings.join " " cmd in
          let v =
            Hstr.fold
              (fun _ x acc ->
                match (acc, x) with
                | None, (sp, sv, _, _) when sp = 1 -> Some sv
                | _ -> acc)
...
let () =
  Scheduler.timeout ~ms:Controller_itp.default_delay_ms (fun () ->
      List.iter treat_notification (get_notified ());
      true);
  main_window#add_accel_group accel_group;
  main_window#set_icon (Some !Gconfig.why_icon);
  print_message ~kind:1 ~notif_kind:"Info"
    "Welcome to Why3 IDE\ntype 'help' for help\n";
  (match !opt_batch with
  | Some s ->
    let (_ : GMain.Idle.id) =
      GMain.Idle.add ~prio:300 (backtrace_and_exit (batch s))
    in
    ()
  | None -> ());
  GMain.main ()
