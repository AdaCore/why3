...
let check_term ?vsenv ctx t =
  ...
  match res with
  | Some true ->
      Debug.dprintf debug_rac_check_sat "%a@." report_cntr_head (ctx, "is ok", t)
  | Some false ->
      Debug.dprintf debug_rac_check_sat "%a@." report_cntr_head (ctx, "has failed", t);
      raise (Contr (ctx, t))
  | None ->
      let msg = "cannot be evaluated" in
      Debug.dprintf debug_rac_check_sat "%a: %a@." report_cntr_title (ctx, msg) print_term t;
...
let check_model_rs rac env pm model rs =
  ...
  try
    ...
    {verdict= Bad_model; reason; exec_log= !(env.rac.exec_log)}
  with
  | Contr (ctx, t) when t.t_loc <> None && Opt.equal Loc.equal t.t_loc (Model_parser.get_model_term_loc model) ->
      ...
      {verdict= Good_model; reason; exec_log= !(ctx.c_env.rac.exec_log)}
  | Contr (ctx, t) ->
