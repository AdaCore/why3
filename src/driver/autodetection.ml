...
let generate_auto_strategies env =
  ...
  in
  List.iter (fun s -> fprintf str_formatter "c %s 1.0 1000@\n" s) provers_level1;
  ...
  let auto0 = {
      ...
      strategy_code = code }
  in
  List.iter (fun s -> fprintf str_formatter "c %s 5.0 1000@\n" s) provers_level1;
  let code = flush_str_formatter () in
  let auto1 = {
  ...
  List.iter (fun s -> fprintf str_formatter "c %s 10.0 4000@\n" s) provers_level1;
  let code = flush_str_formatter () in
  let auto2 = {
  ...
  List.iter (fun s -> fprintf str_formatter "c %s 1.0 1000@\n" s) provers_level3;
  fprintf str_formatter "t split_vc start@\n";
  List.iter (fun s -> fprintf str_formatter "c %s 5.0 2000@\n" s) provers_level3;
  ...
  List.iter (fun s -> fprintf str_formatter "c %s 30.0 4000@\n" s) provers_level3;
  let code = flush_str_formatter () in
  let auto3 = {
