...
(** A basic strategy is defined by a program declared under a simple
    assembly-like form: instructions are indexed by integers starting from 0
    (the initial instruction counter). An instruction is either
...
type proof_tree =
  ...
  | Sapply_trans of string * string list * proof_tree list
      (** `Sapply_trans(trans, trans_args, strats)` : execute the transformation
          with the specified args, then for each generated subgoals execute the
          corresponding strat in strats. The length of the strat list must match
          the length of the generated subgoal list. *)
  | Scont of string * string list * (Env.env -> Task.task -> proof_tree)
      (** `Scont(trans, trans_args, strat_fn)` : execute the transformation with
          the specified args, then for each generated subgoals generate a new
          strat by applying `strat_fn` to the task of the subgoal. The new strat
          is then executed of the subgoal. Note that unlike with `Sapply_trans`,
          you don't need to know in advance the number of subgoals generated by
          the transformation. *)
  | Scall_prover of
      ...
      (** `Scall_prover((prover, timelimit, memlimit, steplimit) list, strat)` :
          Try to prove the goal with the specified provers. If no value is given
          for the limits, the default one is used. If no prover proves the goal,
          then strat is applied on it. *)
...
val register_strat :
  desc:Pp.formatted -> string -> strat -> unit
