...
let print_status fmt st =
  ...
  | Done r            -> fprintf fmt "Done(%a)" (Call_provers.print_prover_result ~colorize:false ?json:None ?check_ce:None) r
...
let print_report fmt (r: report) =
  ...
  | Result (new_r, old_r) ->
    Format.fprintf fmt "new_result = %a, old_result = %a@."
      (Call_provers.print_prover_result ~colorize:false ?json:None ?check_ce:None) new_r
      (Call_provers.print_prover_result ~colorize:false ?json:None ?check_ce:None) old_r
  | CallFailed e ->
                ...
                let callback paid st =
                  ...
                  | Done res ->
                     ...
                     Debug.dprintf debug "Bisecting: %a.@."
                       (Call_provers.print_prover_result ~colorize:false ?json:None ~check_ce:false) res
                  end
                in
