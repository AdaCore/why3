...
type statement_node = private
    ...
    (** remove given var from the given statement *)
    | Sbreak
...
val s_drop : string -> Abstract.why_var -> statement -> statement
(** [s_drop t v s] builds the drop statement [drop v in s], with tag [t].  *)
