  ...
  let infer_loop_invariants ~widening env pmod =
    ...
    let clone_infer_pdecl pdecl sm =
      ...
      | PDpure ->
         ...
         let open Decl in
         create_pure_decl_l (List.filter is_not_vc pdecl.pd_pure), sm
      | PDlet (LDsym (rs1, {c_node = Capp (rs2,pvl)})) ->
         ...
         let rs2 = Mrs.find_def rs2 rs2 sm in
         ...
         create_let_decl let_defn, Mrs.add rs1 new_rs sm
      ...
      | PDlet (LDsym (rs, ({c_node = Cfun e} as cexp))) ->
         ...
         let ce = c_rs_subst sm ce in
         let let_defn, new_rs =
           let_sym id ~ghost:(rs_ghost rs) ~kind:(rs_kind rs) ce in
         create_let_decl let_defn, Mrs.add rs new_rs sm
      ...
      | PDlet (LDvar (pv, e)) ->
         ...
         let e = e_rs_subst sm e in
         let let_defn = let_var_raw pv e in
      ...
      | PDlet (LDrec lrd) ->
         ...
         let rec_defn {rec_sym;rec_rsym;rec_fun;rec_varl} =
           rec_rsym,c_rs_subst sm rec_fun,rec_varl,rs_kind rec_rsym in
         let let_defn, new_lrd = let_rec (List.map rec_defn lrd) in
         let sm = List.fold_left2 (fun sm rd1 rd2 ->
                      Mrs.add rd1.rec_sym rd2.rec_sym sm) sm lrd new_lrd in
         create_let_decl let_defn, sm
      | PDexn _ | PDtype _ -> pdecl, sm
    in

    let rec add_to_pmod (pmod_uc,sm) decl =
      match decl with
      | Udecl pdecl ->
         let cloned_pdecl, sm = clone_infer_pdecl pdecl sm in
         add_pdecl ~warn:true ~vc:true pmod_uc cloned_pdecl, sm
      | Uclone mod_inst -> add_clone pmod_uc mod_inst, sm
      | Umeta (m, margs) -> add_meta pmod_uc m margs, sm
      | Uscope (s, mis) ->
         let (s,sm) = List.fold_left add_to_pmod (open_scope pmod_uc s, sm) mis in
         close_scope s ~import:true, sm
      | Uuse pmod -> use_export pmod_uc pmod, sm
    in
