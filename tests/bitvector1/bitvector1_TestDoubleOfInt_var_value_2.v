...
Axiom realx_a_mx : forall (x:Z),
  ((((pow21 x) * (pow21 31%Z))%R * (pow21 (-x)%Z))%R = (pow21 31%Z)).

Definition var_as_double(x:Z): R := (double_of_bv64 (var x)).

Axiom var_value0 : forall (x:Z),
  ((var_as_double x) = ((1%R * (pow21 (1075%Z - 1023%Z)%Z))%R * (1%R + (((pow21 31%Z) + (IZR x))%R * (pow21 (-52%Z)%Z))%R)%R)%R).

(* YOU MAY EDIT THE CONTEXT BELOW *)

(* DO NOT EDIT BELOW *)

Theorem var_value : forall (x:Z),
