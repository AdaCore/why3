...
Axiom nth_from_int_high_odd : forall (n:Z) (i:Z), (((i <  64%Z)%Z /\
  (0%Z <  i)%Z) /\ ((mod1 (div n (pow_of2 i)) 2%Z) = 1%Z)) ->
...
Axiom sign_value_true : ((sign_value true) = 1%R).

Axiom sign_value_false : ((sign_value false) = (-1%R)%R).

Axiom zero : forall (b:bv), (((to_nat_sub b 62%Z 52%Z) = 0%Z) /\
  ((to_nat_sub b 51%Z 0%Z) = 0%Z)) -> ((double_of_bv64 b) = 0%R).

Axiom sign_of_double_positive : forall (b:bv), ((nth b 63%Z) = true) ->
  (0%R <= (double_of_bv64 b))%R.

Axiom sign_of_double_negative : forall (b:bv), ((nth b 63%Z) = false) ->
  ((double_of_bv64 b) <  0%R)%R.

Axiom double_of_bv64_value : forall (b:bv), ((0%Z <  (to_nat_sub b 62%Z
  52%Z))%Z /\ ((to_nat_sub b 62%Z 52%Z) <  2047%Z)%Z) ->
  ((double_of_bv64 b) = (((sign_value (nth b 63%Z)) * (IZR (power 2%Z
  ((to_nat_sub b 62%Z 52%Z) - 1023%Z)%Z)))%R * (IZR (1%Z + ((to_nat_sub b
  51%Z 0%Z) * (power 2%Z (1%Z - 64%Z)%Z))%Z)%Z))%R).

(* YOU MAY EDIT THE CONTEXT BELOW *)

(* DO NOT EDIT BELOW *)

Theorem to_nat_sub_jqt : ((to_nat_sub (from_int 9223372036854775808%Z) 7%Z
  0%Z) = 0%Z).
