...
Axiom x_negative2 : forall (x:Z), ((nth1 (var x) 31%Z) = false) ->
  ((to_nat_sub1 (var x) 30%Z 0%Z) = ((pow2 31%Z) + x)%Z).
...
intros.
rewrite to_nat_of_zero21 with (i:=30).
2:auto with *.
now rewrite x_negative2.
intros.
assert (h:(k=31 \/ k >= 32)) by omega.
destruct h.
subst k; auto.
rewrite nth_var3.
auto.
