...
Theorem from_int2c_to_nat_sub : forall (x:Z), (0%Z <= x)%Z ->
...
apply Z_lt_induction with 
   (P:= fun x => 0 <= x -> to_nat_sub (from_int2c x) 31 0 = x); auto with zarith.
intros x0 Hind Hxpos.
assert (h: x0 = 0 \/ x0 > 0) by omega.
destruct h.
subst x0.
rewrite to_nat_of_zero;auto with zarith.
intros.
apply nth_from_int2c_0;auto with zarith.
