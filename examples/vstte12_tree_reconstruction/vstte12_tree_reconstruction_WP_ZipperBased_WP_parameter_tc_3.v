...
Theorem WP_parameter_tc : forall (left1:(list (Z* tree)%type)),
  forall (right1:(list (Z* tree)%type)), ((right1 = (Nil :(list (Z*
  tree)%type))) -> (left1 = (Nil :(list (Z* tree)%type)))) ->
  match right1 with
  | (Cons x x1) =>
      match x1 with
      | Nil => True
      | _ =>
          ...
          | (x2, x3) =>
              ...
              | (Cons x4 x5) =>
                  match x4 with
                  | (x6, x7) => (x6 = x2) -> (((lex (
                      ((length x5) + (length (Cons ((x6 - 1%Z)%Z, (Node x7
                      x3)) x1)))%Z, (length (Cons ((x6 - 1%Z)%Z, (Node x7
                      x3)) x1))) (((length left1) + (length right1))%Z,
                      (length right1))) /\ (((Cons ((x6 - 1%Z)%Z, (Node x7
                      x3)) x1) = (Nil :(list (Z* tree)%type))) ->
                      (x5 = (Nil :(list (Z* tree)%type))))) ->
