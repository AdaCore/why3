...
Open Scope Z_scope.
Require Import Why3.
Ltac ae := why3 "alt-ergo" timelimit 10; admit.
...
(* Why3 intros b1 b2 j (h1,h2). *)
intros b1  b2 j (h1 & h2).
generalize h1 h2.
pattern j; apply Z_lt_induction; auto.
clear j h1 h2.
intros j Hind h1 h2.
assert (h:j = 0 \/ j > 0) by omega.
destruct h.
ae.

assert (h:(nth1 (concat b1 b2) j = nth b2 j)).
apply concat_low;auto with zarith.

assert (h3:(nth1 (concat b1 b2) j) = true \/ (nth1 (concat b1 b2) j) = false).
destruct (nth1 (concat b1 b2) j);auto.
destruct h3.
