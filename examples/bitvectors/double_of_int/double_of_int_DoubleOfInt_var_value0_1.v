...
Axiom from_int2c_to_nat_sub_neg :
  forall (i:Z), ((0%Z <= i)%Z /\ (i <= 31%Z)%Z) -> forall (x:Z),
  (((-(pow2 i))%Z <= x)%Z /\ (x < 0%Z)%Z) ->
  ((to_nat_sub (from_int2c x) (i - 1%Z)%Z 0%Z) = ((pow2 i) + x)%Z).

Axiom lemma1_neg :
  forall (x:Z), ((is_int32 x) /\ (x < 0%Z)%Z) ->
  ((to_nat_sub (jpxor x) 31%Z 0%Z) = ((pow2 31%Z) + x)%Z).
...
Axiom nth_var_0_31 :
  forall (x:Z), forall (i:Z),
  ((is_int32 x) /\ ((0%Z <= i)%Z /\ (i <= 31%Z)%Z)) ->
  ((nth1 (var x) i) = (nth (jpxor x) i)).

Axiom to_nat_bv32_bv64_aux :
  forall (b1:bv), forall (b2:bv), forall (j:Z),
  ((0%Z <= j)%Z /\ (j < 32%Z)%Z) ->
  ((to_nat_sub1 (concat b1 b2) j 0%Z) = (to_nat_sub b2 j 0%Z)).

Axiom to_nat_bv32_bv64 :
  forall (b1:bv), forall (b2:bv),
  ((to_nat_sub1 (concat b1 b2) 31%Z 0%Z) = (to_nat_sub b2 31%Z 0%Z)).

Axiom to_nat_var_0_31 :
  forall (x:Z), (is_int32 x) ->
  ((to_nat_sub1 (var x) 31%Z 0%Z) = (to_nat_sub (jpxor x) 31%Z 0%Z)).

Axiom nth_var32to63 :
  forall (x:Z) (k:Z), ((32%Z <= k)%Z /\ (k <= 63%Z)%Z) ->
  ((nth1 (var x) k) = (nth (from_int 1127219200%Z) (k - 32%Z)%Z)).

Axiom nth_var3 :
  forall (x:Z) (i:Z), ((is_int32 x) /\ ((32%Z <= i)%Z /\ (i <= 51%Z)%Z)) ->
  ((nth1 (var x) i) = false).
...
Axiom nth_var4 :
  forall (x:Z) (i:Z), ((is_int32 x) /\ ((52%Z <= i)%Z /\ (i <= 53%Z)%Z)) ->
  ((nth1 (var x) i) = true).

Axiom nth_var5 :
  forall (x:Z) (i:Z), ((is_int32 x) /\ ((54%Z <= i)%Z /\ (i <= 55%Z)%Z)) ->
  ((nth1 (var x) i) = false).

Axiom nth_var6 :
  forall (x:Z) (i:Z), ((is_int32 x) /\ ((56%Z <= i)%Z /\ (i <= 57%Z)%Z)) ->
  ((nth1 (var x) i) = true).

Axiom nth_var7 :
  forall (x:Z) (i:Z), ((is_int32 x) /\ ((58%Z <= i)%Z /\ (i <= 61%Z)%Z)) ->
  ((nth1 (var x) i) = false).

Axiom nth_var8 : forall (x:Z), (is_int32 x) -> ((nth1 (var x) 62%Z) = true).

Axiom lemma3 :
  forall (x:Z), (is_int32 x) -> ((to_nat_sub1 (var x) 62%Z 52%Z) = 1075%Z).

Axiom nth_var9 : forall (x:Z), (is_int32 x) -> ((nth1 (var x) 63%Z) = false).

Axiom lemma4 : forall (x:Z), (is_int32 x) -> ((nth1 (var x) 63%Z) = false).

Axiom sign_var : forall (x:Z), (is_int32 x) -> ((nth1 (var x) 63%Z) = false).
...
Theorem var_value0 :
  forall (x:Z), (is_int32 x) ->
  ((var_as_double x) =
   ((pow21 (1075%Z - 1023%Z)%Z) *
    (1%R + ((BuiltIn.IZR ((pow2 31%Z) + x)%Z) * (pow21 (-52%Z)%Z))%R)%R)%R).
(* Why3 intros x h1. *)
(* intros x h1. *)
intros.
unfold var_as_double.
rewrite double_of_bv64_value.
rewrite sign_var; auto.
ae.
ae.
Admitted.
