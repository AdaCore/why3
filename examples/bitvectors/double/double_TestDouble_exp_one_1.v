...
Axiom nth_sign_positive :
  forall (n:Numbers.BinNums.Z), (0%Z <= n)%Z ->
  ((nth (from_int2c n) (64%Z - 1%Z)%Z) = Init.Datatypes.false).

Axiom nth_sign_negative :
  forall (n:Numbers.BinNums.Z), (n < 0%Z)%Z ->
  ((nth (from_int2c n) (64%Z - 1%Z)%Z) = Init.Datatypes.true).
...
Parameter double_of_bv64: bv -> Reals.Rdefinitions.R.
...
Axiom zero :
  forall (b:bv),
  ((to_nat_sub b 62%Z 52%Z) = 0%Z) /\ ((to_nat_sub b 51%Z 0%Z) = 0%Z) ->
  ((double_of_bv64 b) = 0%R).

Axiom sign_of_double_positive :
  forall (b:bv), ((nth b 63%Z) = Init.Datatypes.false) ->
  (0%R <= (double_of_bv64 b))%R.

Axiom sign_of_double_negative :
  forall (b:bv), ((nth b 63%Z) = Init.Datatypes.true) ->
  ((double_of_bv64 b) <= 0%R)%R.

Axiom double_of_bv64_value :
  forall (b:bv),
  (0%Z < (to_nat_sub b 62%Z 52%Z))%Z /\ ((to_nat_sub b 62%Z 52%Z) < 2047%Z)%Z ->
  ((double_of_bv64 b) =
   (((sign_value (nth b 63%Z)) *
     (pow21 ((to_nat_sub b 62%Z 52%Z) - 1023%Z)%Z))%R
    *
    (1%R + ((BuiltIn.IZR (to_nat_sub b 51%Z 0%Z)) * (pow21 (-52%Z)%Z))%R)%R)%R).

Axiom nth_one1 :
  forall (i:Numbers.BinNums.Z), (0%Z <= i)%Z /\ (i <= 51%Z)%Z ->
  ((nth (from_int 4607182418800017408%Z) i) = Init.Datatypes.false).

Axiom nth_one2 :
  forall (i:Numbers.BinNums.Z), (52%Z <= i)%Z /\ (i <= 61%Z)%Z ->
  ((nth (from_int 4607182418800017408%Z) i) = Init.Datatypes.true).

Axiom nth_one3 :
  forall (i:Numbers.BinNums.Z), (62%Z <= i)%Z /\ (i <= 63%Z)%Z ->
  ((nth (from_int 4607182418800017408%Z) i) = Init.Datatypes.false).

Axiom sign_one :
  ((nth (from_int 4607182418800017408%Z) 63%Z) = Init.Datatypes.false).
...
(* Why3 goal *)
Theorem exp_one :
  ((to_nat_sub (from_int 4607182418800017408%Z) 62%Z 52%Z) = 1023%Z).
rewrite to_nat_sub_zero; auto with zarith.
rewrite to_nat_of_one; auto with zarith.
replace (62 - 1 - 52 + 1) with 10 by lia.
