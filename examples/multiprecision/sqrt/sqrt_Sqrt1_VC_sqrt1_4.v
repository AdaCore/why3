...
Axiom H59 : ((rval t2) = (trunc_at (rval t2') ((iexp t2') + 24%Z)%Z)).

Axiom H60 : ((iexp t2) = ((iexp t2') + 24%Z)%Z).

Parameter o10: fxp.

Axiom H61 : ((rval o10) = ((rval x1) * (rval t2))%R).

Axiom H62 : ((iexp o10) = ((iexp x1) + (iexp t2))%Z).
...
Axiom H64 : ((iexp o11) = (((iexp o10) + 15%Z)%Z - 1%Z)%Z).

Parameter x2: fxp.

Axiom H65 : ((rval x2) = ((rval u1) + (rval o11))%R).

Axiom H66 : ((iexp x2) = (iexp u1)).

(* Why3 goal *)
Theorem VC_sqrt1 :
  (~ ((rval x1) = 0%R) /\ ~ ((rval a2) = 0%R)) ->
  (((((rval u1) + (((rval x1) * (rval t2'))%R * (05 / 10)%R)%R)%R -
