...
Axiom Ensures36 : ((rval t2) = (trunc_at (rval t2') ((iexp t2') + 24%Z)%Z)).

Axiom Ensures37 : ((iexp t2) = ((iexp t2') + 24%Z)%Z).

Parameter o10: fxp.

Axiom Ensures38 : ((rval o10) = ((rval x1) * (rval t2))%R).

Axiom Ensures39 : ((iexp o10) = ((iexp x1) + (iexp t2))%Z).

Parameter o11: fxp.

Axiom Ensures40 :
  ((rval o11) =
   (trunc_at ((rval o10) * (pow2 (-1%Z)%Z))%R
    (((iexp o10) + 15%Z)%Z - 1%Z)%Z)).

Axiom Ensures41 : ((iexp o11) = (((iexp o10) + 15%Z)%Z - 1%Z)%Z).

Parameter x2: fxp.

Axiom Ensures42 : ((rval x2) = ((rval u1) + (rval o11))%R).
...
Proof.
intros (h1,h2).
rewrite Ensures34, Ensures32, Ensures30, Ensures28, Ensures24.
set (sa := sqrt (rval a)).
