...
(* Why3 goal *)
Theorem WP_parameter_sort : forall (a:Z) (data:Z), forall (data1:(map.Map.map
  Z Z)) (a1:(map.Map.map Z Z)), let data2 := (mk_array data data1) in
  let a2 := (mk_array a a1) in (((data = a) /\ (range data1 data)) ->
  ((0%Z <= (data - 1%Z)%Z)%Z -> forall (data3:(map.Map.map Z Z)),
  let data4 := (mk_array data data3) in forall (i:Z), ((0%Z <= i)%Z /\
  (i <= (data - 1%Z)%Z)%Z) -> ((((permut data2 data4) /\ (sorted_sub a2 data4
  0%Z i)) /\ (range data3 data)) -> forall (j:Z) (data5:(map.Map.map Z Z)),
  let data6 := (mk_array data data5) in ((((((((0%Z <= j)%Z /\ (j <= i)%Z) /\
  (range data5 data)) /\ (permut data2 data6)) /\ (sorted_sub a2 data6 0%Z
  j)) /\ (sorted_sub a2 data6 j (i + 1%Z)%Z)) /\ forall (k1:Z) (k2:Z),
  (((0%Z <= k1)%Z /\ (k1 < j)%Z) /\ (((j + 1%Z)%Z <= k2)%Z /\
  (k2 <= i)%Z)) -> (le a2 (map.Map.get data5 k1) (map.Map.get data5 k2))) ->
  ((0%Z < j)%Z -> (((0%Z <= j)%Z /\ (j < data)%Z) -> let o :=
  (map.Map.get data5 j) in (((0%Z <= (j - 1%Z)%Z)%Z /\
  ((j - 1%Z)%Z < data)%Z) -> let o1 := (map.Map.get data5 (j - 1%Z)%Z) in
  ((((0%Z <= o1)%Z /\ (o1 <= a)%Z) /\ ((0%Z <= o)%Z /\ (o <= a)%Z)) ->
  forall (o2:Z), ((((o2 = 0%Z) -> (o1 = o)) /\ ((o2 < 0%Z)%Z -> (le a2 o1
  o))) /\ ((0%Z < o2)%Z -> (le a2 o o1))) -> ((0%Z < o2)%Z ->
  (((0%Z <= j)%Z /\ (j < data)%Z) -> (((0%Z <= (j - 1%Z)%Z)%Z /\
  ((j - 1%Z)%Z < data)%Z) -> (((0%Z <= j)%Z /\ (j < data)%Z) ->
  forall (data7:(map.Map.map Z Z)), (data7 = (map.Map.set data5 j
  (map.Map.get data5 (j - 1%Z)%Z))) -> (((0%Z <= (j - 1%Z)%Z)%Z /\
  ((j - 1%Z)%Z < data)%Z) -> forall (data8:(map.Map.map Z Z)),
  (data8 = (map.Map.set data7 (j - 1%Z)%Z (map.Map.get data5 j))) ->
  ((exchange data5 data8 (j - 1%Z)%Z j) -> forall (j1:Z),
  (j1 = (j - 1%Z)%Z) -> (sorted_sub a2 (mk_array data data8) j1
  (i + 1%Z)%Z))))))))))))))).
intros a data data1 a1 data2 a2 (h1,h2) h3 data3 data4 i (h4,h5) ((h6,h7),h8)
j data5 data6 ((((((h9,h10),h11),h12),h13),h14),h15) h16 (h17,h18) o
