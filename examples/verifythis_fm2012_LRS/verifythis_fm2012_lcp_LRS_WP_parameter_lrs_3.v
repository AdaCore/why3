...
Definition inv(s:suffixArray): Prop :=
  ...
  ((range (elts (suffixes s)) (length (values s))) /\
  (injective (elts (suffixes s)) (length (values s)))).
...
Theorem WP_parameter_lrs : forall (a:Z), forall (a1:(map Z Z)), let a2 :=
  ...
  Z Z)) (sa2:Z) (sa3:(map Z Z)), let sa4 := (mk_suffixArray (mk_array sa sa1)
  (mk_array sa2 sa3)) in ((((sa = a) /\ (sa1 = a1)) /\ (inv sa4)) ->
  forall (solStart:Z), (solStart = 0%Z) -> forall (solLength:Z),
  (solLength = 0%Z) -> ((1%Z <= (a - 1%Z)%Z)%Z -> forall (solLength1:Z)
  (solStart1:Z), forall (i:Z), ((1%Z <= i)%Z /\ (i <= (a - 1%Z)%Z)%Z) ->
  (((((0%Z <= solLength1)%Z /\ (solLength1 <= a)%Z) /\
  ((0%Z <= solStart1)%Z /\ (solStart1 <= a)%Z)) /\ exists y:Z,
  ((0%Z <= y)%Z /\ (y <= a)%Z) /\ ((~ (solStart1 = y)) /\
  (solLength1 = (longest_common_prefix a2 solStart1 y)))) -> (((inv sa4) /\
  ((0%Z < i)%Z /\ (i < sa)%Z)) -> let l :=
  (longest_common_prefix (mk_array sa sa1) (get sa3 i) (get sa3
  (i - 1%Z)%Z)) in ((solLength1 < l)%Z -> (((inv sa4) /\ ((0%Z <= i)%Z /\
  (i < sa)%Z)) -> forall (solStart2:Z), (solStart2 = (get sa3 i)) ->
  forall (solLength2:Z), (solLength2 = l) -> exists y:Z, ((0%Z <= y)%Z /\
  (y <= a)%Z) /\ ((~ (solStart2 = y)) /\
  (solLength2 = (longest_common_prefix a2 solStart2 y))))))))))).
intros a a1 a2 h1 h2 sa sa1 sa2 sa3 sa4 (h3,h4) solStart h5 solLength h6 h7
solLength1 solStart1 i (h8,h9)
(((h10,h11),(h12,h13)),(y,((h14,h15),(h16,h17)))) (h18,(h19,h20)) l h21
(h22,(h23,h24)) solStart2 h25 solLength2 h26.
exists (get sa3 (i - 1)%Z).
why3 "alt-ergo" timelimit 3.
Qed.
