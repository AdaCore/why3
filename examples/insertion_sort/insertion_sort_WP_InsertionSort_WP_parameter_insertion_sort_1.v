...
Theorem WP_parameter_insertion_sort : forall (a:Z) (a1:(@map.Map.map Z _
  ...
  (a5 = (map.Map.set a4 j (map.Map.get a4 o2)))) -> ((map.MapPermut.exchange
  (map.Map.set a4 j v) (map.Map.set a5 (j - 1%Z)%Z v) (j - 1%Z)%Z j) ->
  forall (j1:Z), (j1 = (j - 1%Z)%Z) -> (permut a2 (mk_array a (map.Map.set a5
  j1 v))))))))))))).
