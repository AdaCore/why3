...
(* Why3 assumption *)
Definition sorted_sub (a:(@map.Map.map Z _ elt elt_WhyType)) (l:Z)
  (u:Z): Prop := forall (i1:Z) (i2:Z), ((l <= i1)%Z /\ ((i1 <= i2)%Z /\
  (i2 < u)%Z)) -> (le (map.Map.get a i1) (map.Map.get a i2)).
...
(* Why3 assumption *)
Definition sorted_sub1 (a:(@array elt elt_WhyType)) (l:Z) (u:Z): Prop :=
  (sorted_sub (elts a) l u).

(* Why3 assumption *)
Definition sorted (a:(@array elt elt_WhyType)): Prop := (sorted_sub (elts a)
  0%Z (length a)).
...
Theorem WP_parameter_insertion_sort : forall (a:Z) (a1:(@map.Map.map Z _
  ...
  o2)))) -> ((map.MapPermut.exchange (map.Map.set a4 j v) (map.Map.set a5
  (j - 1%Z)%Z v) (j - 1%Z)%Z j) -> forall (j1:Z), (j1 = (j - 1%Z)%Z) ->
  (permut a2 (mk_array a (map.Map.set a5 j1 v))))))))))))).
...
apply permut_trans with (1:=h12b).
ae.
Qed.
