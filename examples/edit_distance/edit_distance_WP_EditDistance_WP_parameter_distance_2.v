...
Theorem WP_parameter_distance : forall (w1:Z) (w2:Z),
  ...
  (t2 = (map.Map.set t1 w2 ((map.Map.get t1 w2) + 1%Z)%Z))) -> let o2 :=
  (w2 - 1%Z)%Z in ((0%Z <= o2)%Z -> forall (oldt:Z) (t3:(@map.Map.map Z _
  Z _)), forall (j:Z), ((j <= o2)%Z /\ (0%Z <= j)%Z) -> ((((forall (k:Z),
  ...
  j)) -> (((0%Z <= j)%Z /\ (j < o)%Z) -> forall (t4:(@map.Map.map Z _ Z _)),
  ((0%Z <= o)%Z /\ (t4 = (map.Map.set t3 j oldt))) -> forall (k:Z),
...
(* Why3 intros w1 w2 w21 w11 w22 w12 (h1,h2) o h3 h4 h5 t h6 o1 h7 t1 i
        ...
        (h30,h31) (h32,h33) h34 (h35,h36) t4 (h37,h38) k (h39,h40). *)
Proof.
intuition.
intuition.
