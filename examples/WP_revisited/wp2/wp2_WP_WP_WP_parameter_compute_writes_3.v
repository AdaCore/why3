...
(* Why3 goal *)
Theorem WP_parameter_compute_writes : forall (s:stmt), forall (x:term)
  (x1:stmt) (x2:stmt), (s = (Sif x x1 x2)) -> forall (o:(set.Set.set Z)),
  (forall (sigma:(map.Map.map Z value)) (pi:(map.Map.map Z value))
  (sigma':(map.Map.map Z value)) (pi':(map.Map.map Z value)) (n:Z),
  (many_steps sigma pi x2 sigma' pi' Sskip n) -> (assigns sigma o sigma')) ->
  forall (o1:(set.Set.set Z)), (forall (sigma:(map.Map.map Z value))
  (pi:(map.Map.map Z value)) (sigma':(map.Map.map Z value)) (pi':(map.Map.map
  Z value)) (n:Z), (many_steps sigma pi x1 sigma' pi' Sskip n) -> (assigns
  sigma o1 sigma')) -> forall (sigma:(map.Map.map Z value)) (pi:(map.Map.map
  Z value)) (sigma':(map.Map.map Z value)) (pi':(map.Map.map Z value)) (n:Z),
  (many_steps sigma pi s sigma' pi' Sskip n) -> (assigns sigma
  (set.Set.union o1 o) sigma').
(* Why3 intros s x x1 x2 h1 o h2 o1 h3 sigma pi sigma' pi' n h4. *)
intros s x x1 x2 H_; rewrite H_ in *;intros.
