...
Theorem progress : forall (s:stmt), forall (x:stmt) (x1:stmt), (s = (Sseq x
  x1)) -> ((forall (sigma:(map.Map.map mident value)) (pi:(list (ident*
  value)%type)) (sigmat:(map.Map.map mident datatype)) (pit:(list (ident*
  datatype)%type)) (q:fmla), (compatible_env sigma sigmat pi pit) ->
  ((type_stmt sigmat pit x1) -> ((eval_fmla sigma pi (wp x1 q)) ->
  ((~ (x1 = Sskip)) -> (reductible sigma pi x1))))) ->
  ...
  x) -> ((eval_fmla sigma pi (wp x q)) -> ((~ (x = Sskip)) -> (reductible
  sigma pi x))))) -> forall (sigma:(map.Map.map mident value))
  (pi:(list (ident* value)%type)) (sigmat:(map.Map.map mident datatype))
