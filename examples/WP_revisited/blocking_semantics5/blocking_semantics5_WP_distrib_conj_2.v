...
Axiom get_stack_def :
  forall (i:ident) (pi:(list (ident* value)%type)),
  match pi with
...
Axiom get_stack_eq :
  forall (x:ident) (v:value) (r:(list (ident* value)%type)),
  ((get_stack x (Init.Datatypes.cons (x, v) r)) = v).

Axiom get_stack_neq :
  forall (x:ident) (i:ident) (v:value) (r:(list (ident* value)%type)),
  ~ (x = i) ->
...
Fixpoint eval_fmla (sigma:mident -> value) (pi:(list (ident* value)%type))
  ...
  | (Fforall x TYint f1) =>
      forall (n:Z), eval_fmla sigma (Init.Datatypes.cons (x, Vint n) pi) f1
  | (Fforall x TYbool f1) =>
      forall (b:bool),
...
Definition valid_fmla (p:fmla) : Prop :=
  forall (sigma:mident -> value) (pi:(list (ident* value)%type)),
...
Inductive one_step: (mident -> value) -> (list (ident* value)%type) ->
  ...
  | one_step_assert :
      forall (sigma:mident -> value) (pi:(list (ident* value)%type)) (f:fmla),
      (eval_fmla sigma pi f) -> one_step sigma pi (Sassert f) sigma pi Sskip
  | one_step_while_true :
      forall (sigma:mident -> value) (pi:(list (ident* value)%type))
        (cond:term) (inv:fmla) (body:stmt),
...
Inductive many_steps: (mident -> value) -> (list (ident* value)%type) ->
  stmt -> (mident -> value) -> (list (ident* value)%type) -> stmt -> 
  Z -> Prop :=
  | many_steps_refl :
      forall (sigma:mident -> value) (pi:(list (ident* value)%type)) (s:stmt),
  ...
  | many_steps_trans :
      forall (sigma1:mident -> value) (sigma2:mident -> value)
        (sigma3:mident -> value) (pi1:(list (ident* value)%type))
        (pi2:(list (ident* value)%type)) (pi3:(list (ident* value)%type))
        (s1:stmt) (s2:stmt) (s3:stmt) (n:Z),
...
(* Why3 assumption *)
Definition reductible (sigma:mident -> value) (pi:(list (ident* value)%type))
    (s:stmt) : Prop :=
  exists sigma':mident -> value, exists pi':(list (ident* value)%type),
  exists s':stmt, one_step sigma pi s sigma' pi' s'.
...
Axiom get_vartype_def :
  forall (i:ident) (pi:(list (ident* datatype)%type)),
  match pi with
...
(* Why3 assumption *)
Inductive type_term: (mident -> datatype) -> (list (ident* datatype)%type) ->
  term -> datatype -> Prop :=
  | Type_value :
      forall (sigma:mident -> datatype) (pi:(list (ident* datatype)%type))
        (v:value),
...
(* Why3 assumption *)
Inductive type_fmla: (mident -> datatype) -> (list (ident* datatype)%type) ->
  fmla -> Prop :=
  | Type_term :
      forall (sigma:mident -> datatype) (pi:(list (ident* datatype)%type))
        (t:term),
...
Definition compatible_env (sigma:mident -> value) (sigmat:mident -> datatype)
    (pi:(list (ident* value)%type)) (pit:(list (ident* datatype)%type)) :
    Prop :=
...
Axiom eval_type_term :
  forall (t:term) (sigma:mident -> value) (pi:(list (ident* value)%type))
    (sigmat:mident -> datatype) (pit:(list (ident* datatype)%type))
    (ty:datatype),
  (compatible_env sigma sigmat pi pit) -> (type_term sigmat pit t ty) ->
  ((type_value (eval_term sigma pi t)) = ty).

Axiom type_preservation :
  forall (s1:stmt) (s2:stmt) (sigma1:mident -> value)
    (sigma2:mident -> value) (pi1:(list (ident* value)%type))
    (pi2:(list (ident* value)%type)) (sigmat:mident -> datatype)
    (pit:(list (ident* datatype)%type)),
...
Axiom Cons_append :
  ...
  forall (a1:a) (l1:(list a)) (l2:(list a)),
  ((Init.Datatypes.cons a1 (Init.Datatypes.app l1 l2)) =
   (Init.Datatypes.app (Init.Datatypes.cons a1 l1) l2)).

Axiom Append_nil_l :
  forall {a:Type} {a_WT:WhyType a},
  forall (l:(list a)), ((Init.Datatypes.app Init.Datatypes.nil l) = l).

Parameter msubst_term: term -> mident -> ident -> term.

Axiom msubst_term_def :
  forall (t:term) (x:mident) (v:ident),
  match t with
  | ((Tvalue _)|(Tvar _)) => ((msubst_term t x v) = t)
  | (Tderef y) =>
      ((x = y) -> ((msubst_term t x v) = (Tvar v))) /\
...
Axiom eval_msubst_term :
  forall (e:term) (sigma:mident -> value) (pi:(list (ident* value)%type))
    (x:mident) (v:ident),
  (fresh_in_term v e) ->
  ((eval_term sigma pi (msubst_term e x v)) =
   (eval_term (map.Map.set sigma x (get_stack v pi)) pi e)).

Axiom eval_msubst :
  forall (f:fmla) (sigma:mident -> value) (pi:(list (ident* value)%type))
    (x:mident) (v:ident),
  (fresh_in_fmla v f) ->
  (eval_fmla sigma pi (msubst f x v)) <->
  (eval_fmla (map.Map.set sigma x (get_stack v pi)) pi f).

Axiom eval_swap_term :
  forall (t:term) (sigma:mident -> value) (pi:(list (ident* value)%type))
    (l:(list (ident* value)%type)) (id1:ident) (id2:ident) (v1:value)
    (v2:value),
  ~ (id1 = id2) ->
  ((eval_term sigma
    (Init.Datatypes.app l
     (Init.Datatypes.cons (id1, v1) (Init.Datatypes.cons (id2, v2) pi)))
    t)
   =
   (eval_term sigma
    (Init.Datatypes.app l
     (Init.Datatypes.cons (id2, v2) (Init.Datatypes.cons (id1, v1) pi)))
    t)).
...
Axiom eval_swap :
  forall (f:fmla) (sigma:mident -> value) (pi:(list (ident* value)%type))
    (id1:ident) (id2:ident) (v1:value) (v2:value),
  ~ (id1 = id2) ->
  (eval_fmla sigma
   (Init.Datatypes.cons (id1, v1) (Init.Datatypes.cons (id2, v2) pi)) f) <->
  (eval_fmla sigma
...
Axiom eval_term_change_free :
  forall (t:term) (sigma:mident -> value) (pi:(list (ident* value)%type))
    (id:ident) (v:value),
  (fresh_in_term id t) ->
  ((eval_term sigma (Init.Datatypes.cons (id, v) pi) t) =
...
Axiom eval_change_free :
  forall (f:fmla) (sigma:mident -> value) (pi:(list (ident* value)%type))
    (id:ident) (v:value),
  (fresh_in_fmla id f) ->
  (eval_fmla sigma (Init.Datatypes.cons (id, v) pi) f) <->
...
Axiom abstract_effects_distrib_conj :
  forall (s:stmt) (p:fmla) (q:fmla) (sigma:mident -> value)
    (pi:(list (ident* value)%type)),
...
Axiom abstract_effects_monotonic :
  forall (s:stmt) (p:fmla) (q:fmla), (valid_fmla (Fimplies p q)) ->
  forall (sigma:mident -> value) (pi:(list (ident* value)%type)),
  (eval_fmla sigma pi (abstract_effects s p)) ->
  eval_fmla sigma pi (abstract_effects s q).
...
Axiom abstract_effects_writes :
  forall (sigma:mident -> value) (pi:(list (ident* value)%type)) (body:stmt)
    (cond:term) (inv:fmla) (q:fmla),
  let f :=
  abstract_effects body
  (Fand (Fimplies (Fand (Fterm cond) inv) (wp body inv))
   (Fimplies (Fand (Fnot (Fterm cond)) inv) q)) in
...
(* Why3 goal *)
Theorem distrib_conj :
  forall (s:stmt), forall (x:stmt) (x1:stmt), (s = (Sseq x x1)) ->
  (forall (sigma:mident -> value) (pi:(list (ident* value)%type)) (p:fmla)
     (q:fmla),
   ((eval_fmla sigma pi (wp x1 p)) /\ (eval_fmla sigma pi (wp x1 q))) ->
   eval_fmla sigma pi (wp x1 (Fand p q))) ->
  (forall (sigma:mident -> value) (pi:(list (ident* value)%type)) (p:fmla)
     (q:fmla),
   ((eval_fmla sigma pi (wp x p)) /\ (eval_fmla sigma pi (wp x q))) ->
   eval_fmla sigma pi (wp x (Fand p q))) ->
  forall (sigma:mident -> value) (pi:(list (ident* value)%type)) (p:fmla)
    (q:fmla),
  ((eval_fmla sigma pi (wp s p)) /\ (eval_fmla sigma pi (wp s q))) ->
  eval_fmla sigma pi (wp s (Fand p q)).
(* Why3 intros s x x1 h1 h2 h3 sigma pi p q (h4,h5). *)
intros s x x1 H_;rewrite H_ in *.
simpl; intros H1 H2 sigma pi p q (H3 & H4).
assert (H: valid_fmla 
        (Fimplies (Fand (wp x1 p) (wp x1 q)) (wp x1 (Fand p q)))).
