...
(* Why3 goal *)
Theorem progress : forall (s:stmt), forall (x:term) (x1:stmt) (x2:stmt),
  (s = (Sif x x1 x2)) -> ((forall (sigma:(map.Map.map mident value))
  (pi:(list (ident* value)%type)) (sigmat:(map.Map.map mident datatype))
  ...
  x1) -> ((eval_fmla sigma pi (wp x1 q)) -> ((~ (x1 = Sskip)) -> (reductible
  sigma pi x1))))) -> forall (sigma:(map.Map.map mident value))
  (pi:(list (ident* value)%type)) (sigmat:(map.Map.map mident datatype))
  (pit:(list (ident* datatype)%type)) (q:fmla), (compatible_env sigma sigmat
  pi pit) -> ((type_stmt sigmat pit s) -> ((eval_fmla sigma pi (wp s q)) ->
  ((~ (s = Sskip)) -> (reductible sigma pi s)))))).
(* Why3 intros s x x1 x2 h1 h2 h3 sigma pi sigmat pit q h4 h5 h6 h7. *)
