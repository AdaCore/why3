...
Theorem WP_parameter_add : forall {a:Type} {a_WT:WhyType a}, forall (h:Z)
  ...
  (k:key) (v:a), (((0%Z < h)%Z /\ ((forall (i:Z), ((0%Z <= i)%Z /\
  (i < h)%Z) -> (good_hash (mk_array h h1) i)) /\ forall (k1:key) (v1:a),
  (good_data k1 v1 h2 (mk_array h h1)))) /\ (0%Z <= h)%Z) -> forall (rho:Z)
  (rho1:(map.Map.map Z (list (key* a)%type))), (((0%Z < rho)%Z /\
  ((forall (i:Z), ((0%Z <= i)%Z /\ (i < rho)%Z) -> (good_hash (mk_array rho
  rho1) i)) /\ forall (k1:key) (v1:a), (good_data k1 v1 h2 (mk_array rho
  rho1)))) /\ (0%Z <= rho)%Z) -> forall (rho2:(map.Map.map key (option a)))
  (rho3:(map.Map.map Z (list (key* a)%type))) (rho4:Z), ((((0%Z < rho)%Z /\
  ((forall (i:Z), ((0%Z <= i)%Z /\ (i < rho)%Z) -> (good_hash (mk_array rho
  rho3) i)) /\ forall (k1:key) (v1:a), (good_data k1 v1 rho2 (mk_array rho
  rho3)))) /\ (0%Z <= rho)%Z) /\ (((map.Map.get rho2
  k) = Init.Datatypes.None) /\ forall (k':key), (~ (k' = k)) ->
  ((map.Map.get rho2 k') = (map.Map.get h2 k')))) -> let i := (bucket k
  rho) in (((0%Z <= i)%Z /\ (i < rho)%Z) -> (((0%Z <= i)%Z /\ (i < rho)%Z) ->
  forall (o:(map.Map.map Z (list (key* a)%type))), ((0%Z <= rho)%Z /\
  (o = (map.Map.set rho3 i (Init.Datatypes.cons (k, v) (map.Map.get rho3
  i))))) -> forall (rho5:Z), (rho5 = (rho4 + 1%Z)%Z) ->
