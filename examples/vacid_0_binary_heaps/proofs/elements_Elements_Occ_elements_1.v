...
(* Why3 goal *)
Theorem Occ_elements : forall {a:Type} {a_WT:WhyType a}, forall (a1:(Z -> a))
  (i:Z) (j:Z) (n:Z), ((i <= j)%Z /\ (j < n)%Z) -> (0%Z < (nb_occ (a1 j)
  (elements a1 i n)))%Z.
(* Why3 intros a a_WT a1 i j n (h1,h2). *)
Proof.
intros X _X a i j n H.
rewrite (Elements_union _ _ j); auto with zarith.
rewrite occ_union.
rewrite (Elements_add1 _ j n); auto with zarith.
rewrite occ_add_eq with (y := a j); auto.
generalize (occ_non_negative (elements a i j) (a j)).
generalize (occ_non_negative (elements a (j+1)n) (a j)).
omega.
Qed.
