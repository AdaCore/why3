...
Axiom bag_simpl_right : forall {a:Type} {a_WT:WhyType a}, forall (a1:(bag a))
...
Axiom occ_add_eq : forall {a:Type} {a_WT:WhyType a}, forall (b:(bag a)) (x:a)
  (y:a), (x = y) -> ((nb_occ y (add x b)) = ((nb_occ y b) + 1%Z)%Z).
...
Axiom Card_nonneg : forall {a:Type} {a_WT:WhyType a}, forall (x:(bag a)),
  (0%Z <= (card x))%Z.
...
Axiom Diff_occ : forall {a:Type} {a_WT:WhyType a}, forall (b1:(bag a))
  (b2:(bag a)) (x:a), ((nb_occ x (diff b1
  b2)) = (ZArith.BinInt.Z.max 0%Z ((nb_occ x b1) - (nb_occ x b2))%Z)).
...
Axiom choose_mem : forall {a:Type} {a_WT:WhyType a}, forall (b:(bag a)),
  (~ ((empty_bag : (bag a)) = b)) -> (mem (choose b) b).

(* Why3 assumption *)
Definition array (a:Type) := (Z -> a).

Parameter elements: forall {a:Type} {a_WT:WhyType a}, (Z -> a) -> Z -> Z ->
  (bag a).

Axiom Elements_empty : forall {a:Type} {a_WT:WhyType a}, forall (a1:(Z -> a))
  (i:Z) (j:Z), (j <= i)%Z -> ((elements a1 i j) = (empty_bag : (bag a))).

Axiom Elements_add : forall {a:Type} {a_WT:WhyType a}, forall (a1:(Z -> a))
  (i:Z) (j:Z), (i < j)%Z -> ((elements a1 i j) = (add (a1 (j - 1%Z)%Z)
  (elements a1 i (j - 1%Z)%Z))).

Axiom Elements_singleton : forall {a:Type} {a_WT:WhyType a}, forall (a1:(Z ->
  a)) (i:Z) (j:Z), (j = (i + 1%Z)%Z) -> ((elements a1 i j) = (singleton (a1
  i))).

Axiom Elements_union : forall {a:Type} {a_WT:WhyType a}, forall (a1:(Z -> a))
  (i:Z) (j:Z) (k:Z), ((i <= j)%Z /\ (j <= k)%Z) -> ((elements a1 i
  k) = (union (elements a1 i j) (elements a1 j k))).

Axiom Elements_add1 : forall {a:Type} {a_WT:WhyType a}, forall (a1:(Z -> a))
  (i:Z) (j:Z), (i < j)%Z -> ((elements a1 i j) = (add (a1 i) (elements a1
  (i + 1%Z)%Z j))).

Axiom Elements_remove_last : forall {a:Type} {a_WT:WhyType a},
  forall (a1:(Z -> a)) (i:Z) (j:Z), (i < (j - 1%Z)%Z)%Z -> ((elements a1 i
  (j - 1%Z)%Z) = (diff (elements a1 i j) (singleton (a1 (j - 1%Z)%Z)))).

Axiom Occ_elements : forall {a:Type} {a_WT:WhyType a}, forall (a1:(Z -> a))
  (i:Z) (j:Z) (n:Z), ((i <= j)%Z /\ (j < n)%Z) -> (0%Z < (nb_occ (a1 j)
  (elements a1 i n)))%Z.

Axiom elements_set_outside : forall {a:Type} {a_WT:WhyType a},
  forall (a1:(Z -> a)) (i:Z) (j:Z), (i <= j)%Z -> forall (k:Z), ((k < i)%Z \/
  (j <= k)%Z) -> forall (e:a), ((elements (map.Map.set a1 k e) i
  j) = (elements a1 i j)).

(* Why3 goal *)
Theorem Elements_set_inside : forall {a:Type} {a_WT:WhyType a},
  forall (a1:(Z -> a)) (i:Z) (j:Z) (n:Z) (e:a) (b:(bag a)), ((i <= j)%Z /\
  (j < n)%Z) -> (((elements a1 i n) = (add (a1 j) b)) ->
  ((elements (map.Map.set a1 j e) i n) = (add e b))).
(* Why3 intros a a_WT a1 i j n e b (h1,h2) h3. *)
Proof.
intros X _X a i j n e b Hi H.
...
rewrite (proj1 (Map.set_def _ _ _ _)); auto with zarith.
