...
(* Why3 goal *)
Theorem Model_set : forall (a:(map.Map.map Z Z)) (v:Z) (i:Z) (n:Z),
  ((0%Z <= i)%Z /\ (i < n)%Z) -> ((add (map.Map.get a i) (model (
  (map.Map.set a i v), n))) = (add v (model (a, n)))).
Proof.
intros a v i n H_is.
unfold model in *.
rewrite (Elements_union _ 0 i n); auto with *.
pattern (elements (Map.set a i v) 0 i); rewrite Elements_set_outside; auto with *.
rewrite (Elements_add1 _ i n); auto with *.
rewrite Map.Select_eq; auto with *.
pattern (elements (Map.set a i v) (i + 1) n); rewrite Elements_set_outside; auto with *.
unfold add.
...
pattern (union (elements a 0 i)
  ...
  rewrite Union_assoc.
pattern (union (singleton (Map.get a i))
...
pattern  (union (singleton (Map.get a i))
  ...
  rewrite Union_assoc.
pattern (union (union (singleton (Map.get a i)) (elements a 0 i))
     (elements a (i + 1) n)); 
     rewrite <- Union_assoc.
...
pattern (union (singleton (Map.get a i))
  ...
  rewrite Union_assoc.
fold (add (Map.get a i) (elements a (i + 1) n)).
pattern (add (Map.get a i) (elements a (i + 1) n)); 
