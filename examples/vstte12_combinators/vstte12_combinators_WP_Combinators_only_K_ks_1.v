...
Axiom ks_def : forall (n:Z), (0%Z <= n)%Z -> (((n = 0%Z) -> ((ks n) = K)) /\
  ((~ (n = 0%Z)) -> ((ks n) = (App (ks (n - 1%Z)%Z) K)))).
...
(* Why3 goal *)
Theorem only_K_ks : forall (n:Z), (0%Z <= n)%Z -> (only_K (ks n)).
(* Why3 intros n h1. *)
intros n hn; generalize hn; pattern n; apply natlike_ind; intuition.
rewrite ks0; auto.
replace (Zsucc x) with (x+1)%Z by omega.
rewrite ksS; intuition.
Qed.
