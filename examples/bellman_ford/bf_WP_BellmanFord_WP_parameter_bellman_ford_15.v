...
Theorem WP_parameter_bellman_ford : let o := ((cardinal vertices) - 1%Z)%Z in
  ((1%Z <= o)%Z -> forall (m:(@map.Map.map vertex vertex_WhyType
  t t_WhyType)), (inv1 m (o + 1%Z)%Z (empty :(@set (vertex*
  vertex)%type _))) -> ((inv1 m (cardinal vertices) (empty :(@set (vertex*
  vertex)%type _))) -> forall (es:(@set (vertex* vertex)%type _)),
  (es = edges) -> forall (es1:(@set (vertex* vertex)%type _)), ((subset es1
  edges) /\ (inv2 m (diff edges es1))) -> forall (o1:bool), ((o1 = true) <->
  (is_empty es1)) -> ((~ (o1 = true)) -> ((~ (is_empty es1)) ->
  forall (es2:(@set (vertex* vertex)%type _)), forall (result:vertex)
...
assert (hv: mem v vertices) by ae.
generalize (hinv1 v hv).
rewrite Heqt1; simpl.
intros (hv1, _).
assert (hu: exists lu: list vertex, path s lu u ) by ae.
destruct hu as (lu, hu1).
assert (path s (app lu (cons u nil)) v) by ae.
assert (exists lv': list vertex, path s lv' v /\ (Length.length lv' < cardinal vertices)%Z) by ae.
ae.
Qed.
