...
Theorem WP_parameter_bellman_ford : (((cardinal vertices) - 1%Z)%Z < 1%Z)%Z ->
  ((inv1 (map.Map.set (map.Map.const Infinite:(@map.Map.map
  vertex vertex_WhyType t t_WhyType)) s (Finite 0%Z)) (cardinal vertices)
  (empty :(@set (vertex* vertex)%type _))) -> forall (es:(@set (vertex*
  vertex)%type _)), (es = edges) -> forall (es1:(@set (vertex*
  vertex)%type _)), ((subset es1 edges) /\ (inv2
  (map.Map.set (map.Map.const Infinite:(@map.Map.map vertex vertex_WhyType
  t t_WhyType)) s (Finite 0%Z)) (diff edges es1))) -> forall (o:bool),
  ((o = true) <-> (is_empty es1)) -> ((~ (o = true)) -> ((~ (is_empty
  es1)) -> forall (es2:(@set (vertex* vertex)%type _)),
  forall (result:vertex) (result1:vertex), let result2 := (result,
  result1) in (((mem result2 es1) /\ (es2 = (remove result2 es1))) ->
  (match (map.Map.get (map.Map.set (map.Map.const Infinite:(@map.Map.map
  vertex vertex_WhyType t t_WhyType)) s (Finite 0%Z))
  result) with
  | Infinite => False
  | (Finite x) =>
      match (map.Map.get (map.Map.set (map.Map.const Infinite:(@map.Map.map
...
intros u v res4. unfold res4.
intros (Huv, _).
assert (u = s) by ae. subst u.
assert (v = s) by ae. subst v.
