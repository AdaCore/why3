...
Theorem WP_parameter_bellman_ford : (((cardinal vertices) - 1%Z)%Z < 1%Z)%Z ->
  ((inv1 (map.Map.set (map.Const.const Infinite: (map.Map.map vertex t)) s
  (Finite 0%Z)) (cardinal vertices) (empty : (set (vertex* vertex)%type))) ->
  forall (es:(set (vertex* vertex)%type)), (es = edges) -> forall (es1:(set
  (vertex* vertex)%type)), ((subset es1 edges) /\ (inv2
  (map.Map.set (map.Const.const Infinite: (map.Map.map vertex t)) s
  (Finite 0%Z)) (diff edges es1))) -> forall (o:bool), ((o = true) <->
  (is_empty es1)) -> ((~ (o = true)) -> ((~ (is_empty es1)) ->
  forall (es2:(set (vertex* vertex)%type)), forall (result:vertex)
  (result1:vertex), let result2 := (result, result1) in (((mem result2
  es1) /\ (es2 = (remove result2 es1))) ->
  (match (map.Map.get (map.Map.set (map.Const.const Infinite: (map.Map.map
