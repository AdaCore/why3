...
Axiom min_elt_spec : forall (s:(set Z)), (~ (is_empty s)) -> ((mem
  (min_elt s) s) /\ forall (x:Z), (mem x s) -> ((min_elt s) <= x)%Z).

Parameter max_elt: (set Z) -> Z.

Axiom max_elt_spec : forall (s:(set Z)), (~ (is_empty s)) -> ((mem
  (max_elt s) s) /\ forall (x:Z), (mem x s) -> (x <= (max_elt s))%Z).

Parameter interval: Z -> Z -> (set Z).

Axiom interval_spec : forall (l:Z) (r:Z), forall (x:Z), (mem x (interval l
  r)) <-> ((l <= x)%Z /\ (x < r)%Z).

Axiom cardinal_interval : forall (l:Z) (r:Z), ((l <= r)%Z ->
  ((cardinal (interval l r)) = (r - l)%Z)) /\ ((~ (l <= r)%Z) ->
  ((cardinal (interval l r)) = 0%Z)).

Parameter succ: (set Z) -> (set Z).

Axiom succ_spec : forall (s:(set Z)), forall (i:Z), (mem i (succ s)) <->
  ((1%Z <= i)%Z /\ (mem (i - 1%Z)%Z s)).

Parameter pred: (set Z) -> (set Z).

Axiom pred_spec : forall (s:(set Z)), forall (i:Z), (mem i (pred s)) <->
...
Definition eq_prefix {a:Type} {a_WT:WhyType a} (t:(Z -> a)) (u:(Z -> a))
  (i:Z): Prop := forall (k:Z), ((0%Z <= k)%Z /\ (k < i)%Z) -> ((t k) = (u
  k)).

(* Why3 assumption *)
Definition partial_solution (k:Z) (s:(Z -> Z)): Prop := forall (i:Z),
  ((0%Z <= i)%Z /\ (i < k)%Z) -> (((0%Z <= (s i))%Z /\ ((s i) < n)%Z) /\
  forall (j:Z), ((0%Z <= j)%Z /\ (j < i)%Z) -> ((~ ((s i) = (s j))) /\
  ((~ (((s i) - (s j))%Z = (i - j)%Z)) /\ ~ (((s i) - (s
  j))%Z = (j - i)%Z)))).

Axiom partial_solution_eq_prefix : forall (u:(Z -> Z)) (t:(Z -> Z)) (k:Z),
  (partial_solution k t) -> ((eq_prefix t u k) -> (partial_solution k u)).

(* Why3 assumption *)
Definition lt_sol (s1:(Z -> Z)) (s2:(Z -> Z)): Prop := exists i:Z,
  ((0%Z <= i)%Z /\ (i < n)%Z) /\ ((eq_prefix s1 s2 i) /\ ((s1 i) < (s2
  i))%Z).
...
(* Why3 assumption *)
Definition sorted (s:(Z -> (Z -> Z))) (a:Z) (b:Z): Prop := forall (i:Z)
  (j:Z), ((a <= i)%Z /\ ((i < j)%Z /\ (j < b)%Z)) -> (lt_sol (s i) (s j)).

Axiom no_duplicate : forall (s:(Z -> (Z -> Z))) (a:Z) (b:Z), (sorted s a
  b) -> forall (i:Z) (j:Z), ((a <= i)%Z /\ ((i < j)%Z /\ (j < b)%Z)) ->
  ~ (eq_prefix (s i) (s j) n).

(* Why3 goal *)
Theorem VC_t3 : forall (col:(Z -> Z)) (k:Z) (sol:(Z -> (Z -> Z))) (s:Z),
  forall (a:(set Z)) (b:(set Z)) (c:(set Z)), ((0%Z <= k)%Z /\
  (((k + (cardinal a))%Z = n) /\ ((0%Z <= s)%Z /\ ((forall (i:Z), (mem i
  a) <-> (((0%Z <= i)%Z /\ (i < n)%Z) /\ forall (j:Z), ((0%Z <= j)%Z /\
  (j < k)%Z) -> ~ ((col j) = i))) /\ ((forall (i:Z), (0%Z <= i)%Z -> ((~ (mem
  i b)) <-> forall (j:Z), ((0%Z <= j)%Z /\ (j < k)%Z) -> ~ ((col
  j) = ((i + j)%Z - k)%Z))) /\ ((forall (i:Z), (0%Z <= i)%Z -> ((~ (mem i
  c)) <-> forall (j:Z), ((0%Z <= j)%Z /\ (j < k)%Z) -> ~ ((col
  j) = ((i + k)%Z - j)%Z))) /\ (partial_solution k col))))))) -> (((is_empty
  a) <-> forall (x:Z), ~ (mem x a)) -> ((~ (is_empty a)) -> let o := (diff a
  b) in ((forall (x:Z), (mem x o) <-> ((mem x a) /\ ~ (mem x b))) ->
  let o1 := (diff o c) in ((forall (x:Z), (mem x o1) <-> ((mem x o) /\ ~ (mem
  x c))) -> forall (f:Z) (e:(set Z)) (s1:Z) (sol1:(Z -> (Z -> Z))) (k1:Z)
  (col1:(Z -> Z)), (((f = (s1 - s)%Z) /\ (0%Z <= (s1 - s)%Z)%Z) /\
  ((k1 = k) /\ ((subset e (diff (diff a b) c)) /\ ((partial_solution k1
  col1) /\ ((sorted sol1 s s1) /\ ((forall (i:Z) (j:Z), (mem i (diff o1
  e)) -> ((mem j e) -> (i < j)%Z)) /\ ((forall (t:(Z -> Z)),
  ((partial_solution n t) /\ ((eq_prefix col1 t k1) /\ (mem (t k1) (diff o1
  e)))) <-> exists i:Z, ((s <= i)%Z /\ (i < s1)%Z) /\ (eq_prefix t (sol1 i)
  n)) /\ ((eq_prefix col col1 k1) /\ (eq_prefix sol sol1 s))))))))) ->
  (((is_empty e) <-> forall (x:Z), ~ (mem x e)) -> ((~ (is_empty e)) ->
  let d := (min_elt e) in (((mem d e) /\ forall (x:Z), (mem x e) ->
  (d <= x)%Z) -> forall (col2:(Z -> Z)), (col2 = (map.Map.set col1 k1 d)) ->
  forall (k2:Z), (k2 = (k1 + 1%Z)%Z) -> let o2 := (add d c) in
  ((forall (y:Z), (mem y o2) <-> ((y = d) \/ (mem y c))) -> ((forall (i:Z),
  (mem i (pred o2)) <-> ((0%Z <= i)%Z /\ (mem (i + 1%Z)%Z o2))) -> let o3 :=
  (add d b) in ((forall (y:Z), (mem y o3) <-> ((y = d) \/ (mem y b))) ->
  ((forall (i:Z), (mem i (succ o3)) <-> ((1%Z <= i)%Z /\ (mem (i - 1%Z)%Z
  o3))) -> ((forall (y:Z), (mem y (remove d a)) <-> ((~ (y = d)) /\ (mem y
  a))) -> forall (s2:Z) (sol2:(Z -> (Z -> Z))) (k3:Z) (col3:(Z -> Z)),
  ((0%Z <= (s2 - s1)%Z)%Z /\ ((k3 = k2) /\ ((sorted sol2 s1 s2) /\
  ((forall (t:(Z -> Z)), ((partial_solution n t) /\ (eq_prefix col3 t
  k3)) <-> exists i:Z, ((s1 <= i)%Z /\ (i < s2)%Z) /\ (eq_prefix t (sol2 i)
  n)) /\ ((eq_prefix col2 col3 k3) /\ (eq_prefix sol1 sol2 s1)))))) ->
  forall (f1:Z), (f1 = (f + (s2 - s1)%Z)%Z) -> forall (k4:Z),
  (k4 = (k3 - 1%Z)%Z) -> let o4 := (remove d e) in ((forall (y:Z), (mem y
  o4) <-> ((~ (y = d)) /\ (mem y e))) -> forall (e1:(set Z)), (e1 = o4) ->
  (sorted sol2 s s2)))))))))))))).
intros col k sol s a b c (h1,(h2,(h3,(h4,(h5,(h6,h7)))))) h8 h9 o h10 o1 h11
f e s1 sol1 k1 col1 ((h12,h13),(h14,(h15,(h16,(h17,(h18,(h19,(h20,h21))))))))
h22 h23 d (h24,h25) col2 h26 k2 h27 o2 h28 h29 o3 h30 h31 h32 s2 sol2 k3 col3
(h33,(h34,(h35,(h36,(h37,h38))))) f1 h39 k4 h40 o4 h41 e1 h42.

Qed.

(* Unused content named WP_parameter_t3
