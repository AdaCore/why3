...
Axiom min_elt_spec :
  forall (s:set Z), ~ (is_empty s) ->
  (mem (min_elt s) s) /\ forall (x:Z), (mem x s) -> ((min_elt s) <= x)%Z.

Parameter max_elt: (set Z) -> Z.

Axiom max_elt_spec :
  forall (s:set Z), ~ (is_empty s) ->
  (mem (max_elt s) s) /\ forall (x:Z), (mem x s) -> (x <= (max_elt s))%Z.

Parameter interval: Z -> Z -> set Z.

Axiom interval_spec :
  forall (l:Z) (r:Z), forall (x:Z),
...
Axiom succ_spec :
  forall (s:set Z), forall (i:Z),
  (mem i (succ s)) <-> ((1%Z <= i)%Z /\ (mem (i - 1%Z)%Z s)).

Parameter pred: (set Z) -> set Z.

Axiom pred_spec :
  forall (s:set Z), forall (i:Z),
  (mem i (pred s)) <-> ((0%Z <= i)%Z /\ (mem (i + 1%Z)%Z s)).
