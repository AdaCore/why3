...
Theorem WP_parameter_reduction3 : forall (t:term), (exists n:Z,
  ...
  ((result = K) -> ((exists n:Z, (0%Z <= n)%Z /\ (x1 = (ks n))) ->
  forall (o:term), ((is_value o) /\ forall (n:Z), (0%Z <= n)%Z ->
  (((x1 = (ks (2%Z * n)%Z)) -> (o = K)) /\
  ((x1 = (ks ((2%Z * n)%Z + 1%Z)%Z)) -> (o = (App K K))))) -> forall (n:Z),
  (0%Z <= n)%Z -> ~ (t = (ks (2%Z * n)%Z))))).
(* Why3 intros t (n,(h1,h2)) x x1 h3 (n1,(h4,h5)) result (h6,h7) h8
        (n2,(h9,h10)) o (h11,h12) n3 h13. *)
intros t (n,(h1,h2)) x x1 h3 (n1,(h4,h5)) result (h6,h7) h8
        (n2,(h9,h10)) o (h11,h12) n0 h13.
subst.
intuition.
generalize (h12 n2 h9); clear h12; intros ht2.
assert (n2 = 0)%Z.
  generalize (ks_inversion n2 h9).
  intuition.
...
ae.
subst.
assert (ks n1 = ks (2*(n0-1)+1))%Z by ae.
