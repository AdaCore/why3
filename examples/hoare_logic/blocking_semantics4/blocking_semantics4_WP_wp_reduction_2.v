...
Theorem wp_reduction : forall (sigma:(map mident value)) (sigma':(map mident
  ...
  match s with
  | (Evalue v) => True
  | (Ebin e o e1) => (forall (s':expr), (one_step sigma pi e1 sigma' pi'
      s') -> forall (q:fmla), (eval_fmla sigma pi (wp e1 q)) ->
      (eval_fmla sigma' pi' (wp s' q))) -> ((forall (s':expr),
      (one_step sigma pi e sigma' pi' s') -> forall (q:fmla),
      (eval_fmla sigma pi (wp e q)) -> (eval_fmla sigma' pi' (wp s' q))) ->
      forall (s':expr), (one_step sigma pi s sigma' pi' s') ->
