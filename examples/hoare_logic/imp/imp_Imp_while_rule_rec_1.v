...
Theorem while_rule_rec : forall (e:expr) (inv:fmla) (i:stmt),
  (valid_triple (Fand (Fterm e) inv) i inv) -> forall (s:(map ident Z))
  (sqt:(map ident Z)) (l:stmt) (skip:stmt), (many_steps s l sqt skip) ->
  ((l = (Swhile e i)) -> ((skip = Sskip) -> ((eval_fmla s inv) ->
  (eval_fmla sqt (Fand (Fnot (Fterm e)) inv))))).
