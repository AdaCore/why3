...
Theorem monotonicity : forall (s:expr),
  ...
  | (Eseq e e1) => True
  | (Elet i e e1) => True
  | (Eif e e1 e2) => (forall (p:fmla) (q:fmla), (valid_fmla (Fimplies p
      q)) -> (valid_fmla (Fimplies (wp e2 p) (wp e2 q)))) ->
      ((forall (p:fmla) (q:fmla), (valid_fmla (Fimplies p q)) ->
      (valid_fmla (Fimplies (wp e1 p) (wp e1 q)))) -> ((forall (p:fmla)
      (q:fmla), (valid_fmla (Fimplies p q)) -> (valid_fmla (Fimplies (wp e p)
      (wp e q)))) -> forall (p:fmla) (q:fmla), (valid_fmla (Fimplies p q)) ->
...
simpl.
intros.
apply H1 with (p := (Fand (Fimplies (Fterm (Tvar result)) (wp s2 p))
             (Fimplies (Fnot (Fterm (Tvar result))) (wp s3 p)))); auto.
intros.
simpl in *.
destruct H4.
split; intro.
apply H0 with (p := p); auto.
