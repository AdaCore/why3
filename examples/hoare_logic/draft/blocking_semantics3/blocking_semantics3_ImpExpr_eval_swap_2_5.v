...
(* Why3 goal *)
Theorem eval_swap_2 : forall (f:fmla),
  match f with
  | (Fterm t) => True
  | (Fand f1 f2) => True
  | (Fnot f1) => True
  | (Fimplies f1 f2) => True
  | (Flet i t f1) => True
  | (Fforall i d f1) => (forall (id1:ident) (id2:ident) (v1:value)
      (v2:value), (~ (id1 = id2)) -> forall (sigma:(map mident value))
      (pi:(list (ident* value)%type)), (eval_fmla sigma (Cons (id1, v1)
      (Cons (id2, v2) pi)) f1) <-> (eval_fmla sigma (Cons (id2, v2) (Cons (
      id1, v1) pi)) f1)) -> forall (id1:ident) (id2:ident) (v1:value)
      (v2:value), (~ (id1 = id2)) -> forall (sigma:(map mident value))
      (pi:(list (ident* value)%type)), (eval_fmla sigma (Cons (id2, v2)
      (Cons (id1, v1) pi)) f) -> (eval_fmla sigma (Cons (id1, v1) (Cons (id2,
      v2) pi)) f)
  end.
destruct f; auto.
simpl.
...
(* TYunit *)
assert (h: (infix_plpl 
  (Cons (i, Vvoid) (Nil : list (ident*value)))
  (Cons (id1, v1) (Cons (id2, v2) pi))) = 
  (Cons (i, Vvoid) (Cons (id1, v1) (Cons (id2, v2) pi)))); auto.
ae.
(* TYint *)
intro n.
assert (h: (infix_plpl 
  (Cons (i, Vint n) (Nil : list (ident*value)))
  (Cons (id1, v1) (Cons (id2, v2) pi))) = 
  (Cons (i, Vint n) (Cons (id1, v1) (Cons (id2, v2) pi)))); auto.
ae.
(* TYbool *)
intro b. 
assert (h: (infix_plpl 
  (Cons (i, Vbool b) (Nil : list (ident*value)))
  (Cons (id1, v1) (Cons (id2, v2) pi))) = 
  (Cons (i, Vbool b) (Cons (id1, v1) (Cons (id2, v2) pi)))); auto.
ae.
Qed.
