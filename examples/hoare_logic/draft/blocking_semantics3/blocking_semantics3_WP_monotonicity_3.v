...
Theorem monotonicity : forall (s:stmt),
  ...
  | (Sassign m t) => True
  | (Sseq s1 s2) => True
  | (Sif t s1 s2) => (forall (p:fmla) (q:fmla), (valid_fmla (Fimplies p
      q)) -> (valid_fmla (Fimplies (wp s2 p) (wp s2 q)))) ->
      ((forall (p:fmla) (q:fmla), (valid_fmla (Fimplies p q)) ->
      (valid_fmla (Fimplies (wp s1 p) (wp s1 q)))) -> forall (p:fmla)
...
intros H1 H2 p q.
intros H sigma pi (H3 & H4).
split.
(*True*)
intro H5.
apply H2 with (p := p); auto.
(*False*)
intro H5.
apply H1 with (p := p); auto.
Qed.
