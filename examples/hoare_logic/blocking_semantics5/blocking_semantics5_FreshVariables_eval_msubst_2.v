...
Theorem eval_msubst : forall (f:fmla), forall (x:ident) (x1:datatype)
  ...
  value)) (pi:(list (ident* value)%type)) (x3:mident) (v:ident),
  (fresh_in_fmla v x2) -> ((eval_fmla sigma pi (msubst x2 x3 v)) <->
  (eval_fmla (map.Map.set sigma x3 (get_stack v pi)) pi x2))) ->
  forall (sigma:(map.Map.map mident value)) (pi:(list (ident* value)%type))
  (x3:mident) (v:ident), (fresh_in_fmla v f) -> ((eval_fmla sigma pi
  (msubst f x3 v)) -> (eval_fmla (map.Map.set sigma x3 (get_stack v pi)) pi
  f))).
