...
(* Why3 goal *)
Theorem wp_preserved_by_reduction : forall (sigma:(map mident value))
  (sigma':(map mident value)) (pi:(list (ident* value)%type)) (pi':(list
  (ident* value)%type)) (s:stmt),
  match s with
  | Sskip => True
  | (Sassign m t) => True
  | (Sseq s1 s2) => (forall (s':stmt), (one_step sigma pi s2 sigma' pi'
      s') -> forall (q:fmla), (eval_fmla sigma pi (wp s2 q)) ->
      (eval_fmla sigma' pi' (wp s' q))) -> ((forall (s':stmt),
      (one_step sigma pi s1 sigma' pi' s') -> forall (q:fmla),
      (eval_fmla sigma pi (wp s1 q)) -> (eval_fmla sigma' pi' (wp s' q))) ->
      forall (s':stmt), (one_step sigma pi s sigma' pi' s') ->
