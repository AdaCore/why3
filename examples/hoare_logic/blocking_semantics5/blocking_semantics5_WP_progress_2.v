...
Theorem progress : forall (s:stmt),
  ...
  | (Sseq s1 s2) => (forall (sigma:(map mident value)) (pi:(list (ident*
      ...
      ((~ (s2 = Sskip)) -> (reducible sigma pi s2))))) ->
      ((forall (sigma:(map mident value)) (pi:(list (ident* value)%type))
      ...
      (reducible sigma pi s1))))) -> forall (sigma:(map mident value))
      ...
      (wp s q)) -> ((~ (s = Sskip)) -> (reducible sigma pi s)))))
  | (Sif t s1 s2) => True
  | (Sassert f) => True
  | (Swhile t f s1) => True
  end.
...
(* case s1 <> Sskip *)
eelim H0 with (q:= (wp s2 q)); auto.
intros sigma2 (pi2 & s3 & h3).
exists sigma2.
exists pi2.
exists (Sseq s3 s2).
apply one_step_seq_noskip.
apply h3.
apply H1.
