...
Axiom wp_preserved_by_reduction : forall (sigma:(map mident value))
  (sigma':(map mident value)) (pi:(list (ident* value)%type)) (pi':(list
  (ident* value)%type)) (s:stmt) (s':stmt), (one_step sigma pi s sigma' pi'
  s') -> forall (q:fmla), (eval_fmla sigma pi (wp s q)) -> (eval_fmla sigma'
  pi' (wp s' q)).
...
Theorem progress : forall (s:stmt),
  ...
  | (Sseq s1 s2) => (forall (sigma:(map mident value)) (pi:(list (ident*
      ...
      ((~ (s2 = Sskip)) -> (reductible sigma pi s2))))) ->
      ((forall (sigma:(map mident value)) (pi:(list (ident* value)%type))
      (sigmat:(map mident datatype)) (pit:(list (ident* datatype)%type))
      (q:fmla), (compatible_env sigma sigmat pi pit) -> ((type_stmt sigmat
      pit s1) -> ((eval_fmla sigma pi (wp s1 q)) -> ((~ (s1 = Sskip)) ->
      (reductible sigma pi s1))))) -> forall (sigma:(map mident value))
      (pi:(list (ident* value)%type)) (sigmat:(map mident datatype))
      (pit:(list (ident* datatype)%type)) (q:fmla), (compatible_env sigma
      sigmat pi pit) -> ((type_stmt sigmat pit s) -> ((eval_fmla sigma pi
      (wp s q)) -> ((~ (s = Sskip)) -> (reductible sigma pi s)))))
  | (Sif t s1 s2) => True
  | (Sassert f) => True
  | (Swhile t f s1) => True
  end.
...
destruct H0 with (1:=H1) (q:= (wp s2 q)) as (sigma2 & pi2 & s3 & h3); auto.
