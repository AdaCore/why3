...
(* Why3 goal *)
Theorem WP_parameter_compute_writes : forall (s:stmt), forall (x:fmla),
  (s = (Sassert x)) -> forall (sigma:(map.Map.map Z value)) (pi:(map.Map.map
...
(* Why3 intros s x h1 sigma pi sigma' pi' n h2. *)
intros s x H_;rewrite H_ in *. intros.
