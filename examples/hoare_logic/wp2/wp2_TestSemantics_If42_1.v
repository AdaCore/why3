...
(* Why3 goal *)
Theorem If42 : forall (sigma1:(map.Map.map Z value)) (pi1:(map.Map.map Z
  value)) (sigma2:(map.Map.map Z value)) (pi2:(map.Map.map Z value))
  (i:stmt), (one_step (map.Const.const (Vint 0%Z): (map.Map.map Z value))
  (map.Const.const (Vint 42%Z): (map.Map.map Z value))
  (Sif (Tbin (Tderef 0%Z) Ole (Tconst 10%Z)) (Sassign 0%Z (Tconst 13%Z))
  (Sassign 0%Z (Tconst 42%Z))) sigma1 pi1 i) -> ((one_step sigma1 pi1 i
  sigma2 pi2 Sskip) -> ((map.Map.get sigma2 0%Z) = (Vint 13%Z))).
(* Why3 intros sigma1 pi1 sigma2 pi2 i h1 h2. *)
