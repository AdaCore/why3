...
Theorem WP_parameter_compute_writes : forall (s:stmt), forall (x:term)
  (x1:fmla) (x2:stmt), (s = (Swhile x x1 x2)) -> forall (result:(set.Set.set
  Z)), (forall (sigma:(map.Map.map Z value)) (pi:(map.Map.map Z value))
  (sigma':(map.Map.map Z value)) (pi':(map.Map.map Z value)) (n:Z),
  (many_steps sigma pi x2 sigma' pi' Sskip n) -> (assigns sigma result
  sigma')) -> forall (sigma:(map.Map.map Z value)) (pi:(map.Map.map Z value))
  (sigma':(map.Map.map Z value)) (pi':(map.Map.map Z value)) (n:Z),
  (many_steps sigma pi s sigma' pi' Sskip n) -> (assigns sigma result
  sigma').
(* Why3 intros s x x1 x2 h1 result h2 sigma pi sigma' pi' n h3. *)
...
generalize (many_steps_seq _ _ _ _ _ _ _ H1).
