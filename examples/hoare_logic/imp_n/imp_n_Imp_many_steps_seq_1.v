...
Theorem many_steps_seq : forall (s1:(map ident Z)) (s3:(map ident Z))
  (i1:stmt) (i2:stmt) (n:Z), (many_steps s1 (Sseq i1 i2) s3 Sskip n) ->
  exists s2:(map ident Z), exists n1:Z, exists n2:Z, (many_steps s1 i1 s2
  Sskip n1) /\ ((many_steps s2 i2 s3 Sskip n2) /\
  (n = ((1%Z + n1)%Z + n2)%Z)).
...
apply Z_lt_induction
 with (P := fun n =>
  forall (s1 : map ident Z) (i1 : stmt),
  many_steps s1 (Sseq i1 i2) s3 Sskip n ->
exists s2 : map ident Z,
  exists n1 : Z,
    exists n2 : Z,
