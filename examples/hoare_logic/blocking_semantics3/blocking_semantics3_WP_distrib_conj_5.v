...
destruct s; auto.
simpl.
intros sigma pi p q (H & H1).
pose (id1 := fresh_from (Fand p q) (Sassign m t));
   fold id1 in H.
pose (id2 := fresh_from (Fand p q) (Sassign m t));
   fold id2.
assert (h:fresh_in_fmla (fresh_from (Fand p q) (Sassign m t)) (Fand p q)).
...
rewrite get_stack_eq.
rewrite eval_change_free.
rewrite eval_msubst in H1.
rewrite get_stack_eq in H1.
rewrite eval_change_free in H1; auto.
apply fresh_from_fmla; auto.
apply fresh_from_fmla; auto.
simpl in h; destruct h; auto.
simpl in h; destruct h; auto.
