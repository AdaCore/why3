...
(* Why3 goal *)
Theorem eval_bool_term : forall (t:term),
  match t with
  | (Tvalue v) => forall (sigma:(map mident value)) (pi:(list (ident*
      value)%type)) (sigmat:(map mident datatype)) (pit:(list (ident*
      datatype)%type)), (type_term sigmat pit t TYbool) ->
      ((forall (id:mident), (compatible (get sigmat id) (get sigma id))) ->
      ((forall (id:ident), (compatible (get_vartype id pit) (get_stack id
      pi))) -> exists b:bool, ((eval_term sigma pi t) = (Vbool b))))
  | (Tvar i) => True
  | (Tderef m) => True
  | (Tbin t1 o t2) => True
  end.
