...
(* Why3 goal *)
Theorem distrib_conj : forall (sigma:(map mident value)) (pi:(list (ident*
  value)%type)), forall (s:stmt),
  match s with
  | Sskip => True
  | (Sassign m t) => True
  | (Sseq s1 s2) => (forall (p:fmla) (q:fmla), ((eval_fmla sigma pi (wp s2
      p)) /\ (eval_fmla sigma pi (wp s2 q))) -> (eval_fmla sigma pi (wp s2
      (Fand p q)))) -> ((forall (p:fmla) (q:fmla), ((eval_fmla sigma pi
      (wp s1 p)) /\ (eval_fmla sigma pi (wp s1 q))) -> (eval_fmla sigma pi
      (wp s1 (Fand p q)))) -> forall (p:fmla) (q:fmla), ((eval_fmla sigma pi
      (wp s p)) /\ (eval_fmla sigma pi (wp s q))) -> (eval_fmla sigma pi
      (wp s (Fand p q))))
  | (Sif t s1 s2) => True
  | (Sassert f) => True
  | (Swhile t f s1) => True
