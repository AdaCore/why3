...
Theorem WP_parameter_quick_rec : forall (t:Z) (l:Z) (r:Z),
  ...
  t3 l m)) -> let o := (m - 1%Z)%Z in (((0%Z <= l)%Z /\ (o < t)%Z) ->
  forall (t4:(map.Map.map Z Z)), ((0%Z <= t)%Z /\ ((sorted_sub t4 l
  (o + 1%Z)%Z) /\ (map.MapPermut.permut_sub t3 t4 l (o + 1%Z)%Z))) ->
  let o1 := (m + 1%Z)%Z in (((0%Z <= o1)%Z /\ (r < t)%Z) ->
  forall (t5:(map.Map.map Z Z)), ((0%Z <= t)%Z /\ ((sorted_sub t5 o1
  (r + 1%Z)%Z) /\ (map.MapPermut.permut_sub t4 t5 o1 (r + 1%Z)%Z))) ->
  ((sorted_sub t5 l (r + 1%Z)%Z) /\ (map.MapPermut.permut_sub t1 t5 l
  (r + 1%Z)%Z)))))))).
intros n l r t1.
