...
Theorem WP_parameter_quick_rec : forall (t:Z) (l:Z) (r:Z),
  ...
  (j < (r + 1%Z)%Z)%Z) -> (v <= (map.Map.get t2 j))%Z) /\
  ((map.MapPermut.permut_sub t1 t2 l (r + 1%Z)%Z) /\ (((map.Map.get t2
  l) = v) /\ ((l <= m)%Z /\ (m < (r + 1%Z)%Z)%Z))))) -> (((0%Z <= t)%Z /\
  (((0%Z <= l)%Z /\ (l < t)%Z) /\ ((0%Z <= m)%Z /\ (m < t)%Z))) ->
  forall (t3:(map.Map.map Z Z)), ((0%Z <= t)%Z /\ (map.MapPermut.exchange t2
  t3 l m)) -> (((0%Z <= l)%Z /\ ((m - 1%Z)%Z < t)%Z) ->
  forall (t4:(map.Map.map Z Z)), ((0%Z <= t)%Z /\ ((sorted_sub t4 l
  ((m - 1%Z)%Z + 1%Z)%Z) /\ (map.MapPermut.permut_sub t3 t4 l
  ((m - 1%Z)%Z + 1%Z)%Z))) -> (((0%Z <= (m + 1%Z)%Z)%Z /\ (r < t)%Z) ->
  forall (t5:(map.Map.map Z Z)), ((0%Z <= t)%Z /\ ((sorted_sub t5 (m + 1%Z)%Z
  (r + 1%Z)%Z) /\ (map.MapPermut.permut_sub t4 t5 (m + 1%Z)%Z
  (r + 1%Z)%Z))) -> ((sorted_sub t5 l (r + 1%Z)%Z) /\
  (map.MapPermut.permut_sub t1 t5 l (r + 1%Z)%Z)))))))).
(* intros t l r t1 (h1,(h2,h3)) h4 (h5,h6) v h7 m t2
   (h8,(h9,(h10,(h11,(h12,h13))))) (h14,((h15,h16),(h17,h18))) t3 (h19,h20)
   (h21,h22) t4 (h23,(h24,h25)) (h26,h27) t5 (h28,(h29,h30)). *)
