...
Axiom big_float_is_int : forall (m:mode) (i:t), (t'isFinite i) -> (((le i
  (neg (of_int m 9007199254740992%Z))) \/ (le (of_int m 9007199254740992%Z)
  i)) -> (is_int i)).
...
(* Why3 goal *)
Theorem WP_parameter_my_exp : forall (x:t), ((t'isFinite x) /\ (((le
  (neg fliteral) x) /\ (le x fliteral)) /\
  ((Reals.Rbasic_fun.Rabs (t'real x)) <= 1%R)%R)) -> let x1 := (t'real x) in
  ((Reals.Rbasic_fun.Rabs ((((09890365552 / 10000000000)%R + ((1130258690 / 1000000000)%R * x1)%R)%R + (((05540440796 / 10000000000)%R * x1)%R * x1)%R)%R - (Reals.Rtrigo_def.exp x1))%R) <= (65535 / 1048576)%R)%R.
(* Why3 intros x (h1,((h2,h3),h4)) x1. *)
intros x (h1,((h2,h3),h4)) x1.
subst x1.
