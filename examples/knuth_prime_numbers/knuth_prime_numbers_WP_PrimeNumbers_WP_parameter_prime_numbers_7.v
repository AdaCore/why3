...
Theorem WP_parameter_prime_numbers : forall (m:Z), (2%Z <= m)%Z ->
  ...
  ((2%Z <= o)%Z -> forall (n:Z) (p2:(map.Map.map Z Z)), forall (j:Z),
  ...
  (p3:(map.Map.map Z Z)), ((0%Z <= m)%Z /\ (((((((1%Z <= k)%Z /\
  ...
  (k < m)%Z) -> (((map.Map.get p3 k) < (ZOdiv n1 (map.Map.get p3 k)))%Z ->
  ((k + 1%Z)%Z < j)%Z))))))))))).
...
red in h23. destruct h23 as (hh1, (hh2, _)).
