...
let ge_int : Term.lsymbol =
  Theory.ns_find_ls int_module.Pmodule.mod_intf.Pmodule.mod_theory.Theory.th_export
    [Ident.op_infix ">="]
...
let ref_type : Ity.itysymbol =
  Pmodule.ns_find_its ref_module.Pmodule.mod_intf.Pmodule.mod_export ["ref"]

(* the "ref" function *)
let ref_fun : Expr.rsymbol =
  Pmodule.ns_find_rs ref_module.Pmodule.mod_intf.Pmodule.mod_export ["ref"]

(* the "!" function *)
let get_fun : Expr.rsymbol =
  Pmodule.ns_find_rs ref_module.Pmodule.mod_intf.Pmodule.mod_export [Ident.op_prefix "!"]
...
let my_tasks : Task.task list =
  Task.split_theory mod2.Pmodule.mod_intf.Pmodule.mod_theory None None
