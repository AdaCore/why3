...
module Output ( M : Ast_or_mlw) = struct
  ...
  and print_expr_desc fmt ed =
    ...
    | Efun(binder_list,pty_option,mask,spec,expr) -> fprintf fmt (ast_or_mlw "Efun(%a,%a,%a,%a,%a)" "Efun(%a,%a,%a,%a,%a)") (Pp.print_list Pp.comma (print_binder ~paren:false)) binder_list (Pp.print_option print_pty) pty_option print_mask mask print_spec spec print_expr expr
    | Eany(param_list,rs_kind,pty_option,mask,spec) -> fprintf fmt (ast_or_mlw "Eany(%a,%a,%a,%a,%a)" "Eany(%a,%a,%a,%a,%a)") (Pp.print_list Pp.comma print_param) param_list print_rs_kind rs_kind (Pp.print_option print_pty) pty_option print_mask mask print_spec spec
  ...
  and print_fundef fmt (ident,ghost,rs_kind,binder_list,pty_option,mask,spec,expr) =
    match pp_kind with
