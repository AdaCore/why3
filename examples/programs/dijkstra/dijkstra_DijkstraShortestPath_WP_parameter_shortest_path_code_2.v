...
Theorem WP_parameter_shortest_path_code : forall (src:vertex) (dst:vertex),
  ...
  v1 (get d3 v1))) /\ ((forall (v1:vertex), (mem v1 q3) -> (path src v1
  (get d3 v1))) /\ forall (m:vertex), (min m q3 d3) -> forall (x:vertex),
  forall (dx:Z), (path src x dx) -> ((dx < (get d3 m))%Z -> (mem x
  visited2)))))))))) /\ forall (x:vertex), (mem x visited2) ->
  forall (y:vertex), (mem y (g_succ x)) -> (((~ (x = u)) \/ ((x = u) /\
  ~ (mem y su))) -> ((mem y visited2) \/ (mem y q3)))) ->
  forall (result:bool), ((result = true) <-> ~ forall (x:vertex), ~ (mem x
  su)) -> ((result = true) -> ((~ forall (x:vertex), ~ (mem x su)) ->
  forall (su1:(set vertex)), forall (v1:vertex), ((mem v1 su) /\
  (su1 = (remove v1 su))) -> forall (q4:(set vertex)) (d4:(map vertex Z)),
  (((mem v1 visited2) /\ ((q4 = q3) /\ (d4 = d3))) \/ (((mem v1 q4) /\
  (((get d4 v1) <= ((get d4 u) + (weight u v1))%Z)%Z /\ ((q4 = q3) /\
  (d4 = d3)))) \/ (((mem v1 q4) /\ ((((get d3 u) + (weight u v1))%Z < (get d3
  v1))%Z /\ ((q4 = q3) /\ (d4 = (set1 d3 v1 ((get d3 u) + (weight u
  v1))%Z))))) \/ ((~ (mem v1 visited2)) /\ ((~ (mem v1 q3)) /\ ((q4 = (add v1
  q3)) /\ (d4 = (set1 d3 v1 ((get d3 u) + (weight u v1))%Z)))))))) ->
  forall (v2:vertex), (mem v2 q4) -> (path src v2 (get d4 v2))))))).
