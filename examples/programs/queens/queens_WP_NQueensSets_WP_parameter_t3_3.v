...
Theorem WP_parameter_t3 : forall (a:(set Z)) (b:(set Z)) (c:(set Z)),
  ...
  (0%Z <= i)%Z -> ((forall (j:Z), ((0%Z <= j)%Z /\ (j < k2)%Z) ->
  ~ ((get col2 j) = ((i + j)%Z - k2)%Z)) -> ~ (mem i (succ (add (min_elt e)
  b)))))).
(* YOU MAY EDIT THE PROOF BELOW *)
intuition.
generalize (succ_def (add (min_elt e) b) i); intuition.
clear H23.
generalize (add_def1 _ (i-1)%Z (min_elt e) b); intuition.
apply H21 with k1; try omega.
subst col2; rewrite Select_eq; omega.
