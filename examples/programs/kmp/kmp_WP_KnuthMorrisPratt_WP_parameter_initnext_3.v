...
Theorem WP_parameter_initnext : forall (p:Z), forall (p1:(map Z char)),
  ...
  ((j = 0%Z) -> forall (i1:Z), (i1 = (i + 1%Z)%Z) -> (((0%Z <= i1)%Z /\
  (i1 < p)%Z) -> forall (next2:(map Z Z)), (next2 = (set next1 i1 0%Z)) ->
  forall (z:Z), (((j + 1%Z)%Z < z)%Z /\ (z < (i1 + 1%Z)%Z)%Z) ->
  ~ (matches p2 ((i1 + 1%Z)%Z - z)%Z p2 0%Z z))))))))))).
