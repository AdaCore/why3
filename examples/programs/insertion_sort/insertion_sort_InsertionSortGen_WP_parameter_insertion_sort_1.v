...
(* Why3 assumption *)
Definition sorted_sub(a:(map Z elt)) (l:Z) (u:Z): Prop := forall (i1:Z)
...
(* Why3 assumption *)
Definition sorted_sub1(a:(array elt)) (l:Z) (u:Z): Prop :=
...
(* Why3 goal *)
Theorem WP_parameter_insertion_sort : forall (a:Z), forall (a1:(map Z elt)),
  let a2 := (mk_array a a1) in ((1%Z <= (a - 1%Z)%Z)%Z -> forall (a3:(map Z
  elt)), forall (i:Z), ((1%Z <= i)%Z /\ (i <= (a - 1%Z)%Z)%Z) ->
  (((sorted_sub a3 0%Z i) /\ (permut a2 (mk_array a a3))) ->
  (((0%Z <= i)%Z /\ (i < a)%Z) -> let v := (get a3 i) in forall (j:Z)
  (a4:(map Z elt)), (((((0%Z <= j)%Z /\ (j <= i)%Z) /\ (permut a2 (mk_array a
  (set a4 j v)))) /\ forall (k1:Z) (k2:Z), (((0%Z <= k1)%Z /\
  (k1 <= k2)%Z) /\ (k2 <= i)%Z) -> ((~ (k1 = j)) -> ((~ (k2 = j)) ->
  (le (get a4 k1) (get a4 k2))))) /\ forall (k:Z), (((j + 1%Z)%Z <= k)%Z /\
  (k <= i)%Z) -> (le v (get a4 k))) -> ((0%Z < j)%Z ->
  (((0%Z <= (j - 1%Z)%Z)%Z /\ ((j - 1%Z)%Z < a)%Z) -> ((~ (le (get a4
  (j - 1%Z)%Z) v)) -> (((0%Z <= (j - 1%Z)%Z)%Z /\ ((j - 1%Z)%Z < a)%Z) ->
  (((0%Z <= j)%Z /\ (j < a)%Z) -> forall (a5:(map Z elt)), (a5 = (set a4 j
  (get a4 (j - 1%Z)%Z))) -> ((exchange (set a4 j v) (set a5 (j - 1%Z)%Z v)
  (j - 1%Z)%Z j) -> forall (j1:Z), (j1 = (j - 1%Z)%Z) -> (permut a2
  (mk_array a (set a5 j1 v)))))))))))).
intros a a1 a2 h1 a3 i (h2,h3) (h4,h5) (h6,h7) v j a4
