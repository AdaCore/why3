...
Theorem ks_value : forall (n:Z), (0%Z <= n)%Z -> ((is_value (ks n)) ->
  ((0%Z <= n)%Z /\ (n <= 1%Z)%Z)).
intuition.
assert (case: (n=0 \/ 0<=n-1)%Z) by omega. destruct case.
omega.
assert (case: (n=1 \/ 0<=(n-1)-1)%Z) by omega. destruct case.
omega.
replace n with ((((n-1)-1)+1)+1)%Z in H0 by omega.
rewrite ksS in H0; try omega.
rewrite ksS in H0; try omega.
simpl in H0.
generalize (only_K_ks (n-1-1)%Z H2).
destruct (ks (n-1-1)%Z) in *; intuition.
inversion H3.
Qed.
