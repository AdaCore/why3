...
Theorem WP_parameter_reduction3 : forall (t:term), (exists n:Z,
  ...
  | (App t1 t2) => (exists n:Z, (0%Z <= n)%Z /\ (t1 = (ks n))) ->
      ...
      | (App (App S v1) v2) => (exists n:Z, (0%Z <= n)%Z /\ (t2 = (ks n))) ->
          ...
          result1)) = (ks n))) -> forall (result2:term),
          ((is_value result2) /\ forall (n:Z), (0%Z <= n)%Z ->
          ((((App (App v1 result1) (App v2 result1)) = (ks (2%Z * n)%Z)) ->
          (result2 = K)) /\ (((App (App v1 result1) (App v2
          result1)) = (ks ((2%Z * n)%Z + 1%Z)%Z)) -> (result2 = (App K
          K))))) -> ((is_value result2) /\ forall (n:Z), (0%Z <= n)%Z ->
          (((t = (ks (2%Z * n)%Z)) -> (result2 = K)) /\
          ((t = (ks ((2%Z * n)%Z + 1%Z)%Z)) -> (result2 = (App K K))))))
      | _ => True
      end
  end.
intros t (n, (h1, h2)).
