...
Theorem WP_parameter_reduction3 : forall (t:term), (exists n:Z,
  ...
  match t with
  | S => True
  | K => forall (n:Z), (0%Z <= n)%Z -> ((t = (ks n)) ->
      (K = (ks (int.EuclideanDivision.mod1 n 2%Z))))
  | (App t1 t2) => True
  end.
intuition.
destruct t; auto.
intros.
assert (n = 0)%Z.
assert (case: (n = 0 \/ n = (n-1 + 1) /\ 0 <= n-1)%Z) by omega.
destruct case; auto.
destruct H2 as (h2,h3).
rewrite h2 in H1.
rewrite ksS in H1; auto.
discriminate H1.
subst n.
assert (EuclideanDivision.mod1 0 2 = 0)%Z.
