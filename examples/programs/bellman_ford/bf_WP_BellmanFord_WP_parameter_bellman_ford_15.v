...
Theorem WP_parameter_bellman_ford : (1%Z <= ((cardinal vertices) - 1%Z)%Z)%Z ->
  forall (m:(map vertex t)), (inv1 m (((cardinal vertices) - 1%Z)%Z + 1%Z)%Z
  (empty :(set1 (vertex* vertex)%type))) -> ((inv1 m (cardinal vertices)
  (empty :(set1 (vertex* vertex)%type))) -> forall (result:(set1 (vertex*
  vertex)%type)), (result = edges) -> forall (es:(set1 (vertex*
  vertex)%type)), ((subset es edges) /\ (inv2 m (diff edges es))) ->
  forall (result1:bool), ((result1 = true) <-> (is_empty es)) ->
...
assert (path s (infix_plpl lu (Cons u Nil)) v) by ae.
assert (exists lv': list vertex, path s lv' v /\ (length lv' < cardinal vertices)%Z) by ae.
ae.
Qed.
