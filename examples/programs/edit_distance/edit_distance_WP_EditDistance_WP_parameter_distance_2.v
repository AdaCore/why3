...
Theorem WP_parameter_distance : forall (w1:Z) (w2:Z),
  ...
  (((0%Z <= i)%Z /\ (i < w1)%Z) -> (((map.Map.get w11 i) = (map.Map.get w21
  j)) -> (((0%Z <= j)%Z /\ (j < (w2 + 1%Z)%Z)%Z) -> forall (t4:(map.Map.map Z
  Z)), (t4 = (map.Map.set t3 j oldt)) -> forall (k:Z),
  (((j - 1%Z)%Z < k)%Z /\ (k <= w2)%Z) -> (min_dist (suffix w12 i)
  (suffix w22 k) (map.Map.get t4 k))))))))))))))))).
Proof.
intuition.
intuition.
