...
Axiom Add_diff : forall {a:Type}, forall (b:(bag a)) (x:a), (mem x b) ->
...
Axiom Occ_elements : forall {a:Type}, forall (a1:(map Z a)) (i:Z) (j:Z)
  (n:Z), ((i <= j)%Z /\ (j < n)%Z) -> (0%Z < (nb_occ (get a1 j) (elements a1
  i n)))%Z.
...
Axiom Left_sup_el : forall (a:(map Z Z)) (n:Z), (is_heap_array a 0%Z n) ->
  forall (j:Z), ((0%Z <= j)%Z /\ (j < n)%Z) -> (((left1 j) < n)%Z -> ((get a
  j) <= (get a (left1 j)))%Z).

Axiom Right_sup_el : forall (a:(map Z Z)) (n:Z), (is_heap_array a 0%Z n) ->
  forall (j:Z), ((0%Z <= j)%Z /\ (j < n)%Z) -> (((right1 j) < n)%Z -> ((get a
  j) <= (get a (right1 j)))%Z).
...
Axiom Model_empty : forall (a:(map Z Z)), ((model (a,
  0%Z)) = (empty_bag :(bag Z))).
...
Theorem Model_set : forall (a:(map Z Z)) (v:Z) (i:Z) (n:Z), ((0%Z <= i)%Z /\
  (i < n)%Z) -> ((add (get a i) (model ((set a i v), n))) = (add v (model (a,
  n)))).
