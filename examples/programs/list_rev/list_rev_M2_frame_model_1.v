...
Axiom frame_list : forall (next1:(map pointer pointer)) (p:pointer)
...
Axiom frame_list_ft : forall (next1:(map pointer pointer)) (p:pointer)
  (q:pointer) (v:pointer), (~ (in_ft q (list_ft next1 p))) -> ((is_list next1
  p) -> ((list_ft next1 p) = (list_ft (set next1 q v) p))).

(* Why3 assumption *)
Definition sep_list_list(next1:(map pointer pointer)) (p1:pointer)
  (p2:pointer): Prop := forall (q:pointer), (~ (in_ft q (list_ft next1
  p1))) \/ ~ (in_ft q (list_ft next1 p2)).

Axiom acyclic_list : forall (next1:(map pointer pointer)) (p:pointer),
  (~ (p = null)) -> ((is_list next1 p) -> (sep_node_list next1 p (get next1
  p))).
...
Axiom model_def1 : forall (next1:(map pointer pointer)) (p:pointer),
  (p = null) -> ((model next1 p) = (Nil :(list pointer))).

Axiom model_def2 : forall (next1:(map pointer pointer)) (p:pointer),
  (~ (p = null)) -> ((model next1 p) = (Cons p (model next1 (get next1 p)))).


(* Why3 goal *)
Theorem frame_model : forall (next1:(map pointer pointer)) (p:pointer)
  (q:pointer) (v:pointer), (is_list next1 p) -> ((~ (in_ft q (list_ft next1
  p))) -> ((model next1 p) = (model (set next1 q v) p))).
