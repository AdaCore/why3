...
Theorem WP_parameter_shuffle : forall (b:(list Z)), forall (a:(list Z)),
  ...
  ((result3 = true) \/ (result4 = true)) -> forall (a2:(list Z)),
  forall (result5:Z),
  match a1 with
  | Nil => False
  | (Cons x t1) => (result5 = x) /\ (a2 = t1)
  end -> forall (c1:(list Z)), (c1 = (Cons result5 c)) -> exists aqt:(list
  Z), exists bqt:(list Z), ((reverse a) = (infix_plpl (reverse a2) aqt)) /\
  (((reverse b) = (infix_plpl (reverse b1) bqt)) /\ (shuffle aqt bqt c1)))).
