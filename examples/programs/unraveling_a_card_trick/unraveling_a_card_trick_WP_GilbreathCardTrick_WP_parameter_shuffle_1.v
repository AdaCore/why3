...
Theorem WP_parameter_shuffle : forall (b:(list Z)), forall (a:(list Z)),
  ...
  (b1 = (Nil:(list Z)))) -> ((~ (result2 = true)) -> forall (result3:bool),
  ((result3 = true) <-> (a1 = (Nil:(list Z)))) -> ((result3 = true) ->
  forall (b2:(list Z)), forall (result4:Z),
  match b1 with
  | Nil => False
  | (Cons x t1) => (result4 = x) /\ (b2 = t1)
  end -> forall (c1:(list Z)), (c1 = (Cons result4 c)) -> exists aqt:(list
...
subst.
exists a'; exists (Cons result4 b').
destruct b1.
inversion H9.
intuition (subst; clear H5 H6 H10 H7 H4 H3 result2).
rewrite h2.
