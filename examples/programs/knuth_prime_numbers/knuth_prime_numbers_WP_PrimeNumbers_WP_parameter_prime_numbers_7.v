...
Theorem WP_parameter_prime_numbers : forall (m:Z), (2%Z <= m)%Z ->
  ...
  k) <  (ZOdiv n1 (get p4 k)))%Z -> ((k + 1%Z)%Z <  j)%Z))))))))))).
...
apply False_ind.
subst k.
assert (2 < get p4 (j-1))%Z.
destruct H10 as (_, (h1, (h2, _))).
rewrite <- h1. apply h2; omega.
assert (ne0: (get p4 (j-1) <> 0)%Z) by omega.
generalize (Div_mod n1 (get p4 (j-1))%Z ne0). intro div.
assert (ne1: (0 <= n1 /\ get p4 (j-1) <> 0)%Z) by omega.
generalize (Mod_sign_pos n1 (get p4 (j-1))%Z ne1). intro mod_.
assert (n1 > get p4 (j - 1) * get p4 (j-1))%Z.
assert (get p4 (j - 1) * (ZOdiv n1 (get p4 (j - 1))) > get p4 (j - 1) * get p4 (j-1))%Z.
...
omega.
assert (n1 < get p4 (j - 1) * get p4 (j - 1)%Z).
assert (2 * get p4 (j - 1) < get p4 (j - 1) * get p4 (j - 1))%Z.
