...
Theorem WP_parameter_prime_numbers : forall (m:Z), (2%Z <= m)%Z ->
  ...
  forall (n2:Z), (n2 = (n1 + 2%Z)%Z) -> (n2 <  (2%Z * (get p4
  (j - 1%Z)%Z))%Z)%Z))))))))).
(* YOU MAY EDIT THE PROOF BELOW *)
intros.
assert (case: (n2 < 2 * get p4 (j-1) \/ n2 >= 2 * get p4 (j-1))%Z) by omega.
destruct case.
auto.
apply False_ind.
apply Bertrand_postulate with (get p4 (j-1)%Z); intuition.
subst p5.
red in H24. unfold get1 in H24; simpl in H24.
apply H24; omega.
red; intros.
assert (case: (x < n1 \/ x = n1 \/ x = n1+1)%Z) by omega. destruct case.
apply H32; omega.
destruct H36; subst x.
intuition.
assert (even (n1+1))%Z.
apply odd_even; auto.
red; intro.
assert (n1+1 = 2).
apply even_prime; auto.
omega.
Qed.
(* DO NOT EDIT BELOW *)
