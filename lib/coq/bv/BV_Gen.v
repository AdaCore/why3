...
Lemma to_uint_bounds : forall (v:t), (uint_in_range (to_uint v)).
  ...
  apply inj_le.
  unfold max_int.
  change (bvec_to_nat size_bv v <= Z.to_nat (Z.pred (Pow2int.pow2 size)))%nat.
  rewrite Z2Nat.inj_pred.
  apply NPeano.Nat.lt_le_pred.
  apply bvec_to_nat_range.
Qed.

Lemma to_uint_bounds_lt: forall v, (0%Z <= to_uint v < two_power_size)%Z.
  split.
...
(* Why3 goal *)
Lemma Nth_bv_is_nth2 : forall (x:t) (i:Z), (uint_in_range i) -> ((nth_bv x
  (of_int i)) = (nth x i)).
  intros x i h1.
...
Definition rotate_right: t -> t -> t.
  exact (fun b p => rotate_right_aux b (bvec_to_nat size_bv p)).
Defined.

(* Why3 goal *)
Definition rotate_left: t -> t -> t.
  exact (fun b p => rotate_left_aux b (bvec_to_nat size_bv p)).
Defined.

(* Why3 goal *)
Lemma Nth_rotate_left : forall (v:t) (n:t) (i:t), (ult i (of_int size)) ->
  ((ult n (sub ones (of_int size))) -> ((nth_bv v i) = (nth_bv (rotate_left v
  n) (urem (add i n) (of_int size))))).
  intros v n i h1 h2.
  unfold ult in h1, h2.
  rewrite to_uint_of_int in h1 by apply size_in_range.
  unfold nth_bv.
  rewrite to_uint_urem, to_uint_add.
  rewrite mod1_out with (x := (to_uint i + to_uint n)).
  rewrite to_uint_of_int by apply size_in_range.
  rewrite size_int_S.
  ...
  rewrite to_uint_of_int in h2 by apply size_in_range.
...
(* Why3 goal *)
Lemma Nth_rotate_right : forall (v:t) (n:t) (i:t), (ult i (of_int size)) ->
  ((ult n (sub ones (of_int size))) -> ((nth_bv (rotate_right v n)
  i) = (nth_bv v (urem (add i n) (of_int size))))).
  intros v n i h1 h2.
  unfold ult in h1, h2.
  rewrite to_uint_of_int in h1 by apply size_in_range.
  unfold nth_bv.
  ...
  simpl (_+_).
  apply Z.lt_le_pred, id_lt_pow2.
  unfold two_power_size, max_int, size.
  omega.
  split.
  unfold max_int.
  apply Z.lt_le_pred, Pow2int.pow2pos.
...
Lemma mask_correctness :
  ...
  rewrite mod1_out by (rewrite max_int_S;split; auto with zarith); trivial.
  rewrite H2.
  ...
  pose size_in_range as w; unfold uint_in_range in w.
  ...
  rewrite max_int_S, mod1_out; auto with zarith.
  rewrite H1.
  rewrite Nth_bv_is_nth.
