...
(* Why3 goal *)
Lemma eb_gt_1 : (1%Z < 11%Z)%Z.
Proof.
now simpl.
Qed.

(* Why3 goal *)
Lemma sb_gt_1 : (1%Z < 53%Z)%Z.
Proof.
now simpl.
Qed.
...
Lemma max_int_spec :
  (%Z
   =
   ((bv.Pow2int.pow2 (bv.Pow2int.pow2 (11%Z - 1%Z)%Z)) -
    (bv.Pow2int.pow2 ((bv.Pow2int.pow2 (11%Z - 1%Z)%Z) - 53%Z)%Z))%Z).
Proof.
now simpl.
Qed.
...
Lemma max_real_cst :
  max_real 11 53 = %R.
Proof.
  rewrite <- max_real_is_F2R.
  change (F2R (Float radix2 _ _)) with (IZR 9007199254740991 * IZR (Zpower 2 971))%R.
...
(* Why3 goal *)
Lemma pow2sb1 : (9007199254740992%Z = (bv.Pow2int.pow2 53%Z)).
Proof.
now simpl.
Qed.
