...
Lemma OneMinSb : ((1 - sb <=? emax - sb)%Z)%bool = true.
  apply Zle_bool_true.
  pose Hemax'; pose sb_gt_1; omega.
Qed.

(* split to reuse for half *)
Lemma oneF_bounded : bounded sb emax (shift_pos (sb_pos - 1) 1) (1 - sb) = true.
  unfold bounded.
  apply Bool.andb_true_iff; split;[|apply OneMinSb].
...
Lemma two_bounded: bounded sb emax (shift_pos (sb_pos - 1) 1) (2 - sb) = true.
  unfold bounded.
  ...
  fold sb.
  unfold FLT_exp.
  replace (sb - 1 + 1 + (2 - sb))%Z with (2)%Z by ring.
  apply Zmax_l.
  pose sb_gt_1; pose Hemax'; omega.
  apply Zle_bool_true.
  pose Hemax'; pose sb_gt_1; omega.
...
Lemma max_eb_bounded : bounded sb emax (2 ^ sb_pos - 1) (emax - sb) = true.
  ...
  omega.
  apply Zle_bool_true; easy.
Qed.

(* Why3 goal *)
Definition from_binary: Z -> t.
Admitted.

(* Why3 goal *)
Definition prefix_ex: mode -> R -> t.
Admitted.
...
Definition One_Nan: t := B754_nan sb emax false One_Nan_pl.
...
(* Why3 goal *)
Notation sub := (Bminus sb emax _ Hemax' nan_bf).

(* Why3 goal *)
Notation mul := (Bmult sb emax _ Hemax' nan_bf).
...
Notation ge := (fun a b => le b a).

Notation gt := (fun a b => lt b a).

Lemma gt_bcompare : forall {x y}, gt x y <-> Bcompare _ _ x y = Some Gt.
  intros x y.
...
Lemma ge_bcompare : forall {x y}, ge x y <-> Bcompare _ _ x y = Some Gt \/ Bcompare _ _ x y = Some Eq.
  intros x y.
  unfold le.
  rewrite <-gt_bcompare.
...
Lemma eq_zero_dec: forall x, {eq x zeroF} + {~eq x zeroF}.
  intros.
  destruct x, b; unfold eq; simpl; auto; right; easy.
Qed.

Lemma eq_zero_iff: forall {x}, eq zeroF x <-> x = zeroF \/ x = neg zeroF.
  intro; unfold eq; destruct x, b; simpl.
  split; auto.
  split; auto.
...
Lemma zero_is_zero: forall {b}, eq zeroF (B754_zero _ _ b).
  easy.
Qed.
...
Definition is_normal: t -> Prop.
  exact (fun x => match x with
                    ...
                    | B754_finite _ _ _ _ e _ => (emin < e)%Z
                    | _ => False
        end).
Defined.
...
(* Why3 goal *)
Notation is_nan := (is_nan sb emax).
...
Lemma is_nan_dec: forall x, {is_nan x} + {~ is_nan x}.
  intro; destruct x; simpl; intuition.
Qed.
...
Lemma le_not_nan: forall {x y}, le x y -> ~ is_nan x /\ ~ is_nan y.
  intros x y h; destruct h; [apply (lt_not_nan H)| apply (eq_not_nan H)].
Qed.
...
Lemma le_or_lt_or_nan: forall x y, le x y \/ lt y x \/ is_nan x \/ is_nan y.
  ...
  unfold le, lt.
  set (x:=(B754_finite sb emax b m e e0)).
  set (y:=(B754_finite sb emax b0 m0 e1 e2)).
...
Lemma min_nan: forall x y, (is_nan x \/ is_nan y) <-> is_nan (min x y).
  intros; destruct x, y; split; auto; intro; try (destruct H; easy); try (destruct b, b0; easy).
  destruct b, b0; try easy.
  unfold min in H.
  inversion H.
  case (e ?= e1)%Z in H1; try easy.
  case CompOpp in H1; try easy.
  unfold min in H; inversion H.
  case (e ?= e1)%Z in H1; try easy.
  case Pos.compare_cont in H1; try easy.
Qed.

(* Why3 goal *)
Definition is_positive: t -> Prop.
  exact (fun x => match x with
                    | B754_zero _ _ false => True
                    | B754_finite _ _ false _ e _ => True
...
Lemma is_positive_Bsign: forall x, ~ is_nan x -> is_positive x <-> Bsign sb emax x = false.
  split.
  destruct x, b; easy.
  destruct x, b; try easy.
  contradict H; easy.
Qed.

Lemma is_positive_correct: forall x, is_positive x <-> lt zeroF x \/ x = B754_zero sb emax false.
  split.
  intro h; destruct x, b; auto; easy.
  intro h; destruct h, x, b; easy.
Qed.

(* Why3 goal *)
Definition is_negative: t -> Prop.
  exact (fun x => match x with
                    | B754_zero _ _ true => True
                    | B754_finite _ _ true _ e _ => True
...
Lemma is_negative_correct: forall x, is_negative x <-> gt zeroF x \/ x = B754_zero sb emax true.
  split.
  intro h; destruct x, b; auto; easy.
  intro h; destruct h, x, b; easy.
Qed.
...
(* Why3 goal *)
Notation is_finite := (is_finite sb emax).
...
(* Why3 goal *)
Lemma zero_of_int : forall (m:mode), (zeroF = (of_int m 0%Z)).
auto.
Qed.

(* Why3 goal *)
Notation round := (fun m => round radix2 fexp (round_mode m)).

Lemma bpow_0: bpow radix2 0 = 1. easy. Qed.
...
Lemma round_of_int_alt: forall {m:mode} {x},
  Fcore_generic_fmt.round radix2 (FLT_exp (3 - emax - sb) sb) (round_mode m) (@F2R radix2 {| Fnum := x; Fexp := 0 |}) = round m (IZR x).
  intros.
  rewrite IZR_alt.
  reflexivity.
Qed.
...
Definition max_value: t.
  exact (B754_finite sb emax false (2 ^ sb_pos - 1) (emax - sb) max_eb_bounded).
Defined.
...
Lemma is_finite_B: forall (x:t), is_finite x ->
                                 exists b, x = B754_zero sb emax b \/ exists b m e p, x = B754_finite sb emax b m e p.
  intro x.
  destruct x; try easy; unfold is_finite; intros.
  exists b; auto.
  exists b; right.
...
Lemma max_real_gt_zero : 0 < max_real.
  rewrite max_real_alt.
  assert (bpow radix2 (emax - sb) < bpow radix2 emax ).
  apply bpow_lt.
  pose Hsb'.
  auto with zarith.
  fourier.
...
Lemma max_real_ge_zero : 0 <= max_real.
  apply (Rlt_le _ _ max_real_gt_zero).
Qed.
...
Lemma max_real_Rabs : Rabs max_real = max_real.
  apply Abs.abs_def.
  apply max_real_ge_zero.
Qed.
...
Lemma max_real_round : Fcore_generic_fmt.round radix2 fexp ZnearestA max_real = max_real.
  rewrite <-max_value_to_real at 2.
  rewrite round_sign_NA_correct with (e := (emax - sb)%Z) (m := Z.pos (2 ^ sb_pos - 1)) (l := loc_Exact).

  simpl.
  rewrite F2R_cond_Zopp.
  unfold cond_Ropp.
  assert (Rlt_bool max_real 0 = false) by apply (Rlt_bool_false _ _ max_real_ge_zero).
  rewrite H; reflexivity.

  unfold inbetween_float.
  apply inbetween_Exact.
  rewrite max_real_Rabs.
  symmetry; apply max_real_is_F2R.
...
Lemma Rabs_round_max_real_emax: forall {m} {x}, Rabs (round m x) <= max_real <-> Rabs (round m x) < bpow radix2 emax.
...
Lemma r_to_fp_no_overflow :
  forall {m} {x}, no_overflow m x -> is_finite (r_to_fp m x) /\ to_real (r_to_fp m x) = round m x.
  intros m x h.
  generalize (r_to_fp_correct m x (proj1 no_overflow_Rabs_round_emax h)).
  intro.
  destruct H.
  split; easy.
Qed.
...
Lemma of_int_correct :
  ...
  - intro; destruct H, H0.
    ...
    split.
    rewrite round_of_int_alt in H; auto.
    split; auto.
...
Lemma is_finite_no_overflow : forall {x} (m:mode), is_finite x -> no_overflow m (to_real x).
  intros x m h.
  pose proof (is_finite1 _ h).
  rewrite no_overflow_Rabs_round_max_real, Abs.Abs_le.
  unfold in_range in H.
  destruct (is_finite_B _ h).
  destruct H0 as [H0|(b,(m0,(e,(p,H0))))]; rewrite H0.
  + simpl.
    unfold Fcore_generic_fmt.round.
    rewrite scaled_mantissa_0.
    destruct (valid_rnd_round_mode m).
    change 0 with (Z2R 0).
    rewrite Zrnd_Z2R, F2R_0.
    pose proof (@zero_is_zero x0) as a;
      rewrite <-H0, (to_real_eq zero_is_finite h) in a.
    rewrite <-a in H; apply H.
  + rewrite H0 in H.
    simpl in H.
    ...
    destruct (valid_rnd_round_mode m).
    unfold to_real.
    assert (canonic_mantissa sb emax m0 e = true).
    assert (t:=p).
    apply Bool.andb_true_iff in t. easy.
    pose proof (canonic_canonic_mantissa sb emax b m0 e H1).
    pose proof (generic_format_canonic _ _ _ H2).
    change fexp with (FLT_exp (3 - emax - sb) sb).
    rewrite (scaled_mantissa_generic _ _ _ H3).
    rewrite Zrnd_Z2R.
    unfold F2R at 1 4.
    unfold Fnum, Fexp.
    rewrite <-(scaled_mantissa_generic _ _ _ H3).
...
Lemma round_idempotent : forall (m1:mode) (m2:mode) (x:R), ((round m1
...
Lemma r_to_fp_round : forall {x} (m:mode), r_to_fp m (round m x) = r_to_fp m x.
  intros x m.
  unfold r_to_fp.
  rewrite round_idempotent.
  reflexivity.
Qed.
...
(* Why3 goal *)
Lemma Bounded_real_no_overflow : forall (m:mode) (x:R), (in_range x) ->
  (no_overflow m x).
intros m x h1.
...
(* Why3 goal *)
Lemma Round_monotonic : forall (m:mode) (x:R) (y:R), (x <= y)%R -> ((round m
  x) <= (round m y))%R.
  intros m x y h1.
  apply round_le.
  apply fexp_Valid.
...
(* Why3 goal *)
Lemma Round_to_real : forall (m:mode) (x:t), (is_finite x) -> ((round m
  (to_real x)) = (to_real x)).
Proof with auto with typeclass_instances.
intros m x h.
apply round_generic...
...
(* Why3 goal *)
Lemma Round_down_le : forall (x:R), ((round RTN x) <= x)%R.
Proof with auto with typeclass_instances.
intros x.
apply round_DN_pt...
Qed.
...
(* Why3 goal *)
Lemma Round_up_ge : forall (x:R), (x <= (round RTP x))%R.
Proof with auto with typeclass_instances.
intros x.
apply round_UP_pt...
Qed.
...
(* Why3 goal *)
Lemma Round_down_neg : forall (x:R), ((round RTN (-x)%R) = (-(round RTP
  x))%R).
Proof.
intros x.
apply round_opp.
...
(* Why3 goal *)
Lemma Round_up_neg : forall (x:R), ((round RTP (-x)%R) = (-(round RTN x))%R).
Proof.
intros x.
...
Lemma Bmax_rep_int_finite: is_finite Bmax_rep_int.
  auto.
Qed.
...
- destruct e3.
  ...
  + destruct e1.
    Lemma Bool_dec: forall x y : bool, x = y \/ x <> y.
      intros.
      case (Bool.bool_dec x y); auto.
    Qed.
    destruct (Eqdep_dec.eq_proofs_unicity Bool_dec e0 e2).
    reflexivity.

  + unfold eq in h4; simpl in h4.
    destruct b0.
    * rewrite Z.compare_refl, Some_ext, <-Pos.compare_antisym in h4.
...
Lemma eq_trans : forall (x:t) (y:t) (z:t), (eq x y) -> ((eq y z) -> (eq x
  z)).
  intros x y z h1 h2.
  destruct x, y, z; try easy; destruct b, b0, b1; try easy;
...
Lemma match_comp_some : forall {T} {a:comparison} {b c d e: T},
    match a with
  | Eq => Some b
  | Lt => Some c
  | Gt => Some d
  ...
  split; intros; destruct a; inversion H; reflexivity.
Qed.
...
Lemma lt_eq_trans : forall {x y z:t}, lt x y -> eq y z -> lt x z.
  ...
  destruct x, y, z; try (destruct b, b0, b1; try easy; destruct h1; easy).
  unfold lt in h1.
  ...
  apply Rlt_le_trans with (B2R sb emax (B754_finite sb emax b0 m0 e1 e2)).
  + apply Rcompare_Lt_inv.
    now injection h1.
  + apply Rcompare_not_Gt_inv.
    inversion h1; inversion h2.
    destruct Rcompare; try easy.
    destruct Rcompare; easy.
Qed.
...
Lemma eq_lt_trans : forall {x y z:t}, eq x y -> lt y z -> lt x z.
  ...
  rewrite H1 by easy; clear H1.
  apply f_equal, Rcompare_Lt.
  apply Rle_lt_trans with (B2R sb emax (B754_finite sb emax b0 m0 e1 e2)).
  + apply Rcompare_not_Gt_inv.
    inversion h1; inversion h2.
    destruct Rcompare; easy.
  + apply Rcompare_Lt_inv.
...
Lemma lt_lt_trans : forall {x y z:t}, lt x y -> lt y z -> lt x z.
  ...
  rewrite H1 by easy; clear H1.
  apply f_equal, Rcompare_Lt.
  apply Rlt_trans with (B2R sb emax (B754_finite sb emax b0 m0 e1 e2)).
  + apply Rcompare_Lt_inv.
    now injection h1.
  + apply Rcompare_Lt_inv.
    now injection h2.
...
destruct x, y; intros (h,h1); try easy;
  try (destruct b,b0; try (destruct h; easy); destruct h1; easy).
...
+ apply Rcompare_not_Gt_inv.
  destruct (Rcompare (to_real (B754_finite sb emax b0 m0 e1 e2)) (to_real (B754_finite sb emax b m e e0))).
  easy.
  easy.
...
Lemma not_lt_ge : forall (x:t) (y:t), ((~ (lt x y)) /\ ((is_not_nan x) /\
  ...
  destruct x, y; intros (h,(h1,h2)); try easy; auto; try (destruct b, b0; auto; destruct h1; auto; easy).
...
Lemma le_special : forall (x:t) (y:t), (le x y) -> (((is_finite x) /\
  ...
  pose proof (Bcompare_correct sb emax x y).
  unfold le in h.
  destruct x, y; try easy; auto; try (destruct b, b0; auto; destruct h; auto; easy).

  - right; right. destruct b0, h; easy.
  - right; left. destruct b, h; easy.
  - right. destruct b, b0, h; try easy.
    + left; easy.
    + left; easy.
    + right; easy.
  - right; left. destruct b, h; easy.
  - right; right. destruct b, b0, h; easy.
Qed.
...
Lemma lt_special : forall (x:t) (y:t), (lt x y) -> (((is_finite x) /\
  ...
  destruct x, y; try easy; auto; try (destruct b, b0; easy).

  - right; right. destruct b0; easy.
  - right; left. destruct b; easy.
...
Lemma positive_non_zero_to_real : forall {x:t},
    is_finite x -> ~ is_zero x -> is_positive x ->
     0 < to_real x.
  intros x h1 h2 h3.
  rewrite <-zeroF_to_real; apply lt_finite; auto.
  destruct x; try easy.
  contradict h2; easy.
  simpl in h3.
...
Lemma negative_non_zero_to_real : forall {x:t},
...
- split; [apply to_real_positive; try easy|apply to_real_negative; easy].
  ...
  rewrite Rmult_0_l, <-Ropp_mult_distr_r.
...
(* Why3 goal *)
Lemma add_finite : forall (m:mode) (x:t) (y:t), (is_finite x) -> ((is_finite
  y) -> ((no_overflow m ((to_real x) + (to_real y))%R) -> ((is_finite (add m
  x y)) /\ ((to_real (add m x y)) = (round m
...
Lemma to_real_B754_zero: forall {b}, to_real (B754_zero _ _ b) = 0.
  intro b.
...
(* Why3 goal *)
Lemma add_finite_rev : forall (m:mode) (x:t) (y:t), (is_finite (add m x
  y)) -> ((is_finite x) /\ (is_finite y)).
intros m x y h1.
destruct x, y; try easy; destruct b, b0; easy.
Qed.

(* Why3 goal *)
Lemma add_finite_rev_n : forall (m:mode) (x:t) (y:t), (to_nearest m) ->
  ((is_finite (add m x y)) -> ((no_overflow m
  ((to_real x) + (to_real y))%R) /\ ((to_real (add m x y)) = (round m
  ((to_real x) + (to_real y))%R)))).
intros m x y h1 h2.
...
destruct x, y; try easy; try (rewrite to_real_B754_zero).
apply Bounded_real_no_overflow; rewrite (@to_real_B754_zero b0); split; fourier.
...
set (x := B754_finite sb emax b m0 e e0).
set (y := B754_finite sb emax b0 m1 e1 e2).
fold x y in h2, H, H0.
...
(* Why3 goal *)
Lemma sub_finite : forall (m:mode) (x:t) (y:t), (is_finite x) -> ((is_finite
  y) -> ((no_overflow m ((to_real x) - (to_real y))%R) -> ((is_finite (sub m
  x y)) /\ ((to_real (sub m x y)) = (round m
  ((to_real x) - (to_real y))%R))))).
intros m x y h1 h2 h3.
...
(* Why3 goal *)
Lemma sub_finite_rev : forall (m:mode) (x:t) (y:t), (is_finite (sub m x
  y)) -> ((is_finite x) /\ (is_finite y)).
  intros m x y h1.
  destruct x, y; try easy; destruct b, b0; easy.
Qed.
...
(* Why3 goal *)
Lemma sub_finite_rev_n : forall (m:mode) (x:t) (y:t), (to_nearest m) ->
  ((is_finite (sub m x y)) -> ((no_overflow m
  ((to_real x) - (to_real y))%R) /\ ((to_real (sub m x y)) = (round m
  ((to_real x) - (to_real y))%R)))).
intros m x y h1 h2.
...
(* Why3 goal *)
Lemma mul_finite : forall (m:mode) (x:t) (y:t), (is_finite x) -> ((is_finite
  y) -> ((no_overflow m ((to_real x) * (to_real y))%R) -> ((is_finite (mul m
  x y)) /\ ((to_real (mul m x y)) = (round m
  ((to_real x) * (to_real y))%R))))).
intros m x y h1 h2 h3.
...
(* Why3 goal *)
Lemma mul_finite_rev : forall (m:mode) (x:t) (y:t), (is_finite (mul m x
  y)) -> ((is_finite x) /\ (is_finite y)).
  intros m x y h1.
  destruct x, y; try easy; destruct b, b0; easy.
Qed.
...
(* Why3 goal *)
Lemma mul_finite_rev_n : forall (m:mode) (x:t) (y:t), (to_nearest m) ->
  ((is_finite (mul m x y)) -> ((no_overflow m
  ((to_real x) * (to_real y))%R) /\ ((to_real (mul m x y)) = (round m
  ((to_real x) * (to_real y))%R)))).
intros m x y h1 h2.
...
(* Why3 goal *)
Lemma div_finite : forall (m:mode) (x:t) (y:t), (is_finite x) -> ((is_finite
  y) -> ((~ (is_zero y)) -> ((no_overflow m ((to_real x) / (to_real y))%R) ->
...
(* Why3 goal *)
Lemma div_finite_rev : forall (m:mode) (x:t) (y:t), (is_finite (div m x
  y)) -> (((is_finite x) /\ ((is_finite y) /\ ~ (is_zero y))) \/ ((is_finite
  x) /\ ((is_infinite y) /\ ((to_real (div m x y)) = 0%R)))).
  intros m x y h1.
  destruct x, y; try easy.
  right; destruct b, b0; easy.
  left; destruct b, b0; easy.
  right; destruct b, b0; easy.
  left; destruct b, b0; easy.
Qed.

(* Why3 goal *)
Lemma div_finite_rev_n : forall (m:mode) (x:t) (y:t), (to_nearest m) ->
  ((is_finite (div m x y)) -> ((is_finite y) -> ((no_overflow m
  ((to_real x) / (to_real y))%R) /\ ((to_real (div m x y)) = (round m
  ((to_real x) / (to_real y))%R))))).
  intros m x y h1 h2 h3.
...
Lemma abs_finite : forall (x:t), (is_finite x) -> ((is_finite (abs x)) /\
  ...
  (abs x)))).
  intros x h1.
  assert (is_finite (abs x)).
  rewrite is_finite_Babs; apply h1.
  split;[assumption|split].
  apply B2R_Babs.
  pose proof (is_finite_not_nan h1).
  assert (is_nan x = false).
  - destruct is_nan; auto.
    destruct H0; auto.
  - pose proof (Bsign_Babs sb emax pair _ H1).
...
(* Why3 goal *)
Lemma fma_finite : forall (m:mode) (x:t) (y:t) (z:t), (is_finite x) ->
  ((is_finite y) -> ((is_finite z) -> ((no_overflow m
...
(* Why3 goal *)
Lemma fma_finite_rev : forall (m:mode) (x:t) (y:t) (z:t), (is_finite (fma m x
  y z)) -> ((is_finite x) /\ ((is_finite y) /\ (is_finite z))).
intros m x y z h1.
Admitted.

(* Why3 goal *)
Lemma fma_finite_rev_n : forall (m:mode) (x:t) (y:t) (z:t), (to_nearest m) ->
  ((is_finite (fma m x y z)) -> ((no_overflow m
  (((to_real x) * (to_real y))%R + (to_real z))%R) /\ ((to_real (fma m x y
  z)) = (round m (((to_real x) * (to_real y))%R + (to_real z))%R)))).
intros m x y z h1 h2.
Admitted.

(* Why3 goal *)
Lemma sqrt_finite : forall (m:mode) (x:t), (is_finite x) ->
  ((0%R <= (to_real x))%R -> ((is_finite (sqrt m x)) /\ ((to_real (sqrt m
  x)) = (round m (Reals.R_sqrt.sqrt (to_real x)))))).
  intros m x h1 h2.
  destruct (Bsqrt_correct sb emax Hsb' Hemax' (fun b => (true,One_Nan_pl)) m x) as (g,(g1,g2)).
  split; auto.
  destruct x, b; try easy.
  contradict h2.
  apply Rlt_not_le, negative_non_zero_to_real; easy.
Qed.

(* Why3 goal *)
Lemma sqrt_finite_rev : forall (m:mode) (x:t), (is_finite (sqrt m x)) ->
  ((is_finite x) /\ ((0%R <= (to_real x))%R /\ ((to_real (sqrt m
  x)) = (round m (Reals.R_sqrt.sqrt (to_real x)))))).
  intros m x h1.
  assert (is_finite x) by (destruct x, b; easy).
  split; [easy|].
  assert (0 <= to_real x).
  { destruct x, b; try easy.
    rewrite (@to_real_B754_zero true); fourier.
    rewrite (@to_real_B754_zero false); fourier.
    assert (is_positive (B754_finite sb emax false m0 e e0)) by easy.
...
Lemma Bsign_overflow : forall {m b H},
    Bsign sb emax (FF2B sb emax (binary_overflow sb emax m b) H) = b.
  intros m b H.
  rewrite Bsign_FF2B.
...
(* Why3 goal *)
Lemma add_special : forall (m:mode) (x:t) (y:t), let r := (add m x y) in
  ((((is_nan x) \/ (is_nan y)) -> (is_nan r)) /\ ((((is_finite x) /\
  (is_infinite y)) -> ((is_infinite r) /\ (same_sign r y))) /\
  ((((is_infinite x) /\ (is_finite y)) -> ((is_infinite r) /\ (same_sign r
...
intros m x y r.
  ...
  - destruct x, y, b, b0; destruct h; easy.
  - destruct x, y, b, b0; auto; easy.
  - destruct x, y, b, b0; auto; easy.
  - destruct x, y, b, b0; destruct h2 as [(h2,h3)|(h2,h3)]; auto; easy.
  - destruct x, y, b, b0; destruct h2 as [(h2,h3)|(h2,h3)]; auto; easy.
  - rewrite no_overflow_Rabs_round_max_real, Rabs_round_max_real_emax in h2.
    ...
    + destruct x, y; try easy.
      ...
      * rewrite to_real_B754_zero at 1 2.
        rewrite Rplus_0_l.
        rewrite <-(@to_real_B754_zero false) at 1 2.
      ...
      * replace (add m (B754_finite sb emax b m0 e e0) (B754_finite sb emax b0 m1 e1 e2)) with r in H by auto.
        pose proof (sign_FF_overflow m (Bsign sb emax (B754_finite sb emax b m0 e e0))).
        rewrite <-H in H1.
        ...
        -- assert (to_real (B754_finite sb emax true m0 e e0) < 0) by
              (apply negative_non_zero_to_real; easy).
           assert (to_real (B754_finite sb emax true m1 e1 e2) < 0) by
               (apply negative_non_zero_to_real; easy).
           fourier.
        -- rewrite <-is_positive_Bsign in H1; auto.
        -- assert (0 < to_real (B754_finite sb emax false m0 e e0)) by
              (apply positive_non_zero_to_real; easy).
           assert (0 < to_real (B754_finite sb emax false m1 e1 e2)) by
               (apply positive_non_zero_to_real; easy).
           fourier.
    ...
    + replace (add m x y) with r in H by auto.
      ...
      * assert (Bsign sb emax x = true \/ Bsign sb emax x = false) by
        ...
        -- split.
           ...
           ++ rewrite <-min_real_is_F2R, <-FF2R_B2FF, H; auto.
      ...
      * assert (Bsign sb emax x = true \/ Bsign sb emax x = false) by
        ...
        -- split.
           ...
           destruct r; easy.
           replace (Z.pow_pos 2 sb_pos - 1)%Z with (Z.pos (2 ^ sb_pos - 1)) in H.
           ++ rewrite <-max_real_is_F2R, <-FF2R_B2FF, H; auto.
           ++ rewrite Pos2Z.inj_sub, Pos2Z.inj_pow_pos; auto.
              apply Pos.pow_gt_1.
              zify; auto with zarith.
           ++ assert (~is_nan r) by
  ...
  - destruct H.
    ...
    + rewrite Rlt_bool_false in H1; auto.
      ...
      replace (add m x y) with r in h1 by auto.
      destruct (is_nan_dec r).
      destruct x,b,y,b; try easy; destruct m;
  ...
  - intuition.
    ...
    + destruct H4.
      ...
      replace (add m x y) with r in H4 by auto.
...
(* Why3 goal *)
Lemma sub_special : forall (m:mode) (x:t) (y:t), let r := (sub m x y) in
  ((((is_nan x) \/ (is_nan y)) -> (is_nan r)) /\ ((((is_finite x) /\
  ...
  - rewrite no_overflow_Rabs_round_max_real, Rabs_round_max_real_emax in h2.
    ...
    + destruct x, y; try easy.
      ...
      * rewrite to_real_B754_zero at 1 2.
        rewrite Rminus_0_l.
        rewrite <-(@to_real_B754_zero false) at 1 2.
        destruct (neg_finite (B754_finite sb emax b0 m0 e e0)) as (_,h3); auto.
        rewrite <-h3.
      ...
      * rewrite to_real_B754_zero at 1 2.
        ...
        rewrite <-(@to_real_B754_zero false) at 1 2.
        destruct b;[right;split;[easy|]|left;split;[easy|]]; apply lt_finite; easy.
      * replace (sub m (B754_finite sb emax b m0 e e0) (B754_finite sb emax b0 m1 e1 e2)) with r in H by auto.
        pose proof (sign_FF_overflow m (Bsign sb emax (B754_finite sb emax b m0 e e0))).
        rewrite <-H in H1.
        rewrite sign_FF_B2FF in H1.
        simpl Bsign at 2 in H1.
    ...
    + replace (sub m x y) with r in H by auto.
      assert (H':=H).
      apply (f_equal sign_FF) in H'.
      rewrite sign_FF_B2FF, sign_FF_overflow in H'.
      destruct m; simpl;
  ...
  - destruct H.
    ...
    + rewrite Rlt_bool_true in H1; auto.
      destruct H1 as (h1,(h2,h3)).
      replace (sub m x y) with r in h1, h2, h3 by auto.
      assert (~is_nan r) by (destruct r; easy).
      destruct h as [(h,h')|(h,h')];[left|right];split;auto.
      * rewrite is_positive_Bsign; auto.
        destruct y,b,x,b; try easy.
    ...
    + rewrite Rlt_bool_false in H1; auto.
      destruct H1 as (h1,h2).
      replace (sub m x y) with r in h1 by auto.
      destruct (is_nan_dec r).
      destruct x,b,y,b; try easy; destruct m;
        unfold binary_overflow in h1; simpl in h1;
          destruct r; try easy; destruct n; easy.
  ...
  - intuition.
    ...
    + destruct H4.
      unfold binary_overflow in H4.
      replace (sub m x y) with r in H4 by auto.
      destruct H2 as [H2|H2]; rewrite H2 in H4; simpl in H4;
        destruct overflow_to_inf in H4; easy.
...
(* Why3 goal *)
Lemma mul_special : forall (m:mode) (x:t) (y:t), let r := (mul m x y) in
  ((((is_nan x) \/ (is_nan y)) -> (is_nan r)) /\ ((((is_zero x) /\
  (is_infinite y)) -> (is_nan r)) /\ ((((is_finite x) /\ ((is_infinite y) /\
  ~ (is_zero x))) -> (is_infinite r)) /\ ((((is_infinite x) /\ (is_zero
  y)) -> (is_nan r)) /\ ((((is_infinite x) /\ ((is_finite y) /\ ~ (is_zero
  y))) -> (is_infinite r)) /\ ((((is_infinite x) /\ (is_infinite y)) ->
  (is_infinite r)) /\ ((((is_finite x) /\ ((is_finite y) /\ ~ (no_overflow m
  ((to_real x) * (to_real y))%R))) -> (overflow_value m r)) /\ ((~ (is_nan
  ...
  - destruct x, y; easy.
  - destruct x, y, b; easy.
  - destruct x, y, b; try easy; contradict H2; easy.
  - destruct x, y, b0; easy.
  - destruct x, y, b0; try easy; contradict H2; easy.
  - destruct x, y; easy.
  - rewrite no_overflow_Rabs_round_max_real, Rabs_round_max_real_emax in H2.
    apply Rnot_lt_le in H2.
    pose proof (Bmult_correct sb emax Hsb' Hemax' nan_bf m x y).
    rewrite Rlt_bool_false in H1 by auto.
    replace (mul m x y) with r in H1 by auto.
    assert (H1':=H1).
    apply (f_equal sign_FF) in H1'.
    rewrite sign_FF_B2FF, sign_FF_overflow in H1'.
    destruct m; simpl;
    ...
    * assert (Bsign sb emax x = true \/ Bsign sb emax x = false) by
      ...
      -- split.
         ...
         ++ rewrite <-min_real_is_F2R, <-FF2R_B2FF, H1; auto.
         ++ rewrite Pos2Z.inj_sub, Pos2Z.inj_pow_pos; auto.
            apply Pos.pow_gt_1.
            zify; auto with zarith.
      -- split.
    ...
    * assert (Bsign sb emax x = true \/ Bsign sb emax x = false) by
      ...
      -- split.
         { intro; split.
           destruct r; try easy; destruct n; easy.
           replace (Z.pow_pos 2 sb_pos - 1)%Z with (Z.pos (2 ^ sb_pos - 1)) in H1.
           ++ rewrite <-max_real_is_F2R, <-FF2R_B2FF, H1; auto.
           ++ rewrite Pos2Z.inj_sub, Pos2Z.inj_pow_pos; auto.
              apply Pos.pow_gt_1.
              zify; auto with zarith. }
         assert (~is_nan r) by
    ...
    * assert (Bsign sb emax x = true \/ Bsign sb emax x = false) by
      ...
      assert (Bsign sb emax y = true \/ Bsign sb emax y = false) by
          (destruct y, b; simpl; auto).
      replace (Z.pow_pos 2 sb_pos - 1)%Z with (Z.pos (2 ^ sb_pos - 1)) in H1.
      { rewrite <-min_real_is_F2R,<-max_real_is_F2R, <-FF2R_B2FF, H1; auto.
  ...
  - pose proof (Bmult_correct sb emax Hsb' Hemax' nan_bf m x y).
    ...
    + destruct H1 as (h1, (h2, h3)).
      assert (is_nan (mul m x y) = false).
      { unfold r in H.
        destruct (mul m x y); auto.
        simpl in H. contradict H; auto. }
      apply h3 in H1.
      rewrite is_positive_Bsign; auto.
      unfold r.
      destruct x,b,y,b; easy.
    + apply (f_equal sign_FF) in H1.
      rewrite sign_FF_B2FF, sign_FF_overflow in H1.
      rewrite is_positive_Bsign; auto.
      destruct x,b,y,b; easy.
...
Lemma is_zero_B754_zero :
  forall x, is_zero x <-> exists b, x = B754_zero sb emax b.
  intro.
...
(* Why3 goal *)
Lemma div_special : forall (m:mode) (x:t) (y:t), let r := (div m x y) in
  ((((is_nan x) \/ (is_nan y)) -> (is_nan r)) /\ ((((is_finite x) /\
  (is_infinite y)) -> (is_zero r)) /\ ((((is_infinite x) /\ (is_finite y)) ->
  (is_infinite r)) /\ ((((is_infinite x) /\ (is_infinite y)) -> (is_nan
  r)) /\ ((((is_finite x) /\ ((is_finite y) /\ ((~ (is_zero y)) /\
  ~ (no_overflow m ((to_real x) / (to_real y))%R)))) -> (overflow_value m
  r)) /\ ((((is_finite x) /\ ((is_zero y) /\ ~ (is_zero x))) -> (is_infinite
  ...
  - assert (to_real y <> 0) by (rewrite zero_to_real in H1; auto).
    ...
    rewrite Rlt_bool_false in H4.
    + unfold binary_overflow in H4.
      replace (div m x y) with r in H4; auto.
      destruct m; simpl in *.
      * destruct r; try easy.
      ...
      * destruct (xorb (Bsign sb emax x) (Bsign sb emax y)); simpl in H4;
        split; intro.
        -- destruct r, b; easy.
        -- split;[destruct r, b; try easy; destruct n; easy|].
           apply (f_equal (FF2R radix2)) in H4.
           rewrite FF2R_B2FF in H4.
           simpl in H4.
           replace (Z.pow_pos 2 sb_pos - 1)%Z with (Z.pos (2 ^ sb_pos - 1)) in H4 by
        ...
        -- split; [destruct r; try easy| ].
           ...
           rewrite <-max_real_is_F2R; assumption.
        -- contradict H5.
           destruct r, b; try easy; destruct n; easy.
    + clear H4.
  ...
  - rewrite is_zero_B754_zero in H0; destruct H0.
    ...
    destruct x, y; easy.
  - destruct x, b, y, b; try easy.
    contradict H; auto.
    contradict H; auto.
    set (x:=B754_finite sb emax false m0 e e0);
      set (y:=B754_finite sb emax false m1 e1 e2); fold x y in r, H0, H2.
    assert (to_real y <> 0).
    ...
    + destruct H3 as (_,(_,h)).
      assert (is_nan (div m x y) = false).
      { unfold r in H.
        destruct (div m x y); auto.
        simpl in H; contradict H; auto. }
      rewrite is_positive_Bsign; auto.
  ...
  - destruct x, b, y, b; try easy.
    contradict H; auto.
    contradict H; auto.
    set (x:=B754_finite sb emax true m0 e e0);
      set (y:=B754_finite sb emax true m1 e1 e2); fold x y in r, H0, H2.
    assert (to_real y <> 0).
...
Lemma fma_special : forall (m:mode) (x:t) (y:t) (z:t), let r := (fma m x y
  ...
  (((m = RTN) -> (is_negative r)) /\ ((~ (m = RTN)) -> (is_positive
  r)))))))))))))))))).
intros m x y z r.
Admitted.
...
Lemma sqrt_special : forall (m:mode) (x:t), let r := (sqrt m x) in (((is_nan
  ...
  (((is_minus_infinity x) -> (is_nan r)) /\ ((((is_finite x) /\
  ((to_real x) < 0%R)%R) -> (is_nan r)) /\ (((is_zero x) -> (same_sign r
  x)) /\ (((is_finite x) /\ (0%R < (to_real x))%R) -> (is_positive r))))))).
  intros m x r.
  intuition.
  - destruct x; easy.
  ...
  - unfold is_minus_infinity in H.
    destruct x, b; easy.
  - destruct x, b; try easy.
    rewrite to_real_B754_zero in H1; fourier.
    rewrite to_real_B754_zero in H1; fourier.
  ...
  - destruct (Bsqrt_correct sb emax Hsb' Hemax' (fun b => (true,One_Nan_pl)) m x) as (_,(h1,h2));
      fold r in h1, h2.
    destruct x; try easy.
    rewrite to_real_B754_zero in H1 at 1; fourier.
    apply (to_real_positive _ H0) in H1.
    rewrite is_positive_Bsign in H1 by easy.
    simpl in H1.
    destruct b; try easy.
    assert (is_nan r = false) by (destruct r; easy).
    rewrite is_positive_Bsign; auto.
    rewrite H; easy.
Qed.
...
Lemma B754_zero_is_int : forall {b}, (is_int (B754_zero sb emax b)).
  split;auto.
  rewrite to_real_B754_zero.
  unfold is_intR.
...
Lemma le_infinity: forall {x}, ~ is_nan x -> le x (B754_infinity _ _ false).
  intros.
  unfold le.
  destruct x; try easy.
  + left; easy.
...
Lemma abs_le: forall {x y}, le (neg y) x /\ le x y -> le (abs x) y.
  ...
  + destruct (Finite_Infinite_Nan_dec x) as [[hx|hx]|hx].
    ...
    * intros (a,b).
      ...
      - pose proof (le_special _ _ a).
        unfold is_minus_infinity, is_plus_infinity, neg in H0.
        destruct H0, H0, x, y; try easy; destruct b0, b1; easy.
  ...
  + intros (a,b).
    ...
    * destruct (le_special _ _ b) as [H|[H|H]].
      ...
      - destruct y, H as (H,(_,H')), b0; try easy.
        assert (~ is_nan x) by (rewrite <-is_not_nan1; assumption).
        apply (le_infinity (abs_not_nan H0)).
    * destruct x, y; try easy.
      unfold le, Bcompare.
  ...
  + intros a.
    ...
    * pose proof (is_finite_abs _ hx).
      ...
      - destruct y, H1; try easy.
        ...
        destruct x, b; easy.
    * unfold le, neg, Bcompare.
      destruct x, y, b, b0; simpl; split; auto; easy.
    * destruct x; unfold le, abs, Bcompare in a; try easy.
...
Lemma is_finite_range_to_int : forall {x} m, (is_finite x) -> (in_range (IZR (to_int m x))).
  ...
  split; apply IZR_le; apply to_int_le; auto.
Qed.
...
Lemma to_int_of_int_generic_format: forall {i} m, (- max_int <= i <= max_int)%Z -> generic_format radix2 fexp (Z2R (to_int m (of_int m i))).
  intros i m h.
  pose proof (of_int_is_int m i h).
  pose proof (int_to_real m _ H).
  rewrite Z2R_IZR, <-H0.
  apply generic_format_B2R.
Qed.
...
Lemma roundToIntegral_is_int : forall (m:mode) (x:t), (is_finite x) ->
  ...
  + simpl.
    ...
    rewrite <-FromInt.Zero, Floor_int; reflexivity.
    change (is_int (of_int RTZ (to_int m (B754_finite sb emax b m0 e e0)))).
    apply of_int_is_int.
    apply is_finite_to_int2.
    assumption.
Qed.

Lemma to_int_B754_zero: forall {b m}, to_int m (B754_zero _ _ b) = 0%Z.
  intros b m.
  rewrite <-(to_int_eq zero_is_zero).
  apply to_int_zeroF.
Qed.

Lemma eq_zero': forall {x b0}, eq x (B754_zero _ _ b0) -> exists b, x = B754_zero _ _ b.
  intros x h.
  unfold eq, Bcompare, zeroF in h.
  destruct x, b; try easy.
...
Lemma to_real_roundToIntegral: forall {x} m, is_finite x -> to_real (roundToIntegral m x) = Z2R (to_int m x).
  ...
  destruct x; try easy.
  rewrite to_real_B754_zero, e; auto.
  rewrite to_real_B754_zero, e; auto.
  ...
  set (x:= B754_finite sb emax b m0 e e0); fold x in h, h.
  change (to_real (of_int RTZ (to_int m x)) = Z2R (to_int m x)).

  pose proof Bmax_rep_int_finite.
...
(* add to theory
 to_int m (roundToIntegral m x) = to_int m x ??*)

(* Why3 goal *)
Lemma truncate_int : forall (m:mode) (i:t), (is_int i) -> (eq
...
Lemma roundToIntegral_eq_to_int: forall {x y} {m:mode}, is_finite x -> is_finite y ->
    to_int m x = to_int m y -> eq (roundToIntegral m x) (roundToIntegral m y).
  intros x y m h1 h2.
  destruct x, y; try easy.
  - rewrite to_int_B754_zero.
    simpl roundToIntegral.
    intro h; rewrite <-h.
    replace (binary_normalize sb emax Hsb' Hemax' mode_ZR 0 0 false) with (of_int RTZ 0%Z) by auto.
    rewrite <-zero_of_int.
    symmetry; apply eq_zero.
  - rewrite to_int_B754_zero.
    simpl roundToIntegral.
    intro h; rewrite h.
    replace (binary_normalize sb emax Hsb' Hemax' mode_ZR 0 0 false) with (of_int RTZ 0%Z) by auto.
...
(* Why3 goal *)
Lemma ceil_le : forall (x:t), (is_finite x) -> (le x (roundToIntegral RTP
  x)).
intros x h1.
...
(* Why3 goal *)
Lemma ceil_to_int : forall (m:mode) (x:t), (is_finite x) -> ((to_int m
  (roundToIntegral RTP x)) = (real.Truncate.ceil (to_real x))).
  intros m x h.
  unfold to_int.
  rewrite to_real_roundToIntegral; auto.
...
(* Why3 goal *)
Lemma floor_le : forall (x:t), (is_finite x) -> (le (roundToIntegral RTN x)
  x).
  intros x h1.
...
(* Why3 goal *)
Lemma floor_to_int : forall (m:mode) (x:t), (is_finite x) -> ((to_int m
  (roundToIntegral RTN x)) = (real.Truncate.floor (to_real x))).
  intros m x h.
  unfold to_int.
  rewrite to_real_roundToIntegral; auto.
  destruct (valid_rnd_round_mode m) as (_,h').
...
Lemma zero_or_not : forall x, ~ is_zero x \/ is_zero x.
  destruct x, b; try (right; easy); left; simpl; try easy.
Qed.
...
Lemma is_negative_to_int: forall {x} {m:mode}, is_finite x -> is_negative x -> (to_int m x <= 0)%Z.
  ...
  destruct m; simpl in h2; apply h2; rewrite zeroF_to_real; apply negative_to_real; auto.
Qed.
...
Lemma is_positive_to_int: forall {x} {m:mode}, is_finite x -> is_positive x -> (0 <= to_int m x)%Z.
  ...
  rewrite <-(to_int_zeroF m).
  destruct (valid_rnd_round_mode m) as (h2,_).
  destruct m; simpl in h2; apply h2; rewrite zeroF_to_real; apply positive_to_real; auto.
Qed.
...
(* Why3 goal *)
Lemma RNA_down : forall (x:t), (lt (sub RNE x (roundToIntegral RTN x))
  (sub RNE (roundToIntegral RTP x) x)) -> ((roundToIntegral RNA
  x) = (roundToIntegral RTN x)).
  intros x h.
  destruct x; try easy.
  set (x:=(B754_finite sb emax b m e e0)); fold x in h.
  assert (forall m', is_finite (roundToIntegral m' x)) as h1.
  intro m'.
  apply roundToIntegral_finite; auto.
  apply to_real_refl; auto.
  2: apply same_sign_roundToIntegral2; easy.

  rewrite to_real_roundToIntegral, to_real_roundToIntegral; auto.
  f_equal.
  unfold to_int.
  apply Znearest_imp.
  rewrite Rabs_pos_eq.
  2: pose proof (Zfloor_lb (to_real x)); fourier.

  assert (is_finite x) as x_fin by auto.
...
(* Why3 goal *)
Lemma RNA_up : forall (x:t), (lt (sub RNE (roundToIntegral RTP x) x) (sub RNE
  ...
  apply Znearest_imp.
  rewrite Rabs_left1.
  2: pose proof (Zceil_ub (to_real x)); fourier.
...
Lemma is_int_or_not: forall (x:t), is_int x \/ ~ is_int x.
  ...
  destruct x.
  - left.
    unfold is_int; split; auto.
    rewrite to_real_B754_zero.
    unfold is_intR.
    replace 0 with (Z2R 0%Z) at 2; auto.
...
Lemma eq_diff_floor_ceil: forall {x}, eq (sub RNE x (roundToIntegral RTN x)) (sub RNE (roundToIntegral RTP x) x) -> to_real x - Z2R (floor (to_real x)) = Z2R (ceil (to_real x)) - to_real x.
  ...
  - set (x:=(B754_finite sb emax b m e e0)); fold x in h.
    assert (forall m', is_finite (roundToIntegral m' x)) as h1 by
          (intro m'; apply roundToIntegral_finite; auto).
    ...
    + destruct (Rle_lt_dec (to_real x) 0).
      ...
      * assert (floor (to_real x) = (-1)%Z) by
        ...
        rewrite (to_real_roundToIntegral RTP) in h; auto.
        unfold to_int in h.
        rewrite H1 in *.
        simpl Z2R at 2; ring_simplify.
        simpl Z2R in h.
        replace (0 - to_real x) with (- to_real x) in h by ring.
        rewrite <-(proj2 (neg_finite _ x_fin)) in h.
        rewrite (Round_to_real RNE) in h by auto.
        rewrite (proj2 (neg_finite _ x_fin)) in h.
        ...
        -- assert (
           ...
           rewrite aux1 in h.
           rewrite to_real_roundToIntegral in h; auto.
      ...
      * assert (floor (to_real x) = 0%Z) by
        ...
        rewrite (to_real_roundToIntegral RTN) in h; auto.
        unfold to_int in h.
        rewrite H in *.
        simpl Z2R at 1; ring_simplify.
        simpl Z2R in h.
        replace (to_real x - 0) with (to_real x) in h by ring.
        rewrite (Round_to_real RNE) in h by auto.
...
(* Why3 goal *)
Lemma RNA_down_tie : forall (x:t), (eq (sub RNE x (roundToIntegral RTN x))
  (sub RNE (roundToIntegral RTP x) x)) -> ((is_negative x) ->
  ((roundToIntegral RNA x) = (roundToIntegral RTN x))).
  intros x h h'.
  destruct x; try easy.
  set (x:=(B754_finite sb emax b m e e0)); fold x in h, h'.
  assert (forall m', is_finite (roundToIntegral m' x)) as h1 by
        (intro m'; apply roundToIntegral_finite; auto).
  apply to_real_refl; auto.
  2: apply same_sign_roundToIntegral2; easy.
...
(* Why3 goal *)
Lemma RNA_up_tie : forall (x:t), (eq (sub RNE (roundToIntegral RTP x) x)
  ...
  apply positive_non_zero_to_real in h'; auto.
  apply Zfloor_lub.
  simpl Z2R; fourier.
  unfold is_zero.
  unfold eq. simpl.
...
Lemma pow_sb_bounded : bounded sb emax (2 ^ (sb_pos - 1)) 1 = true.
  assert (0 <= sb - 1)%Z.
  apply Zlt_0_le_0_pred, Hsb'.
  unfold bounded; apply Bool.andb_true_iff; split.
  unfold canonic_mantissa, FLT_exp.
  ...
  rewrite Fcore_digits.Zpos_digits2_pos.
  rewrite (Fcore_digits.Zdigits_unique radix2 _ sb).
  replace (sb + 1 - sb)%Z with 1%Z by omega.
  apply Zmax_left.
  assert (2 <= sb)%Z by (pose proof sb_gt_1; auto with zarith).
  pose proof Hemax'; omega.
  rewrite Z.abs_eq by auto with zarith.
  change ((2 ^ (sb - 1) <= Z.pos (2 ^ (sb_pos - 1)) < 2 ^ sb)%Z).
  assert (Z.pos (2 ^ (sb_pos - 1)) = 2 ^ (sb -1))%Z.
  rewrite Pos2Z.inj_pow, Pos2Z.inj_sub; auto.
  pose proof sb_gt_1.
  unfold sb in H0; auto with zarith.
  rewrite H0.
  split; auto with zarith.
  assert (sb = Z.succ (sb - 1)) by auto with zarith.
  rewrite H1 at 2.
  ...
  apply Z.lt_gt, (Zpower_gt_0 radix2 (sb - 1));trivial.
  apply Zle_bool_true.
  pose proof Hemax'; omega.
Qed.

Lemma bpow_sb_to_real: bpow radix2 sb = to_real  (B754_finite _ _ false _ _ pow_sb_bounded).
  unfold to_real, F2R, Fnum, Fexp.
  simpl cond_Zopp; simpl bpow at 2.
  rewrite Pos2Z.inj_pow.
  rewrite <-Z_pos_sub_gt, <-Pos2Z.add_pos_neg; fold sb.
  change (bpow radix2 sb = (Z2R (radix2 ^ (sb + - 1)) * 2)).
  rewrite Z2R_Zpower.
  rewrite bpow_plus. simpl bpow at 3; field.
  pose proof sb_gt_1; omega.
  pose proof sb_gt_1; unfold sb in H; apply Z.lt_gt in H; auto.
Qed.

Lemma bpow_sb_le_max_real: bpow radix2 sb <= max_real.
  rewrite bpow_sb_to_real.
  rewrite <-max_value_to_real.
  apply le_finite; auto.
  assert (B754_finite sb emax false (2 ^ (sb_pos - 1)) 1 pow_sb_bounded =
          abs (B754_finite sb emax false (2 ^ (sb_pos - 1)) 1 pow_sb_bounded))
    by easy.
  rewrite H; apply bounded_floats_le; auto.
Qed.

Lemma One_lt_ulp_max_real: (1 < ulp radix2 fexp max_real).
  apply Rle_lt_trans with (r2:= Rabs max_real * bpow radix2 (-sb)).
  pose proof bpow_sb_le_max_real.
  rewrite Rabs_right.
  2:apply Rle_ge, max_real_ge_zero.
  rewrite bpow_opp.
  apply Rmult_le_reg_r with (r:=(bpow radix2 sb)).
  apply bpow_gt_0.
  field_simplify.
  fourier.
  apply not_eq_sym, Rlt_not_eq, bpow_gt_0.

  apply ulp_FLT_ge, Hsb''.
Qed.

Lemma Znearest_half_away: forall {i x:R},
    is_intR i -> (i - /2 < x) /\ (x < i + /2) ->
    ZnearestA x = ZnearestA i.
  intros i x h (h1,h2).
  unfold is_intR in h.
  destruct (Rle_dec i x).
  + destruct (Rle_lt_or_eq_dec _ _ r); clear r.
    - rewrite (inbetween_int_NA x (floor i) (loc_Inexact Lt)).
      * simpl.
        destruct (valid_rnd_round_mode RNA).
        simpl in Zrnd_Z2R.
        rewrite h at 2; rewrite (Zrnd_Z2R (floor i)).
        reflexivity.
      * unfold inbetween_int.
        rewrite Z2R_plus, <-h; simpl.
        apply inbetween_Inexact.
        split; fourier.
        apply Rcompare_Lt.
        rewrite <-Rplus_assoc, Rdiv_plus_distr, <-double.
        replace (2*i/2) with i by field.
        fourier.
    - rewrite <-e.
      f_equal; ring.
  + apply Rnot_le_lt in n.
    rewrite (inbetween_int_NA x ((floor i) - 1) (loc_Inexact Gt)).
    * simpl.
      ring_simplify.
      destruct (valid_rnd_round_mode RNA).
      simpl in Zrnd_Z2R.
      ...
      reflexivity.
    * unfold inbetween_int.
      rewrite Z2R_minus, Z2R_plus, Z2R_minus, <-h; simpl.
      replace (i - 1 + 1) with i by ring.
      apply inbetween_Inexact.
      split; fourier.
      apply Rcompare_Gt.
      ring_simplify (i -1 +i).
      replace ((2*i-1)/2) with (i - 1/2) by field.
      fourier.
Qed.

(* Why3 goal *)
Lemma to_int_roundToIntegral : forall (m:mode) (x:t), ((to_int m
  ...
  rewrite to_real_roundToIntegral; auto.
  destruct (valid_rnd_round_mode m) as (_,h).
  pose proof (h (to_int m (B754_finite sb emax b m0 e e0))).
  destruct m; auto.
Qed.

(* Why3 goal *)
Lemma to_int_monotonic : forall (m:mode) (x:t) (y:t), (is_finite x) ->
  ((is_finite y) -> ((le x y) -> ((to_int m x) <= (to_int m y))%Z)).
intros m x y h1 h2 h3.
apply to_int_le; auto.
Qed.
