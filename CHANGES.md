...
Core
  * time taken by solvers is now uniformly reported with the precision
    of a microsecond, at least internally, in particular in session
    files. The statistics with cumulated times computed e.g. by `why3
    session info --provers-stats` are more precise. Additionally, the
...
WhyML language
  * Module interfaces: `module Impl : Intf ... end` can be used to hide the
...
Input formats
  * New input format `.sexp` for programs written as S-expressions. It
    is not intended to be used for handwritten programs but when Why3
    is used as an intermediate language. This input format is
    compatible with the output of `why3 pp --output=sexp` and the
    S-expressions generated by the API function
