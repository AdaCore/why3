...
let translate_cfg preconds block (blocks : (label * block) list) =
  let blocks =
    List.fold_left (fun acc (l, b) -> Wstdlib.Mstr.add l.id_str b acc) Wstdlib.Mstr.empty blocks
  in
  ...
  let rec translate_term startlabel visited_terms (t : cfg_term) : Ptree.expr =
    ...
    if List.memq t visited_terms then begin
      ...
      let msg =
        if startlabel = thestartlabel then msg else msg ^ " starting from `" ^ startlabel ^ "`"
      in
            ...
            | CFGinvariant l, _ ->
                ...
                let l =
                  List.map
                    ...
                    l
                in
                if not (List.mem id.id_str !visited_entry_points) then begin
                  visited_entry_points := id.id_str :: !visited_entry_points;
                  traverse_from_entry_point id.id_str l (rem, term)
                end;
                let k =
                  mk_expr ~loc
                    (Eidapp (Qident (mk_id ~loc ("_from_" ^ id.id_str)), [ mk_unit ~loc ]))
                in
                List.fold_right
                  (fun t acc ->
